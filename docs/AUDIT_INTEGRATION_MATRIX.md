...
| 221 | HIGH | SMS-5: no per-user (Firebase UID) SMS cap | multiple SMS senders | — | — | DEFERRED — needs shared per-uid cap middleware; own lane |
...
| 223 | MED | SMS-10: rate-limiter in-memory per-instance store | `server/middleware/rateLimiter.ts` | — | — | DEFERRED — needs Redis store swap; own lane |
