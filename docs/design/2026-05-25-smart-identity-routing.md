...
| Cross-site request forgery on session APIs | `doubleCsrfProtection` (from the `csrf-csrf` library) is mounted globally in `server/index.ts` with an explicit skip list. Auth POSTs are covered by the app-wide gate. (Historical note: this row previously pointed at `webauthn/csrfProtection.ts`, a hand-rolled helper that was never mounted; it was deleted 2026-08-15 in PR-AUTH-CSRF-DELETE-DEAD-MIDDLEWARE.) |
