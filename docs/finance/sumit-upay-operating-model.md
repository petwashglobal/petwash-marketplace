...
- Pet Wash issues a **קבלה** (or חשבונית מס/קבלה) to the customer for the wash. The receipt issuer is **Pet Wash Ltd, company № 517145033** (correcting forensic-audit finding F-104's hard-coded mismatch).
...
E-gift is **stored value of the customer**, not regular income at purchase time. Already coherent with `02-wallet-redesign.md` (`wallet.gift_card_received` bucket) and `00-platform-role-model.md` 0.4.3 (deferred-obligation stance). This section adds the **lifecycle** that those docs left implicit.

### 7.2 The 10 lifecycle steps (each is a separate ledger event)

1. **Purchase through UPay/SUMIT.** Customer pays Pet Wash for an e-gift of nominal value V. Money in. **Ledger:** `cash_in_transit_upay` debit, `wallet.gift_card_received` credit (deferred liability). **Document:** receipt or חשבונית מס issued per CPA decision (§10 Q1). **What is NOT created:** no `wallet.cash` credit; no provider record; no booking record.
