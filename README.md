...
Two native apps are built from the web bundle: **Customer** (`il.co.petwash.customer`) and **Provider** (`il.co.petwash.provider`). Each has its own Capacitor config under `config/capacitor/` and its own native project (`ios/`, `ios-customer/`, `android/`, `android-customer/`).

**Local iOS / Android workflow** (from repo root):

```bash
npm run cap:customer:sync        # build web + copy into ios-customer + android-customer
npm run cap:customer:open:ios    # open Customer in Xcode
npm run cap:customer:open:android

npm run cap:provider:sync        # same for Provider
npm run cap:provider:open:ios
npm run cap:provider:open:android
```

The sync step is **required before opening Xcode**: `capacitor.config.json`, `config.xml`, and `public/` under each native project's `App/App/` are intentionally gitignored and regenerated by `cap:*:sync`. Without it, Xcode reports missing files at build time. CI handles this automatically; only local builds need to run sync explicitly.

See `docs/mobile/customer-capacitor-foundation-2026-06-05.md` and `docs/mobile/provider-capacitor-foundation-2026-06-05.md` for full mobile foundation details.
