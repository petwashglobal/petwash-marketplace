...
- **P0 Fintech Auth System**: User status state machine (11 states), server-stored intent validation, authorization gates middleware (8 functions), audit trail with trace IDs, KYC pipeline tables.
- **Wiring Matrix**: 11 new user columns, 9 new tables (consent_snapshots, onboarding_cases, services_catalog, security_events, provider_profiles, provider_assets, dispatch_jobs, payments, ledger_entries). 13 storage methods. Post-login wiring with onboarding cases, security events, requiredActions.
- **Consent Engine**: SHA-256 evidence hash, immutable snapshots (DB rules prevent UPDATE/DELETE), role-based consent requirements, consentGate middleware.
- **Security Events Service**: Specialized logging for MFA, login anomaly, KYC velocity, rate limiting, token reuse with risk scoring.
- **Hardening Phase**: All gate rejections log security events. State machine guards (requireOnboardingComplete, requireKycApproved, requireProviderCanAcceptBooking). Ledger immutability (DB rules + hash chain enforcement). Rate limiters per route group (auth 10/min, KYC 5/hr, booking 20/15min, dispatch 30/15min, OTP 3/5min). Incident mode system (super admin activate/deactivate, blocks critical actions). Region-aware data (region_code, legal_hold). Soft delete with anonymization service. Competitor names removed from all comments. DB-level immutability on ledger_entries and consent_snapshots.