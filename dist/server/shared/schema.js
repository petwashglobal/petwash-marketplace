import { pgTable, pgEnum, text, varchar, timestamp, jsonb, index, uniqueIndex, integer, decimal, boolean, serial, date, real, unique } from "drizzle-orm/pg-core";
...
export const sitterProfiles = pgTable("sitter_profiles", {
    ...
    pricingPackages: jsonb("pricing_packages").$type(),
    ...
    extraServices: jsonb("extra_services").$type(),
    ...
    availabilityCalendar: jsonb("availability_calendar").$type(),
...
export const walkerProfiles = pgTable("walker_profiles", {
    ...
    walkPackages: jsonb("walk_packages").$type(),
...
export const pettrekProviders = pgTable("pettrek_providers", {
    ...
    transportLookupIdx: index("idx_pettrek_providers_transport_lookup").on(table.isOnline, table.offersTransport, table.isAvailable, table.isVetted),
}));
...
export const handbookCategories = pgTable("handbook_categories", {
    ...
    nameTranslations: jsonb("name_translations").$type(), // { en, he, ar, ru, fr, es }
    description: text("description"),
    descriptionTranslations: jsonb("description_translations").$type(),
...
export const handbookVersions = pgTable("handbook_versions", {
    ...
    mediaUrls: jsonb("media_urls").$type(), // Array of CDN/asset URLs
...
export const handbookTasks = pgTable("handbook_tasks", {
    ...
    title: varchar("title", { length: 500 }).notNull(),
    titleTranslations: jsonb("title_translations").$type(),
    description: text("description"),
    descriptionTranslations: jsonb("description_translations").$type(),
...
export const staffExpenses = pgTable("staff_expenses", {
    ...
    fraudFlags: jsonb("fraud_flags").$type(), // duplicate, fake, manipulated, excessive, etc.
    // Approval Workflow
...
export const staffLogbook = pgTable("staff_logbook", {
    ...
    startLocation: jsonb("start_location").$type(),
    endLocation: jsonb("end_location").$type(),
    gpsVerified: boolean("gps_verified").default(false),
    gpsVerificationNotes: text("gps_verification_notes"),
    // Job Details
    description: text("description"),
    clientName: varchar("client_name", { length: 255 }),
    petNames: jsonb("pet_names").$type(),
    // Approval
...
export const jobOffers = pgTable("job_offers", {
    ...
    completedAt: timestamp("completed_at"),
    // Audit Trail
    offerHistory: jsonb("offer_history").$type(),
    // Additional Metadata
    petIds: jsonb("pet_ids").$type(),
    specialInstructions: text("special_instructions"),
...
export const PETWASH_PLATFORMS = {
    sitter_suite: {
