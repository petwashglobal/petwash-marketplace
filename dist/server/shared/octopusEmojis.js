...
export function getActionEmoji(action) {
    ...
    return OCTOPUS_ACTIONS[upperAction] || '•';
}
/**
 * Get admin level emoji by role
 * @example getAdminEmoji('SUPER_ADMIN') // 👑
 */
export function getAdminEmoji(role) {
    const upperRole = role.toUpperCase();
    return OCTOPUS_ADMIN[upperRole] || OCTOPUS_ADMIN.STANDARD_USER;
}
/**
...
export function getRiskEmoji(level) {
    if (level === 'HIGH' || level === 'CRITICAL')
        return OCTOPUS_SECURITY.FRAUD_ALERT;
    if (level === 'MEDIUM')
        return OCTOPUS_SECURITY.WARNING;
 ...
 */
export function formatAuditLog(adminLevel, action, resource, userId, ip) {
...
export const LUXURY_MATERIALS = {
    ADMIN_SUPER: 'gold', // 👑 Royal Gold
    ADMIN_EXECUTIVE: 'diamond', // 💎 VVS Diamond
    ADMIN_REGIONAL: 'gold', // 🥇 Olympic Gold
    ADMIN_FRANCHISE: 'platinum', // 🥈 Platinum Silver
    ADMIN_STANDARD: 'champagne', // 🥉 Bronze Champagne
    SECURITY_HIGH: 'gold', // 🚨 Alert Gold
    SECURITY_MEDIUM: 'champagne', // ⚠️ Warning Champagne
    SECURITY_SAFE: 'diamond', // ✅ Safe Diamond
    PAYMENT_PREMIUM: 'gold', // 💳 Premium Gold
    PAYMENT_REFUND: 'rose-gold', // 💰 Refund Rose
    PAYMENT_LOYALTY: 'diamond', // 💎 Loyalty Diamond
};
