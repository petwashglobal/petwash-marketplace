 ...
 */
export async function trackChatInteraction(interaction) {
    try {
        ...
        await learnFromInteraction(safeInteraction);
 ...
 */
function detectTopic(question, language) {
 ...
 * Update topic statistics
 */
async function updateTopicStatistics(topic, language) {
    try {
 ...
 */
function calculateSatisfaction(followUpQuestion, timeToRead) {
    ...
    if (followUpQuestion) {
        ...
        const needsClarification = clarificationWords.some(word => followUpQuestion.toLowerCase().includes(word));
 ...
 */
function generateFAQId(question, topic, language) {
    // Normalize question
 ...
 */
export async function getLearnedFAQAnswer(question, language) {
    try {
 ...
 */
export async function getUserBehaviorInsights(language) {
    try {
        // Get top topics
        let topicsQuery = adminDb.collection('ai_topic_stats').orderBy('count', 'desc').limit(10);
        if (language) {
            topicsQuery = topicsQuery.where('language', '==', language);
        }
 ...
 */
export async function getProblematicFAQs(language) {
    try {
        let query = adminDb
            .collection('ai_faq_learning')
 ...
 */
export function anonymizeInteraction(interaction) {
