...
class LinkChecker {
    ...
    srcDir = path.join(process.cwd(), 'client/src');
    /**
     * Check if asset file exists
     */
    checkAssetExists(assetPath) {
    ...
    async runAllChecks() {
        ...
        const byType = this.results.reduce((acc, result) => {
            ...
            if (result.status === 'valid')
                acc[result.type].valid++;
            if (result.status === 'broken')
                acc[result.type].broken++;
            if (result.status === 'warning')
                acc[result.type].warnings++;
            return acc;
        }, {});
        const summary = {
     ...
     */
    generateHTMLReport(checkResults) {
