...
import { translateWithGemini, batchTranslate, getTranslationMetrics, getQualityScore, healthCheck } from '../services/geminiTranslation';
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await translateWithGemini(text, targetLanguage, sourceLanguage, context);
...
router.post('/batch', async (req, res) => {
    try {
        ...
        const results = await batchTranslate(texts, targetLanguage, sourceLanguage, context);
...
router.post('/verify', async (req, res) => {
    try {
      ...
      Preserve brand names like "Pet Wash™", "K9000™", "The Sitter Suite™", etc.`;
        const result = await translateWithGemini(englishText, targetLanguage, 'en', brandContext);
        if (!result.success) {
            return res.status(500).json({
