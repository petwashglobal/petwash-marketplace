...
import { db } from '../db';
import { providerApplicants, providerDocuments, providerBackgroundChecks, providerOnboardingTasks, providerStageTransitions, providerApplicationFormSchema } from '@shared/schema-enterprise';
...
function getRequiredDocuments(serviceTypes) {
...
async function createOnboardingTasks(applicantId, serviceTypes) {
...
function getHebrewDocName(docType) {
    const names = {
...
async function recordStageTransition(applicantId, fromStage, toStage, triggeredByUid, reason, metadata = {}, req) {
...
router.post('/', async (req, res) => {
    try {
        ...
        await recordStageTransition(application.id, 'application_submitted', 'documents_pending', userId, 'Application submitted, awaiting document uploads', { submittedFrom: 'web' }, req);
        ...
        try {
            ...
            await sendProviderEnrollmentConfirmation(formData.email, formData.firstName, formData.lastName, formData.serviceTypes, application.id, language);
...
router.post('/withdraw', async (req, res) => {
    try {
        ...
        await recordStageTransition(application.id, previousStage, 'withdrawn', userId, 'Applicant withdrew their application', { reason: req.body.reason || 'User requested' }, req);
...
router.get('/admin/list', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (stage && stage !== 'all') {
            conditions.push(eq(providerApplicants.stage, stage));
        }
        if (status && status !== 'all') {
            conditions.push(eq(providerApplicants.status, status));
        }
...
router.post('/admin/:id/approve', async (req, res) => {
    try {
        ...
        await db.update(providerApplicants)
            ...
            .where(eq(providerApplicants.id, applicationId));
        await recordStageTransition(applicationId, previousStage, 'approved', user.uid, `Approved by admin: ${notes || 'No notes'}`, { approvedBy: user.email }, req);
...
router.post('/admin/:id/reject', async (req, res) => {
    try {
        ...
        await db.update(providerApplicants)
            ...
            .where(eq(providerApplicants.id, applicationId));
        await recordStageTransition(applicationId, previousStage, 'rejected', user.uid, `Rejected: ${reason}`, { rejectedBy: user.email }, req);
...
router.post('/admin/:id/advance-stage', async (req, res) => {
    try {
        const user = req.firebaseUser;
        if (!user?.accountType || !['internal', 'admin'].includes(user.accountType)) {
            return res.status(403).json({ error: 'Admin access required' });
        ...
        await db.update(providerApplicants)
            ...
            .where(eq(providerApplicants.id, applicationId));
        await recordStageTransition(applicationId, previousStage, targetStage, user.uid, notes || `Stage advanced by admin`, {}, req);
