...
import { db } from '../db';
import { trainers, trainerBookings, contractorReviews, insertTrainerSchema, insertTrainerBookingSchema, } from '@shared/schema';
...
router.get('/trainers', async (req, res) => {
    try {
        ...
        let query = db
            .select()
            .from(trainers)
            .where(and(eq(trainers.isActive, true), eq(trainers.verificationStatus, 'approved'), eq(trainers.isAcceptingBookings, true)));
        ...
        if (specialty) {
            filteredTrainers = allTrainers.filter(t => t.specialties?.includes(specialty));
        }
        // Filter by service type if provided
        if (serviceType) {
            filteredTrainers = filteredTrainers.filter(t => t.serviceTypes?.includes(serviceType));
        }
        // Filter by minimum rating if provided
        if (minRating) {
            const minRatingNum = parseFloat(minRating);
            filteredTrainers = filteredTrainers.filter(t => parseFloat(t.averageRating || '0') >= minRatingNum);
        }
...
router.get('/trainers/:id', async (req, res) => {
    try {
        ...
        const reviews = await db
            ...
            .where(and(eq(contractorReviews.subjectId, trainerData.userId), eq(contractorReviews.reviewType, 'owner_to_contractor'), eq(contractorReviews.isVisible, true)))
...
router.get('/bookings', async (req, res) => {
    try {
        ...
        const bookingsWithTrainers = await Promise.all(userBookings.map(async (booking) => {
            const [trainer] = await db
                .select()
                .from(trainers)
...
router.get('/admin/trainers', async (req, res) => {
    try {
        ...
        if (status) {
            query = query.where(eq(trainers.verificationStatus, status));
        }
...
router.get('/admin/stats', async (req, res) => {
    try {
        ...
        const bookingStats = await db
            ...
            count: sql `count(*)`,
            totalRevenue: sql `sum(${trainerBookings.totalAmount})`,
        })
            .from(trainerBookings)
