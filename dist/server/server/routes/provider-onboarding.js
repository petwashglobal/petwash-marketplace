...
import { db } from '../db';
import { providerInviteCodes, providerApplications } from '@shared/schema';
...
router.post('/admin/invite-codes/generate', requireAdmin, async (req, res) => {
    try {
...
router.post('/apply', upload.fields([
    ...
    try {
        ...
        const { inviteCode, firstName, lastName, phoneNumber, city, country, providerType } = req.body;
        ...
        const existingApp = await db
            ...
            .where(and(eq(providerApplications.userId, authenticatedUser.uid), eq(providerApplications.status, 'pending')))
            .limit(1);
...
router.get('/admin/applications/pending', requireAdmin, async (req, res) => {
    try {
...
router.post('/admin/applications/approve', requireAdmin, async (req, res) => {
    try {
...
router.post('/admin/applications/reject', requireAdmin, async (req, res) => {
    try {
