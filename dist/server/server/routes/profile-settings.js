...
router.patch('/settings/profile', async (req, res) => {
    try {
        ...
        if (updates.birthdate !== undefined)
            updateData.dateOfBirth = updates.birthdate;
        if (updates.preferredLanguage !== undefined)
            updateData.language = updates.preferredLanguage;
        if (updates.address !== undefined || updates.city !== undefined) {
            try {
                const firestore = admin.firestore();
                const firestoreUpdate = {};
                if (updates.address !== undefined)
                    firestoreUpdate.address = updates.address;
                if (updates.city !== undefined)
                    firestoreUpdate.city = updates.city;
