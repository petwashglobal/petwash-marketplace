...
import { staffApplications, staffExpenses, staffLogbook, franchiseOrders, insertStaffApplicationSchema, insertStaffExpenseSchema, insertStaffLogbookSchema, insertFranchiseOrderSchema, } from '../../shared/schema';
...
export function registerStaffOnboardingRoutes(app) {
    // =================== STAFF APPLICATIONS ===================
    /**
    ...
    app.post('/api/staff/applications/:id/documents', requireAuth, async (req, res) => {
        try {
            ...
            await staffOnboardingService.uploadDocument(applicationId, documentType, documentUrl, metadata);
            res.json({
                success: true,
    ...
    app.post('/api/staff/applications/:id/verify-biometrics', requireAuth, async (req, res) => {
        try {
            ...
            const result = await staffOnboardingService.verifyBiometrics(applicationId, idPhotoUrl, selfieUrl);
            res.json({
                success: true,
    ...
    app.get('/api/staff/applications', requireAdmin, async (req, res) => {
        try {
            ...
            if (type) {
                query = query.where(eq(staffApplications.applicationType, type));
            }
    ...
    app.post('/api/staff/expenses', requireAuth, async (req, res) => {
        try {
            ...
            if (data.receiptUrl) {
                ...
                fraudAnalysis = await receiptFraudDetection.analyzeReceipt(data.receiptUrl, data.employeeId, parseFloat(data.amount.toString()));
    ...
    app.get('/api/staff/expenses', requireAdmin, async (req, res) => {
        try {
            ...
            if (employeeId) {
                query = query.where(eq(staffExpenses.employeeId, employeeId));
            }
            if (status) {
                query = query.where(eq(staffExpenses.status, status));
            }
            if (verificationStatus) {
                query = query.where(eq(staffExpenses.receiptVerificationStatus, verificationStatus));
            }
    ...
    app.post('/api/staff/logbook', requireAuth, async (req, res) => {
        try {
            ...
            if (data.startLocation && data.endLocation) {
                ...
                const distance = calculateDistance(data.startLocation, data.endLocation);
    ...
    app.get('/api/staff/logbook', requireAdmin, async (req, res) => {
        try {
            ...
            if (employeeId) {
                query = query.where(eq(staffLogbook.employeeId, employeeId));
            }
            if (status) {
                query = query.where(eq(staffLogbook.status, status));
            }
    ...
    app.get('/api/franchise/orders', requireAdmin, async (req, res) => {
        try {
            ...
            if (franchiseId) {
                query = query.where(eq(franchiseOrders.franchiseId, parseInt(franchiseId)));
            }
            if (paymentStatus) {
                query = query.where(eq(franchiseOrders.paymentStatus, paymentStatus));
            }
            if (orderStatus) {
                query = query.where(eq(franchiseOrders.orderStatus, orderStatus));
            }
