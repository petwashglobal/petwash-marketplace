...
import { db } from "../db";
import { contractorProfiles, identityVerifications, livenessChecks, criminalBackgroundChecks, driverSafetyProfiles, contractorRatings, contractorIncidents, complianceDecisions, complianceAuditLogs, } from "@shared/schema";
import { eq, and, desc, gte } from "drizzle-orm";
import jwt from "jsonwebtoken";
import { evaluateAssignmentEligibility, } from "../../shared/globalCompliance";
...
async function loadContractorComplianceData(contractorId) {
    ...
    const recentRatings = await db.query.contractorRatings.findMany({
        where: and(eq(contractorRatings.contractorId, contractorId), gte(contractorRatings.createdAt, thirtyDaysAgo)),
    });
    // Get all unresolved incidents
    const unresolvedIncidents = await db.query.contractorIncidents.findMany({
        where: and(eq(contractorIncidents.contractorId, contractorId), eq(contractorIncidents.resolvedAt, null)),
    });
    ...
    const criminalCheck = latestCriminalCheck
        ? {
            status: latestCriminalCheck.status,
            blockingOffenses: latestCriminalCheck.blockingOffenses || [],
...
router.get("/contractors/list", requireAuth, async (req, res) => {
    try {
        // Get all contractors
        ...
        const contractorsWithCompliance = await Promise.all(allContractors.map(async (contractor) => {
            try {
...
router.post("/contractors/:id/evaluate-eligibility", authMiddleware, async (req, res) => {
    try {
...
router.get("/contractors/:id/compliance-status", authMiddleware, async (req, res) => {
    try {
        const contractorId = req.params.id;
        // Load all contractor compliance data
        const data = await loadContractorComplianceData(contractorId);
...
router.get("/contractors/:id/compliance-history", authMiddleware, async (req, res) => {
    try {
        const contractorId = req.params.id;
...
router.post("/contractors/:id/israeli-contractor-check", authMiddleware, async (req, res) => {
    try {
        const contractorId = req.params.id;
        ...
        const israeliProfile = {
            ...
            languageCodes: contractor.languageCodes || ["he", "en"],
            ...
            riskLevel: contractor.riskLevel || "LOW",
        ...
        const result = evaluateIsraeliContractorForService(israeliProfile, serviceType || "SELF_SERVICE_STATION_CLEANING");
        // Save decision
        const [decision] = await db
            .insert(complianceDecisions)
            .values({
