...
import { db } from '../db';
import { spareParts, stationSpareParts, franchiseOrderRequests, stockTransactions, supplierNotificationSettings, petWashStations, franchisees, insertSparePartSchema, insertFranchiseOrderRequestSchema, insertStockTransactionSchema, insertSupplierNotificationSettingSchema } from '@shared/schema-enterprise';
 ...
 */
router.patch('/spare-parts/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/inventory-dashboard', async (req, res) => {
    try {
        ...
        const pendingOrders = await db
            ...
            .where(or(eq(franchiseOrderRequests.status, 'pending'), eq(franchiseOrderRequests.status, 'approved')))
...
router.get('/orders', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (status) {
            conditions.push(eq(franchiseOrderRequests.status, status));
        }
        if (franchiseeId) {
            conditions.push(eq(franchiseOrderRequests.franchiseeId, parseInt(franchiseeId)));
        }
        if (priority) {
            conditions.push(eq(franchiseOrderRequests.priority, priority));
        }
...
router.post('/orders', async (req, res) => {
    try {
        ...
        const orderCount = await db.select({ count: sql `count(*)` }).from(franchiseOrderRequests);
...
router.post('/transactions', async (req, res) => {
    try {
        ...
        const txCount = await db.select({ count: sql `count(*)` }).from(stockTransactions);
...
router.get('/transactions', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (sparePartId) {
            conditions.push(eq(stockTransactions.sparePartId, parseInt(sparePartId)));
        }
        if (transactionType) {
            conditions.push(eq(stockTransactions.transactionType, transactionType));
        }
        if (startDate) {
            conditions.push(gte(stockTransactions.createdAt, new Date(startDate)));
        }
        if (endDate) {
            conditions.push(lte(stockTransactions.createdAt, new Date(endDate)));
        }
 ...
 */
router.patch('/notification-settings/:id', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.delete('/notification-settings/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await db
