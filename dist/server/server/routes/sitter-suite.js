...
import { db } from '../db';
import { sitterProfiles, petProfilesForSitting, sitterBookings, sitterReviews, insertSitterProfileSchema, insertPetProfileForSittingSchema, insertSitterReviewSchema, } from '@shared/schema';
...
router.patch('/sitters/:id', async (req, res) => {
    try {
        ...
        const { firstName, lastName, phone, city, bio, yearsOfExperience, pricePerDayCents, profilePictureUrl, isActive, } = req.body;
        ...
        if (pricePerDayCents !== undefined)
            updateData.pricePerDayCents = pricePerDayCents;
...
router.post('/bookings', requireLoyaltyMember, async (req, res) => {
    try {
        const { ownerId, sitterId, petId, startDate, endDate, specialInstructions, ownerPaymentToken, } = req.body;
        ...
        const bookingConfirmation = await sitterAdvancedBookingEngine.confirmBooking(bookingId, pricing, ownerId);
...
router.get('/bookings', async (req, res) => {
    try {
        const userId = req.query.userId;
        const role = req.query.role; // owner or sitter
        if (!userId || !role) {
...
router.post('/search/nearby', async (req, res) => {
    try {
        ...
        const isEligible = await proximitySearch.isEligibleToBook(req.session?.userId || 'anonymous', loyaltyTier);
        if (!isEligible) {
        ...
        const sitters = await proximitySearch.findSittersNearby({ latitude, longitude }, radiusKm || 25);
