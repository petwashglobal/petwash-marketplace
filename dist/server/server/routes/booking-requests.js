...
import { db } from '../db';
import { bookingRequests, sitterProfiles, walkerProfiles, trainers, createBookingRequestSchema, providerBookingResponseSchema } from '@shared/schema';
...
router.get('/', async (req, res) => {
    try {
        ...
        const role = req.query.role; // 'owner' or 'provider'
...
router.post('/:requestId/pay', async (req, res) => {
    try {
        ...
        try {
            const escrow = await EscrowService.createEscrowPayment(requestId, booking.ownerId, booking.providerId, booking.totalCents / 100, // Convert cents to ILS
            nayaxTransactionId, {
    ...
    try {
        ...
        try {
            ...
            await createEarningRecord({
                ...
                contractorType: booking.providerType,
...
router.post('/:requestId/photo-update', async (req, res) => {
    try {
        ...
        const photoUpdates = booking.photoUpdates || [];
        photoUpdates.push({
            url: photoUrl,
            caption: caption || '',
            timestamp: new Date().toISOString(),
