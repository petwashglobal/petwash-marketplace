...
import { getAllDepartments, getAllRoles, getAllPlatforms, getUserEffectiveRoles, assignRoleToUser, revokeRoleFromUser, isUserAdmin, initializeControlPanelRegistry, } from '../services/ControlPanelRegistry';
 ...
 */
router.get('/user/:userId/roles', requireAuth, async (req, res) => {
    try {
        await ensureInitialized();
 ...
 */
router.post('/user/:userId/roles', requireAuth, async (req, res) => {
    try {
        await ensureInitialized();
        const { userId } = req.params;
        const requestingUserId = req.userId;
        ...
        // Assign role
        const assigned = await assignRoleToUser(userId, validatedData.roleKey, validatedData.scopeType || null, validatedData.scopeId || null, requestingUserId);
 ...
 */
router.delete('/user/:userId/roles/:roleId', requireAuth, async (req, res) => {
    try {
        await ensureInitialized();
