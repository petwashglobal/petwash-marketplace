...
import { db } from '../db';
import { nayaxTelemetry, nayaxTransactions, auditLedger } from '@shared/schema';
...
router.get('/dashboard/stations', async (req, res) => {
    try {
        const stationsStatus = await Promise.all(K9000_STATIONS.map(async (station) => {
            ...
            const washesToday = await db
                ...
                .where(and(eq(nayaxTransactions.stationId, station.stationId), gte(nayaxTransactions.createdAt, today)));
            ...
            const alerts = [];
            if (supplyLevels.shampoo < 20)
                alerts.push('Low Shampoo');
            if (supplyLevels.conditioner < 20)
                alerts.push('Low Conditioner');
            if (supplyLevels.salt < 5)
                alerts.push('Low Salt - Water Softener');
            return {
                ...station,
...
router.get('/dashboard/salt-report', async (req, res) => {
    try {
        const saltReport = await Promise.all(K9000_STATIONS.map(async (station) => {
        ...
        res.json({
            ...
            stations: saltReport,
            alerts: saltReport
                .filter(s => s.supplies.shampoo.status === 'low' ||
...
router.get('/dashboard/stats', async (req, res) => {
    try {
        ...
        const stats = await Promise.all(K9000_STATIONS.map(async (station) => {
            // Total washes in period
            const washes = await db
                .select({ count: sql `count(*)::int` })
                .from(nayaxTransactions)
                .where(and(eq(nayaxTransactions.stationId, station.stationId), gte(nayaxTransactions.createdAt, startTime), eq(nayaxTransactions.status, 'settled')));
            // Revenue in period
            const revenue = await db
                .select({
                total: sql `COALESCE(SUM(${nayaxTransactions.amount}), 0)::float`
            })
                .from(nayaxTransactions)
                .where(and(eq(nayaxTransactions.stationId, station.stationId), gte(nayaxTransactions.createdAt, startTime), eq(nayaxTransactions.status, 'settled')));
...
router.get('/dashboard/station/:stationId', async (req, res) => {
    try {
        ...
        const lifetimeWashes = await db
            .select({ count: sql `count(*)::int` })
            .from(nayaxTransactions)
            .where(and(eq(nayaxTransactions.stationId, stationId), eq(nayaxTransactions.status, 'settled')));
