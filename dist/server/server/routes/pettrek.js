...
router.post('/fare-estimate', requireAuth, async (req, res) => {
    try {
        ...
        const estimate = fareEstimationService.getFareEstimate({ latitude: data.pickupLatitude, longitude: data.pickupLongitude }, { latitude: data.dropoffLatitude, longitude: data.dropoffLongitude }, data.scheduledPickupTime);
        res.json(estimate);
    }
    catch (error) {
...
router.post('/request-trip', requireAuth, requireLoyaltyMember, async (req, res) => {
    try {
        ...
        const distanceKm = calculateDistance(data.pickupLatitude, data.pickupLongitude, data.dropoffLatitude, data.dropoffLongitude);
        ...
        const bookingConfirmation = await petTrekChauffeurBookingEngine.confirmBooking(tripId, pricing, customerId);
        ...
        const dispatchResult = await dispatchService.dispatchTrip(trip.id, data.pickupLatitude, data.pickupLongitude, data.petSize);
...
router.get('/trips/active', requireAuth, async (req, res) => {
    try {
        ...
        const trips = await db
            .select()
            .from(pettrekTrips)
            .where(and(eq(pettrekTrips.customerId, customerId), sql `${pettrekTrips.status} IN ('requested', 'dispatched', 'accepted', 'in_progress')`))
...
router.get('/trips/:tripId/details', requireAuth, async (req, res) => {
    try {
        ...
        const trip = await db
            .select()
            .from(pettrekTrips)
            .where(and(eq(pettrekTrips.id, tripId), eq(pettrekTrips.customerId, customerId)))
            .limit(1);
        if (trip.length === 0) {
...
router.get('/provider/jobs', requireAuth, async (req, res) => {
    try {
        ...
        const pendingJobs = await db
            ...
            .where(and(eq(pettrekDispatchRecords.providerId, provider[0].id), eq(pettrekDispatchRecords.responseStatus, 'pending'), eq(pettrekDispatchRecords.isExpired, false)))
...
router.post('/provider/accept-trip', requireAuth, async (req, res) => {
    try {
        ...
        const success = await dispatchService.acceptTrip(data.dispatchRecordId, provider[0].id);
        if (success) {
...
router.post('/provider/decline-trip', requireAuth, async (req, res) => {
    try {
        ...
        const data = schema.parse(req.body);
        const success = await dispatchService.declineTrip(data.dispatchRecordId, data.reason, data.notes);
        if (success) {
...
router.get('/provider/active-trips', requireAuth, async (req, res) => {
    try {
        ...
        const activeTrips = await db
            .select()
            .from(pettrekTrips)
            .where(and(eq(pettrekTrips.providerId, provider[0].id), sql `${pettrekTrips.status} IN ('accepted', 'in_progress')`))
...
router.get('/provider/trip-history', requireAuth, async (req, res) => {
    try {
        ...
        const tripHistory = await db
            .select()
            .from(pettrekTrips)
            .where(and(eq(pettrekTrips.providerId, provider[0].id), sql `${pettrekTrips.status} IN ('completed', 'canceled')`))
            .orderBy(sql `${pettrekTrips.createdAt} DESC`)
            .limit(50);
