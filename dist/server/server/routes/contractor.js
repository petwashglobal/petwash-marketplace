...
import { db } from '../db';
import { contractorViolations, contractorBadges, contractorEarnings, contractorReviews } from '@shared/schema';
 ...
 */
router.get('/:contractorId/trust-score', requireAuth, async (req, res) => {
    try {
 ...
 */
router.post('/:contractorId/update-trust-score', requireAdmin, async (req, res) => {
    try {
        const { contractorId } = req.params;
 ...
 */
router.get('/:contractorId/earnings', requireAuth, async (req, res) => {
    try {
        const { contractorId } = req.params;
        ...
        // Calculate totals
        const totalEarned = earnings.reduce((sum, e) => sum + parseFloat(e.baseAmount), 0);
        const totalBonus = earnings.reduce((sum, e) => sum + parseFloat(e.bonusAmount || '0'), 0);
        const totalFees = earnings.reduce((sum, e) => sum + parseFloat(e.platformFee), 0);
        const totalNet = earnings.reduce((sum, e) => sum + parseFloat(e.netEarnings), 0);
        ...
        const escrowAmount = inEscrow.reduce((sum, e) => sum + parseFloat(e.netEarnings), 0);
        const pendingAmount = pendingPayout.reduce((sum, e) => sum + parseFloat(e.netEarnings), 0);
        const paidAmount = paidOut.reduce((sum, e) => sum + parseFloat(e.netEarnings), 0);
        res.json({
            contractorId,
            summary: {
 ...
 */
router.get('/:contractorId/tax-summary', requireAuth, async (req, res) => {
    try {
        const { contractorId } = req.params;
        ...
        const earnings = await db
            .select()
            .from(contractorEarnings)
            .where(and(eq(contractorEarnings.contractorId, contractorId), eq(contractorEarnings.taxYear, taxYear), eq(contractorEarnings.includeInTaxReport, true)))
 ...
 */
router.get('/:contractorId/violations', requireAuth, async (req, res) => {
    try {
        const { contractorId } = req.params;
 ...
 */
router.get('/:contractorId/badges', async (req, res) => {
    try {
        const { contractorId } = req.params;
        const badges = await db
            .select()
            .from(contractorBadges)
            .where(and(eq(contractorBadges.contractorId, contractorId), eq(contractorBadges.isVisible, true), sql `${contractorBadges.revokedAt} IS NULL` // Not revoked
        ))
 ...
 */
router.get('/:contractorId/dashboard', requireAuth, async (req, res) => {
    try {
        const { contractorId } = req.params;
        // Check authorization
        if (req.user.uid !== contractorId && req.user.role !== 'admin') {
        ...
        const [trustScores, earningsSummary, recentEarnings, violations, badges, reviews,] = await Promise.all([
            ...
            db
                .select()
                .from(contractorBadges)
                .where(and(eq(contractorBadges.contractorId, contractorId), eq(contractorBadges.isVisible, true), sql `${contractorBadges.revokedAt} IS NULL`))
                .limit(5),
            db
                .select()
        ...
        const totalNet = earningsSummary.reduce((sum, e) => sum + parseFloat(e.netEarnings), 0);
        ...
        res.json({
            ...
            reviews: {
                ...
                averageRating: reviews.reduce((sum, r) => sum + r.overallRating, 0) / reviews.length || 0,
                recent: reviews,
            },
        });
