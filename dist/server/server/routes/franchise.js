...
import { z } from 'zod';
import { FIRESTORE_PATHS, insertServiceTicketSchema, } from '@shared/firestore-schema';
...
router.get('/dashboard/stats', requireFranchiseAuth, async (req, res) => {
    try {
        ...
        const franchiseStations = await db
            .select({ totalWashes: sql `COALESCE(SUM(${stations.totalWashes}), 0)` })
            .from(stations)
            .where(eq(stations.franchiseId, franchiseId));
        // Get revenue for today
        const todayRevenue = await db
            .select({ total: sql `COALESCE(SUM(CAST(${payments.amount} AS NUMERIC)), 0)` })
            .from(payments)
            .innerJoin(bookings, eq(payments.bookingId, bookings.id))
            .innerJoin(stations, eq(bookings.stationId, stations.id))
            .where(and(eq(stations.franchiseId, franchiseId), eq(payments.status, 'succeeded'), gte(payments.createdAt, todayStart)));
        ...
        const thisMonthRevenue = await db
            ...
            .innerJoin(bookings, eq(payments.bookingId, bookings.id))
            .innerJoin(stations, eq(bookings.stationId, stations.id))
            .where(and(eq(stations.franchiseId, franchiseId), eq(payments.status, 'succeeded'), gte(payments.createdAt, monthStart)));
        ...
        const lastMonthRevenue = await db
            ...
            .innerJoin(bookings, eq(payments.bookingId, bookings.id))
            .innerJoin(stations, eq(bookings.stationId, stations.id))
            .where(and(eq(stations.franchiseId, franchiseId), eq(payments.status, 'succeeded'), gte(payments.createdAt, lastMonthStart), lte(payments.createdAt, lastMonthEnd)));
        const stats = {
            ...
            machineStatus: profile?.machineIds?.map((id) => ({
                machineId: id,
                status: 'online',
...
router.get('/reports/financial', requireFranchiseAuth, async (req, res) => {
    try {
        const franchiseId = req.query.franchiseId;
        const period = req.query.period; // 'daily' or 'monthly'
        const date = req.query.date; // YYYY-MM-DD or YYYY-MM
        if (!franchiseId) {
            return res.status(400).json({ error: 'franchiseId is required' });
        }
        ...
        const transactionRecords = await db
            ...
            .innerJoin(bookings, eq(payments.bookingId, bookings.id))
            .innerJoin(stations, eq(bookings.stationId, stations.id))
            .where(and(eq(stations.franchiseId, franchiseId), eq(payments.status, 'succeeded'), gte(payments.createdAt, startDate), lte(payments.createdAt, endDate)))
            .orderBy(desc(payments.createdAt));
        // Calculate totals (VAT rate 18% in Israel - updated Jan 2025)
        const VAT_RATE = parseFloat(process.env.VAT_RATE || '0.18');
        const totalRevenue = transactionRecords.reduce((sum, tx) => sum + parseFloat(String(tx.amount)), 0);
...
router.get('/reports/export/excel', requireFranchiseAuth, async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=franchise_report_${franchiseId}_${date}.xlsx`);
...
router.get('/reports/export/pdf', requireFranchiseAuth, async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=franchise_report_${franchiseId}_${date}.pdf`);
        doc.pipe(res);
        // Header
        ...
        transactionRecords.forEach((tx, index) => {
            const amount = parseFloat(String(tx.amount));
            doc.text(`${index + 1}. ${format(tx.createdAt, 'yyyy-MM-dd HH:mm')} | ${tx.bookingNumber} | ₪${amount.toFixed(2)} | ${tx.paymentMethod || 'N/A'}`);
        });
        doc.end();
    }
    catch (error) {
