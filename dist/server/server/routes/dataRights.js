...
import { users, customers, customerPets, washHistory, nayaxTransactions, eVouchers, eVoucherRedemptions, hrDocuments } from '@shared/schema';
...
// ========================================================================
async function collectAllUserData(userId, email) {
    const userData = {
        metadata: {
    ...
    try {
        ...
        try {
            ...
            const userVoucherIds = userData.vouchers?.map((v) => v.id) || [];
            userData.voucherRedemptions = redemptions.filter((r) => userVoucherIds.includes(r.voucherId));
        }
        catch (error) {
    ...
    try {
        ...
        await AuditLedgerService.recordEvent({
            ...
            newState: {
                ...
                recordCount: Object.values(userData).filter(v => Array.isArray(v)).reduce((sum, arr) => sum + arr.length, 0),
                exportDate: new Date().toISOString(),
            },
            metadata: {
                auditHash,
...
// ========================================================================
router.patch('/correct', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
        const email = req.user.email;
