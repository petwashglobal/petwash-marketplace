 ...
 */
router.get('/wallet/spending/by-platform', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
 ...
 */
router.get('/analytics/my-activity', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
 ...
 */
router.get('/analytics/revenue/by-platform', requireAuth, async (req, res) => {
    try {
 ...
 * Get user events
 */
router.get('/events/my-events', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
        const limit = parseInt(req.query.limit) || 50;
 ...
 */
router.get('/cdp/profile', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
 ...
 */
router.get('/cdp/journey', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
        const limit = parseInt(req.query.limit) || 100;
 ...
 * Track user activity
 */
router.post('/cdp/track', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
 ...
 */
router.post('/cdp/refresh', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
 ...
 */
router.get('/marketing/campaigns', requireAuth, async (req, res) => {
    try {
        // ✅ SECURITY: Admin-only endpoint
        const { checkUserIsAdmin } = await import('../lib/adminCheck');
        if (!await checkUserIsAdmin(req.user.uid)) {
 ...
 */
router.post('/marketing/campaigns', requireAuth, async (req, res) => {
    try {
        // ✅ SECURITY: Admin-only endpoint
        const { checkUserIsAdmin } = await import('../lib/adminCheck');
        if (!await checkUserIsAdmin(req.user.uid)) {
 ...
 * Launch campaign
 */
router.post('/marketing/campaigns/:id/launch', requireAuth, async (req, res) => {
    try {
        // ✅ SECURITY: Admin-only endpoint
        const { checkUserIsAdmin } = await import('../lib/adminCheck');
        if (!await checkUserIsAdmin(req.user.uid)) {
 ...
 */
router.get('/marketing/campaigns/:id/performance', requireAuth, async (req, res) => {
    try {
        // ✅ SECURITY: Admin-only endpoint
        const { checkUserIsAdmin } = await import('../lib/adminCheck');
        if (!await checkUserIsAdmin(req.user.uid)) {
 ...
 */
router.post('/wallet/deduct-funds', requireAuth, async (req, res) => {
    try {
        const userId = req.user.uid;
