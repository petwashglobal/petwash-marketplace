...
import { db } from '../db';
import { watchdogIssues, watchdogUserStruggles, watchdogAutoFixes, watchdogCheckoutMonitoring, watchdogRegistrationMonitoring, watchdogUserJourneys } from '../../shared/schema-gemini-watchdog';
...
router.get('/issues', async (req, res) => {
    try {
        ...
        const issues = await db.query.watchdogIssues.findMany({
            where: status === 'all' ? undefined : eq(watchdogIssues.status, status),
...
router.get('/checkout', async (req, res) => {
    try {
        ...
        let where = undefined;
        if (userId) {
            where = eq(watchdogCheckoutMonitoring.userId, userId);
        }
        else if (step) {
            where = eq(watchdogCheckoutMonitoring.step, step);
        }
...
router.get('/registration', async (req, res) => {
    try {
        ...
        const registrations = await db.query.watchdogRegistrationMonitoring.findMany({
            where: step ? eq(watchdogRegistrationMonitoring.step, step) : undefined,
...
router.get('/journeys', async (req, res) => {
    try {
        ...
        let where = undefined;
        if (userId) {
            where = eq(watchdogUserJourneys.userId, userId);
        }
        else if (journey) {
            where = eq(watchdogUserJourneys.journey, journey);
        }
