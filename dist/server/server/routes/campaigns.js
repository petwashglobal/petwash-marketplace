...
import { db } from '../db';
import { crmEmailTemplates, crmSmsTemplates, crmCommunicationLogs, customers, users } from '@shared/schema';
import { eq, and, sql, inArray } from 'drizzle-orm';
import { EmailService } from '../emailService';
import { GoogleMessagingService } from '../services/GoogleMessagingService';
import { replaceTemplates, validateTemplate } from '../lib/template-engine';
    ...
    try {
        ...
        if (campaign.channel === 'email') {
            ...
            const [template] = await db
                ...
                .where(and(eq(crmEmailTemplates.id, campaign.templateId), eq(crmEmailTemplates.isActive, true)))
                .limit(1);
            if (!template) {
            ...
            const [template] = await db
                ...
                .where(and(eq(crmSmsTemplates.id, campaign.templateId), eq(crmSmsTemplates.isActive, true)))
                .limit(1);
            if (!template) {
                return res.status(404).json({
                    success: false,
        ...
        if (campaign.testMode) {
            ...
            if (campaign.segment === 'loyal_customers') {
                query = query.where(inArray(customers.loyaltyTier, ['silver', 'gold', 'platinum', 'diamond']));
            }
            else if (campaign.segment === 'new_customers') {
                query = query.where(eq(customers.loyaltyTier, 'new'));
            }
            else if (campaign.segment === 'club_members') {
                query = query.where(eq(customers.isClubMember, true));
            }
            ...
            try {
                ...
                if (emailTemplate && recipient.email) {
                    try {
                        ...
                        // Send email
                        const sent = await EmailService.sendEmail(recipient.email, personalizedSubject, personalizedContent);
                        if (sent) {
                            results.emailsSent++;
                            // Log communication
                ...
                if (smsTemplate && recipient.phone) {
                    try {
                        ...
                        const smsResult = await GoogleMessagingService.sendWhatsAppMessage(recipient.phone, personalizedSms);
