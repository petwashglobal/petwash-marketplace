...
const requirePlatformContext = (req, res, next) => {
...
router.post('/:platformId/bookings', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
...
router.get('/:platformId/bookings', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.get('/:platformId/bookings/:bookingId', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.patch('/:platformId/bookings/:bookingId/status', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
        ...
        if (existingBooking.providerId) {
            ...
            const [provider] = await db
                .select()
                .from(providers)
                .where(and(eq(providers.id, existingBooking.providerId), eq(providers.userId, userId), eq(providers.platformId, platformId)));
            isProvider = !!provider;
        }
        ...
        const updatedBooking = await bookingService.updateBookingStatus(bookingId, status, userId, reason);
        // Audit log
...
router.get('/:platformId/provider/bookings', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
        ...
        const providerRecords = await db
            .select()
            .from(providers)
            .where(and(eq(providers.userId, userId), eq(providers.platformId, platformId)));
...
router.get('/:platformId/stations', requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const platformId = req.platformContext.platformId;
...
router.get('/:platformId/providers', requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const platformId = req.platformContext.platformId;
...
router.post('/:platformId/bookings/:bookingId/confirm', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.post('/:platformId/bookings/:bookingId/cancel', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
        ...
        const cancelledBooking = await bookingService.cancelBooking(bookingId, userId, reason);
        // Audit log
...
router.post('/:platformId/bookings/:bookingId/payment-intents', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
        ...
        const existingIntents = await db
            .select()
            .from(paymentIntents)
            .where(and(eq(paymentIntents.bookingId, bookingId), eq(paymentIntents.status, 'pending')));
...
router.get('/:platformId/bookings/:bookingId/payment-intents', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.post('/:platformId/bookings/:bookingId/complete', requireAuth, requirePlatformContext, apiLimiter, async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
