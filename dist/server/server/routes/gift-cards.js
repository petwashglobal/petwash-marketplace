 ...
 */
function isWalletPassConfigured() {
 ...
 */
function generateWalletPassToken(voucherId, expiresInHours = 72) {
 ...
 * SECURITY: Returns null if secret not configured (fail closed)
 */
function verifyWalletPassToken(token) {
...
function generateBlockchainHash(voucherId, code, amount, timestamp) {
...
function generateVoucherCode() {
    ...
    for (let i = 0; i < 16; i++) {
        ...
        if ((i + 1) % 4 === 0 && i !== 15)
            code += '-'; // Add dashes every 4 chars
    }
    return code;
}
// 📧 SEND E-GIFT TO RECIPIENT
async function sendGiftCardToRecipient(voucher, recipientEmail, recipientPhone, recipientName, senderName, message, qrCodeDataURL, deliveryMethod) {
...
async function sendPurchaseConfirmationToBuyer(senderEmail, senderName, recipientName, amount, voucherId, transactionHash) {
    ...
    try {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            const updateResult = await tx
                ...
                .where(and(eq(eVouchers.id, voucherId), eq(eVouchers.codeHash, codeHash), eq(eVouchers.status, 'ISSUED'), // ← CRITICAL: Atomic status check
      ...
      `;
            try {
                await EmailService.sendEmail(voucher.recipientEmail, '✅ PetWash™ Gift Card Redeemed', confirmationHtml);
            }
            catch (error) {
