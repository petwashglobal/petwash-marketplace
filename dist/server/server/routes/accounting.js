...
import { db } from "../db";
import { israeliExpenses, israeliVatDeclarations, israeliIncomeTaxDeclarations, israeliNationalInsuranceDeclarations, israeliMonthlyFinancialPackages, transactionRecords, employeeHierarchy, insertIsraeliExpenseSchema } from "@shared/schema";
import { eq, and, gte, lte, desc, sql } from "drizzle-orm";
import ExcelJS from "exceljs";
import { logger } from "../lib/logger";
import { IsraeliVATReclaimService } from "../services/IsraeliVATReclaimService";
import { recordExpenseSubmission } from "../utils/auditSignature";
const router = Router();
// =================== EMPLOYEE EXPENSE SUBMISSION (AUTHENTICATED ROUTE) ===================
// POST /api/accounting/expenses/employee-submit - Employee expense submission
router.post("/expenses/employee-submit", async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({ error: "Unauthorized - Please log in" });
        }
        const { category, subcategory, description, vendor, amountBeforeVat, vatAmount, totalAmount, paymentMethod, receiptNumber, invoiceNumber, notes, taxMonth, taxYear, } = req.body;
                ...
                if (supervisor) {
                    ...
                    if (supervisor.whatsappPhone) {
                        ...
                        await WhatsAppService.sendExpenseApprovalNotification({
                            ...
                            category,
                            description,
                            language: supervisor.preferredLanguage || 'he',
                        });
...
router.get("/expenses/my-expenses", async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({ error: "Unauthorized - Please log in" });
...
router.get("/expenses", async (req, res) => {
    try {
        ...
        const conditions = [];
        if (year)
            conditions.push(eq(israeliExpenses.taxYear, Number(year)));
        if (month)
            conditions.push(eq(israeliExpenses.taxMonth, Number(month)));
        if (category)
            conditions.push(eq(israeliExpenses.category, String(category)));
        if (status)
            conditions.push(eq(israeliExpenses.status, String(status)));
...
router.get("/vat/declarations", async (req, res) => {
    try {
        ...
        const conditions = [];
        if (year)
            conditions.push(eq(israeliVatDeclarations.taxYear, Number(year)));
        if (status)
            conditions.push(eq(israeliVatDeclarations.status, String(status)));
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
        const declarations = await query.orderBy(desc(israeliVatDeclarations.taxYear), desc(israeliVatDeclarations.taxMonth));
...
router.post("/vat/generate", async (req, res) => {
    try {
        const { year, month } = req.body;
        if (!year || !month) {
            return res.status(400).json({ error: "Year and month are required" });
...
router.get("/vat/report/:year/:month", async (req, res) => {
    try {
...
router.post("/income-tax/generate", async (req, res) => {
    try {
        const { year, month } = req.body;
        const user = req.user;
        if (!year || !month) {
        ...
        const transactions = await db.select().from(transactionRecords)
            .where(and(gte(transactionRecords.timestamp, periodStart), lte(transactionRecords.timestamp, periodEnd)));
        // Get all approved deductible expenses
        const expenses = await db.select().from(israeliExpenses)
            .where(and(eq(israeliExpenses.taxYear, year), eq(israeliExpenses.taxMonth, month), eq(israeliExpenses.status, 'approved'), eq(israeliExpenses.isDeductible, true)));
        // Calculate totals
        const grossIncome = transactions.reduce((sum, t) => sum + Number(t.totalAmount || 0), 0);
        const netIncome = transactions.reduce((sum, t) => sum + Number(t.subtotal || 0), 0);
        // Categorize expenses
        const salaryExpenses = expenses
            .filter((e) => e.category === 'salaries')
            .reduce((sum, e) => sum + (Number(e.totalAmount || 0) * (e.deductionPercentage || 100) / 100), 0);
...
router.post("/national-insurance/generate", async (req, res) => {
    try {
...
router.post("/monthly-package/generate", async (req, res) => {
    try {
...
router.get("/monthly-package/:packageId/export", async (req, res) => {
    try {
...
router.get("/dashboard", async (req, res) => {
    try {
        ...
        const [currentVatDecl] = await db.select().from(israeliVatDeclarations)
            .where(and(eq(israeliVatDeclarations.taxYear, currentYear), eq(israeliVatDeclarations.taxMonth, currentMonth)));
        ...
        const ytdTransactions = await db.select({
            totalRevenue: sql `SUM(${transactionRecords.totalAmount})`,
            totalVat: sql `SUM(${transactionRecords.vatAmount})`,
            count: sql `COUNT(*)`
...
router.post("/email/send-blank-form", async (req, res) => {
    try {
        const { EmailService } = await import('../emailService');
...
router.post("/email/send-sample-vat", async (req, res) => {
    try {
        const { EmailService } = await import('../emailService');
