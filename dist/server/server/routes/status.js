...
import { db } from '../db';
import { petWashStations, stationAlerts } from '@shared/schema-enterprise';
...
router.get('/uptime', async (_req, res) => {
    try {
        ...
        const stationStats = await db
            .select({
            total: sql `count(*)::int`,
            healthy: sql `count(case when ${petWashStations.healthStatus} = 'healthy' then 1 end)::int`,
            warning: sql `count(case when ${petWashStations.healthStatus} = 'warning' then 1 end)::int`,
            critical: sql `count(case when ${petWashStations.healthStatus} = 'critical' then 1 end)::int`,
            offline: sql `count(case when ${petWashStations.healthStatus} = 'offline' then 1 end)::int`,
            operational: sql `count(case when ${petWashStations.operationalStatus} = 'active' then 1 end)::int`,
        })
        ...
        const criticalAlerts = await db
            ...
            .from(stationAlerts)
            .where(and(sql `${stationAlerts.severity} = 'critical'`, sql `${stationAlerts.acknowledgedAt} IS NULL`, gte(stationAlerts.createdAt, twentyFourHoursAgo)));
    ...
    try {
        ...
        const enrichedStations = stations.map(station => {
            ...
            return {
                ...
                maintenanceStatus: getMaintenanceStatus(station.lastMaintenanceDate ? new Date(station.lastMaintenanceDate) : null, station.nextMaintenanceDate ? new Date(station.nextMaintenanceDate) : null)
            };
        });
        // Summary statistics
        const summary = {
            ...
            maintenanceDue: enrichedStations.filter(s => s.maintenanceStatus === 'due' || s.maintenanceStatus === 'overdue').length
        };
        res.status(200).json({
            ok: true,
            timestamp: new Date().toISOString(),
...
function getMaintenanceStatus(lastMaintenance, nextMaintenance) {
    if (!nextMaintenance)
        return 'unknown';
    const now = new Date();
    const daysUntilMaintenance = Math.floor((new Date(nextMaintenance).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
