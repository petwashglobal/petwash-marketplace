 ...
 */
router.post('/export/transactions', async (req, res) => {
    try {
        const { fromDate, includeAI } = req.body;
        const result = await BookingExportService.exportBookingsToSheets(fromDate ? new Date(fromDate) : undefined, includeAI !== false);
        if (result.success) {
 ...
 */
router.post('/export/compliance', async (req, res) => {
    try {
 ...
 */
router.post('/export/escrow', async (req, res) => {
    try {
