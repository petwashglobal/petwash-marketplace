...
import { insertConversationSchema, FIRESTORE_PATHS } from '@shared/firestore-schema';
 ...
 */
router.post('/conversations/:id/messages', upload.array('attachments', 10), async (req, res) => {
    try {
        const uid = req.firebaseUser.uid;
        const employeeProfile = req.employee;
 ...
 */
router.post('/conversations/:id/messages/:messageId/read', async (req, res) => {
    try {
        const uid = req.firebaseUser.uid;
        ...
        const alreadyRead = readBy.some((r) => r.uid === uid);
        if (!alreadyRead) {
...
router.post('/conversations/:id/unarchive', async (req, res) => {
    try {
        ...
        const archived = (conversationData.archived || []).filter((u) => u !== uid);
        await db.collection('conversations').doc(conversationId).update({ archived });
        res.json({ success: true });
    }
    catch (error) {
 ...
 */
router.post('/conversations/:id/pin-message', async (req, res) => {
    try {
        const uid = req.firebaseUser.uid;
        const { id: conversationId } = req.params;
        ...
        conversationData.participants.forEach((participantUid) => {
            notifyConversationUpdate(participantUid, conversationId, {
                type: 'message_pinned',
                messageId,
                pinnedBy: uid,
 ...
 */
router.post('/conversations/:id/unpin-message', async (req, res) => {
    try {
        const uid = req.firebaseUser.uid;
        const { id: conversationId } = req.params;
        const conversationDoc = await db.collection('conversations').doc(conversationId).get();
