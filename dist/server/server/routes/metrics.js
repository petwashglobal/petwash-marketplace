...
function getMetricValue(metrics, metricName, aggregation = 'sum') {
    ...
    if (aggregation === 'avg' && metric.type === 'histogram') {
        const sum = metric.values.find((v) => v.metricName === `${metricName}_sum`)?.value || 0;
        const count = metric.values.find((v) => v.metricName === `${metricName}_count`)?.value || 1;
        return sum / count;
    }
    return 0;
}
function getStationStatuses(metrics) {
    const stationMetric = metrics.find(m => m.name === 'station_status');
    if (!stationMetric || !stationMetric.values)
        return {};
    const statuses = {};
    const statusMap = {
        0: 'offline',
    ...
    stationMetric.values.forEach((v) => {
