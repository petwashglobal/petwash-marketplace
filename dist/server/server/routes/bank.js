...
import { db } from "../db";
import { bankAccounts, bankTransactions, bankImportBatches, bankReconciliations, bankReconciliationSummary, transactionRecords, taxInvoices } from "@shared/schema";
...
router.get("/transactions", async (req, res) => {
    try {
        ...
        const conditions = [];
        if (accountId)
            conditions.push(eq(bankTransactions.accountId, parseInt(accountId)));
        if (startDate)
        ...
        if (status)
            conditions.push(eq(bankTransactions.reconciliationStatus, status));
...
router.get("/transactions/summary", async (req, res) => {
    try {
        ...
        const summary = await db
            ...
            unmatchedCount: sql `COUNT(CASE WHEN ${bankTransactions.reconciliationStatus} = 'unmatched' THEN 1 END)`,
            matchedCount: sql `COUNT(CASE WHEN ${bankTransactions.reconciliationStatus} = 'matched' THEN 1 END)`,
        })
    ...
    try {
        ...
        for (const txn of transactions) {
            try {
                ...
                const existing = await db.select().from(bankTransactions).where(and(eq(bankTransactions.accountId, parseInt(accountId)), sql `${bankTransactions.transactionDate} = ${txn.transactionDate}`, eq(bankTransactions.description, txn.description), eq(bankTransactions.debitAmount, txn.debitAmount || "0"), eq(bankTransactions.creditAmount, txn.creditAmount || "0")));
...
router.get("/import/batches", async (req, res) => {
    try {
        ...
        if (accountId) {
            queryBuilder = queryBuilder.where(eq(bankImportBatches.accountId, parseInt(accountId)));
        }
...
router.post("/reconcile/auto", async (req, res) => {
    try {
        ...
        for (const txn of unmatchedTxns) {
            ...
            const amount = parseFloat(txn.creditAmount) || 0;
            if (amount > 0) {
                ...
                const potentialMatches = await db
                    ...
                    .where(and(sql `${transactionRecords.timestamp} >= ${startMatch.toISOString()}`, sql `${transactionRecords.timestamp} <= ${endMatch.toISOString()}`, eq(transactionRecords.totalAmount, amount.toString())))
                    .limit(1);
...
router.post("/reconcile/manual", async (req, res) => {
    try {
        ...
        const bankAmount = parseFloat(bankTxn.creditAmount) || parseFloat(bankTxn.debitAmount) || 0;
...
router.get("/reconciliation/summary", async (req, res) => {
    try {
        ...
        const summary = await db
            ...
            .where(and(eq(bankReconciliationSummary.accountId, parseInt(accountId)), eq(bankReconciliationSummary.year, parseInt(year)), eq(bankReconciliationSummary.month, parseInt(month))));
...
router.post("/reconciliation/summary", async (req, res) => {
    try {
        const user = req.user;
        ...
        const transactions = await db
            .select()
            .from(bankTransactions)
            .where(and(eq(bankTransactions.accountId, parseInt(accountId)), sql `${bankTransactions.transactionDate} >= ${periodStartStr}`, sql `${bankTransactions.transactionDate} <= ${periodEndStr}`));
        ...
        const openingBalance = sortedTxns.length > 0 ? parseFloat(sortedTxns[0].balance) || 0 : 0;
        const closingBalance = sortedTxns.length > 0
            ? parseFloat(sortedTxns[sortedTxns.length - 1].balance) || 0
            : 0;
...
router.get("/export/excel", async (req, res) => {
    try {
        ...
        // Get account info
        const [account] = await db.select().from(bankAccounts).where(eq(bankAccounts.id, parseInt(accountId)));
        // Get transactions
        ...
        transactions.forEach(txn => {
            worksheet.addRow({
                ...
                debit: parseFloat(txn.debitAmount) || '',
                credit: parseFloat(txn.creditAmount) || '',
