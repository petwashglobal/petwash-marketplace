...
router.post('/wash/start_cycle', async (req, res) => {
    try {
        const { machineId, // K9000 controller ID (e.g., "K9000-TWIN-UNIT-1-BAY-1")
        ...
        const stationInfo = req.k9000Station;
        ...
        try {
            ...
            await eventPublisher.publishEvent(DomainEventType.WASH_STARTED, {
                washId,
                stationId: stationInfo?.stationId || machineId,
 ...
 */
function getEstimatedDuration(washType) {
    const durations = {
        basic: 8, // 8 minutes
