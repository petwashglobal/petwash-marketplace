...
import { stationSchema, insertStationSchema, } from '@shared/firestore-schema';
...
router.get('/', requireAdmin, async (req, res) => {
    try {
        ...
        if (search && typeof search === 'string') {
            ...
            stations = stations.filter(s => s.serialNumber?.toLowerCase().includes(searchLower) ||
...
router.get('/:id/inventory', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/inventory/set', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/inventory/adjust', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/:id/inventory/:itemId', requireAdmin, async (req, res) => {
    try {
...
router.get('/alerts/pending', requireAdmin, async (req, res) => {
    try {
        const now = new Date();
        ...
        for (const stationDoc of stationsSnapshot.docs) {
            ...
            for (const [utilityType, utilityData] of Object.entries(utilities)) {
...
router.get('/health/stations', requireAdmin, async (req, res) => {
    try {
...
router.post('/test/low-stock', requireAdmin, async (req, res) => {
    try {
...
router.post('/test/utility-renewals', requireAdmin, async (req, res) => {
    try {
...
router.post('/test/google-sheets-sync', requireAdmin, async (req, res) => {
    try {
