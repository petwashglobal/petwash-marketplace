...
import { certificateVerification } from '../services/CertificateVerificationService';
 ...
 */
async function scheduleFileCleanup(bucket, userId, timestamp, randomId) {
    ...
    setTimeout(async () => {
        try {
            ...
            const filesToDelete = files.filter((file) => file.name.includes(`_${timestamp}_${randomId}`));
 ...
 */
router.post('/upload', validateFirebaseToken, verifyAppCheckToken, upload.fields([
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                const result = await certificateVerification.verifyDocument(uid, documentType, documentCountry, {
 ...
 */
router.get('/approved-countries', async (req, res) => {
    try {
 ...
 */
router.get('/history', validateFirebaseToken, async (req, res) => {
    try {
 ...
 */
router.get('/admin/pending', validateFirebaseToken, async (req, res) => {
    try {
        const { uid } = req.firebaseUser;
 ...
 */
router.post('/admin/:id/approve', validateFirebaseToken, async (req, res) => {
    try {
        const { uid } = req.firebaseUser;
        const verificationId = parseInt(req.params.id);
        ...
        await certificateVerification.approveVerification(verificationId, uid, notes);
 ...
 */
router.post('/admin/:id/reject', validateFirebaseToken, async (req, res) => {
    try {
        const { uid } = req.firebaseUser;
        const verificationId = parseInt(req.params.id);
        ...
        await certificateVerification.rejectVerification(verificationId, uid, reason);
 ...
 */
router.post('/consent', validateFirebaseToken, async (req, res) => {
    try {
        const { uid } = req.firebaseUser;
        const { verificationId, consentDocumentProcessing, consentBiometricProcessing, deviceFingerprint } = req.body;
 ...
 */
router.get('/consent/status', validateFirebaseToken, async (req, res) => {
    try {
        const { uid } = req.firebaseUser;
        const { verificationId } = req.query;
        const status = await consentService.getConsentStatus(uid, verificationId ? parseInt(verificationId) : undefined);
        res.json({
            success: true,
            consent: status
        });
 ...
 */
router.delete('/consent', validateFirebaseToken, async (req, res) => {
    try {
        const { uid } = req.firebaseUser;
        const { reason } = req.body;
        if (!reason) {
