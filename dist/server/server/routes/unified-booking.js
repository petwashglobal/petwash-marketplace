...
import { unifiedBookingEngine, SERVICE_CONFIGS } from '../services/unified-booking';
const router = Router();
/**
 * Helper: Load booking from database and convert to UnifiedBooking format
 */
async function loadBookingFromDB(bookingId) {
    ...
    if (!dbBooking)
        return null;
    const platformData = dbBooking.platformData || {};
    return {
        id: dbBooking.id,
        ...
        status: (dbBooking.status?.toUpperCase() || 'DRAFT'),
 ...
 */
router.post('/:bookingId/quote', requireAuth, async (req, res) => {
    try {
...
router.post('/:bookingId/cancel', requireAuth, async (req, res) => {
    try {
        ...
        const updatedBooking = await unifiedBookingEngine.cancel(booking, cancelledBy, role, reason || 'User requested cancellation');
        res.json({
            success: true,
            booking: updatedBooking,
            status: 'CANCELLED'
 ...
 */
router.post('/:bookingId/refund', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { bookingId } = req.params;
        ...
        const result = await unifiedBookingEngine.refund(booking, Number(refundAmount), processedBy, role, reason, isPartial);
        res.json({
            success: true,
            booking: result.booking,
 ...
 */
router.post('/admin/free-wash', requireAuth, requireAdmin, async (req, res) => {
    try {
 ...
 */
router.post('/flow', requireAuth, async (req, res) => {
    try {
        const { serviceId, resourceId, resourceType, startTime, endTime, price, paymentProvider, paymentReference, metadata } = req.body;
        const userId = req.user?.uid || req.body.userId;
