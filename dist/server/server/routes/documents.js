...
import { documentCategories, secureDocuments, documentAccessLog, insertSecureDocumentSchema } from '../../shared/schema-enterprise';
import { loadUserRole, checkPermission, checkAccessLevel } from '../middleware/rbac';
 ...
 */
async function logAccess(documentId, userId, userEmail, userName, accessType, accessGranted, denialReason, ipAddress, userAgent) {
    try {
 ...
 */
function canAccessDocument(doc, req) {
    ...
    if (doc.allowedUserIds) {
        const allowedUserIds = doc.allowedUserIds;
 ...
 */
router.post('/upload', upload.single('file'), checkPermission('upload_documents'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const { categoryId, documentType, title, titleHe, description, supplierId, franchiseeId, stationId, invoiceNumber, invoiceDate, invoiceAmount, invoiceCurrency, paymentStatus, tags, isConfidential, accessLevel, allowedRoles, allowedUserIds, } = documentData.data;
        ...
        await logAccess(newDocument.id, req.firebaseUser.uid, req.firebaseUser.email, null, // displayName not available in minimal user type
        'upload', true, undefined, req.ip, req.get('user-agent'));
        ...
        if (req.firebaseUser) {
            await logAccess(0, // No document ID yet
            req.firebaseUser.uid, req.firebaseUser.email, null, 'upload', false, error instanceof Error ? error.message : 'Upload failed', req.ip, req.get('user-agent')).catch(err => logger.error('Failed to log upload error:', err));
        }
 ...
 */
router.get('/', checkPermission('view_documents'), async (req, res) => {
    try {
        const { categoryId, documentType, status, supplierId, franchiseeId, search, limit = 50, offset = 0 } = req.query;
        let query = db
            .select({
        ...
        // Apply filters
        const conditions = [eq(secureDocuments.status, status || 'active')];
        if (categoryId) {
            conditions.push(eq(secureDocuments.categoryId, parseInt(categoryId)));
        }
        if (documentType) {
            conditions.push(eq(secureDocuments.documentType, documentType));
        }
        if (supplierId) {
            conditions.push(eq(secureDocuments.supplierId, supplierId));
        }
        if (franchiseeId) {
            conditions.push(eq(secureDocuments.franchiseeId, parseInt(franchiseeId)));
        }
        // For non-super-admins, only show documents they have access to
        if (!req.userRole?.isSuperAdmin) {
            if (req.userRole?.assignment.franchiseeId) {
                conditions.push(or(eq(secureDocuments.franchiseeId, req.userRole.assignment.franchiseeId), sql `${secureDocuments.franchiseeId} IS NULL`));
            }
        }
 ...
 */
router.get('/:id', checkPermission('view_documents'), async (req, res) => {
    try {
        ...
        // Log access attempt
        await logAccess(documentId, req.firebaseUser.uid, req.firebaseUser.email, null, // displayName not available in minimal user type
        'view', access.granted, access.reason, req.ip, req.get('user-agent'));
        if (!access.granted) {
 ...
 */
router.get('/:id/download', checkPermission('view_documents'), async (req, res) => {
    try {
        const documentId = parseInt(req.params.id);
        const [doc] = await db
            .select()
        ...
        // Log access attempt
        await logAccess(documentId, req.firebaseUser.uid, req.firebaseUser.email, null, // displayName not available in minimal user type
        'download', access.granted, access.reason, req.ip, req.get('user-agent'));
        if (!access.granted) {
            return res.status(403).json({
                error: 'Access denied',
                reason: access.reason
 ...
 */
router.delete('/:id', checkPermission('delete_documents'), async (req, res) => {
    try {
        const documentId = parseInt(req.params.id);
        const [doc] = await db
            .select()
        ...
        // Log access attempt
        await logAccess(documentId, req.firebaseUser.uid, req.firebaseUser.email, null, // displayName not available in minimal user type
        'delete', access.granted, access.reason, req.ip, req.get('user-agent'));
        if (!access.granted) {
            return res.status(403).json({
                error: 'Access denied',
                reason: access.reason
 ...
 */
router.get('/:id/access-log', checkAccessLevel(8), async (req, res) => {
    try {
        const documentId = parseInt(req.params.id);
        const logs = await db
            .select()
