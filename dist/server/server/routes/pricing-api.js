...
import { z } from "zod";
import { calculateQuote, createQuoteRequest, getAvailableAddons, getProviderRateCard, formatPriceSimple } from "../services/pricing-engine";
const router = Router();
...
router.get("/addons/:platform/:serviceType", async (req, res) => {
    try {
...
router.get("/provider-rates/:providerId/:platform/:serviceType", async (req, res) => {
    try {
...
router.post("/instant-estimate", async (req, res) => {
    try {
