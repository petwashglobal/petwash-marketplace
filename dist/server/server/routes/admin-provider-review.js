...
router.post('/apply', requireAuth, async (req, res) => {
    try {
        ...
        const application = await adminProviderReviewService.createApplication(req.userId, platform, priority || 'normal');
        res.json({
            success: true,
            application,
 ...
 */
router.get('/my-status/:platform', requireAuth, async (req, res) => {
    try {
...
router.get('/admin/queue', requireAdmin, async (req, res) => {
    try {
        ...
        const queue = await adminProviderReviewService.getQueue(status, platform, parseInt(limit) || 50);
        res.json({
            queue,
            count: queue.length,
 ...
 */
router.get('/admin/application/:id', requireAdmin, async (req, res) => {
    try {
 ...
 */
router.post('/admin/assign/:id', requireAdmin, async (req, res) => {
    try {
        const applicationId = parseInt(req.params.id);
        if (isNaN(applicationId)) {
            return res.status(400).json({ error: 'מזהה בקשה לא תקין' });
        }
        const result = await adminProviderReviewService.assignToReviewer(applicationId, req.userId);
        res.json(result);
    }
    catch (error) {
...
router.patch('/admin/checklist/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await adminProviderReviewService.updateChecklistItem(applicationId, itemKey, approved, req.userId);
        res.json({
            ...result,
...
router.post('/admin/approve/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await adminProviderReviewService.approveApplication(applicationId, req.userId, reviewNotes);
        res.json(result);
    }
    catch (error) {
...
router.post('/admin/reject/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await adminProviderReviewService.rejectApplication(applicationId, req.userId, validation.data.rejectionReason);
        res.json(result);
    }
    catch (error) {
 ...
 */
router.post('/admin/hold/:id', requireAdmin, async (req, res) => {
    try {
        const applicationId = parseInt(req.params.id);
        if (isNaN(applicationId)) {
            return res.status(400).json({ error: 'מזהה בקשה לא תקין' });
        ...
        const result = await adminProviderReviewService.putOnHold(applicationId, req.userId, validation.data.reason);
        res.json(result);
    }
    catch (error) {
 ...
 */
router.get('/admin/approved/:platform', requireAdmin, async (req, res) => {
    try {
        const { platform } = req.params;
