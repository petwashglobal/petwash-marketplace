...
import { buildBaseVoucher, verifyVoucherJws, voucherSha256 } from "@shared/petwashVoucher2025";
 ...
 */
function reconstructVoucherFromDb(dbVoucher) {
    return {
 ...
 */
async function verifyVoucherIntegrity(dbVoucher) {
    try {
...
router.post("/create", requireAuth, validateBody(createVoucherSchema), async (req, res) => {
    try {
        const { type, value_type, value, washes, currency, expires_at, theme, recipient_name, recipient_email } = req.body;
        ...
        const voucher = await buildBaseVoucher({
            ...
            theme: theme || "neo_black_platinum",
...
router.get("/my-vouchers", requireAuth, async (req, res) => {
    try {
        ...
        const vouchersWithHistory = await Promise.all(vouchers.map(async (v) => {
...
router.post("/redeem", requireAuth, validateBody(redeemVoucherSchema), async (req, res) => {
    try {
        const { public_code, station_id, location_label, method, amount, washes } = req.body;
        // Find voucher
        ...
        const stats = {
            ...
            active_vouchers: vouchers.filter(v => {
                if (v.expiresAt && new Date(v.expiresAt) < new Date())
                    return false;
                if (v.valueType === "currency" && Number(v.valueRemaining || 0) <= 0)
                    return false;
                if (v.valueType === "washes" && (v.washesRemaining || 0) <= 0)
                    return false;
                return true;
            }).length,
