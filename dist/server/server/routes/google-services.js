...
router.get('/places/:placeId', async (req, res) => {
    try {
        ...
        const language = req.query.language || 'iw';
...
router.get('/places/photo', async (req, res) => {
    try {
        ...
        const { getGoogleMapsPlaces } = await import('../services/googleMapsPlaces');
        const placesService = getGoogleMapsPlaces();
        const photoUrl = placesService.getPhotoUrl(reference, maxWidth ? parseInt(maxWidth) : 400);
...
router.get('/reviews/summary', async (req, res) => {
    try {
        ...
        const summary = await placesService.generateReviewSummary(placeDetails.reviews, language || 'he');
