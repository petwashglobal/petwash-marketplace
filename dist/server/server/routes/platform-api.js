...
import { enhancedBookingService, BOOKING_STATUS } from "../services/EnhancedBookingService";
import { geminiMatchingService } from "../services/GeminiMatchingService";
import { requirePlatform, requireFeature, requireBookingAuth } from "../middleware/platformContext";
...
function getUserRoles(req) {
    return (req.user?.roles || req.session?.roles || []);
}
...
router.post("/platforms/seed", async (req, res) => {
    try {
...
router.post("/bookings", requirePlatform, requireFeature("bookings"), async (req, res) => {
    try {
...
router.get("/bookings/:id", requirePlatform, requireBookingAuth, async (req, res) => {
    try {
        const platform = req.platformCtx.platform;
...
router.get("/bookings/:id/quote", requirePlatform, requireBookingAuth, async (req, res) => {
    try {
        const platform = req.platformCtx.platform;
        const booking = await enhancedBookingService.getBookingWithDetails(req.params.id, platform.id);
        const userId = getUserId(req);
...
router.post("/bookings/:id/status", requirePlatform, requireBookingAuth, async (req, res) => {
    try {
        ...
        const result = await enhancedBookingService.transitionStatus(req.params.id, parsed.data.status, actorRole, userId || undefined, { reason: parsed.data.reason }, req.platformCtx.platform.id);
        if (!result.success) {
            return res.status(400).json({
...
router.post("/bookings/:id/pay", requirePlatform, requireBookingAuth, requireFeature("payments"), async (req, res) => {
    try {
        ...
        const result = await enhancedBookingService.processPayment(req.params.id, parsed.data.paymentMethod, parsed.data.gateway, req.platformCtx.platform.id);
        if (!result.success) {
            return res.status(400).json({
...
router.get("/bookings/user/:userId", requirePlatform, requireBookingAuth, async (req, res) => {
    try {
...
router.get("/bookings/provider/:providerId", requirePlatform, requireBookingAuth, async (req, res) => {
    try {
...
router.get("/features", requirePlatform, (req, res) => {
    const platform = req.platformCtx.platform;
    res.json({
        platformId: platform.id,
...
router.post("/match", async (req, res) => {
    try {
        ...
        const result = await geminiMatchingService.findMatchingProviders({ latitude, longitude, city, country }, serviceRequest, radiusKm);
        if (!result.success) {
...
router.post("/match/search", async (req, res) => {
    try {
        ...
        const result = await geminiMatchingService.searchWithAI(query, { latitude, longitude }, preferredLanguage || 'en');
        if (!result.success) {
            return res.status(500).json({
...
router.get("/match/nearest/:serviceType", async (req, res) => {
    try {
        ...
        const nearestProvider = await geminiMatchingService.findNearestProvider({ latitude: lat, longitude: lng }, serviceType);
        if (!nearestProvider) {
            return res.status(404).json({
