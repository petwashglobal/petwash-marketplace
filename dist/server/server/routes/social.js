...
import { db } from '../db';
import { socialPosts, socialComments, socialLikes, socialFriendships, socialDirectMessages, insertSocialPostSchema, insertSocialCommentSchema, insertSocialDirectMessageSchema, insertSocialFriendshipSchema, } from '../../shared/schema';
...
router.get('/feed', async (req, res) => {
    try {
        ...
        const friendships = await db
            ...
            .where(and(or(eq(socialFriendships.requesterId, userId), eq(socialFriendships.addresseeId, userId)), eq(socialFriendships.status, 'accepted')));
        ...
        const posts = await db
            .select()
            .from(socialPosts)
            .where(and(inArray(socialPosts.userId, friendIds), eq(socialPosts.isDeleted, false), eq(socialPosts.moderationStatus, 'approved')))
...
router.post('/posts', async (req, res) => {
    try {
        ...
        if (validated.caption) {
            const moderation = await contentModerationService.moderateContent(validated.caption, 'post', userId, 0);
...
router.post('/posts/:postId/like', async (req, res) => {
    try {
        ...
        const existingLike = await db
            .select()
            .from(socialLikes)
            .where(and(eq(socialLikes.userId, userId), eq(socialLikes.targetType, 'post'), eq(socialLikes.targetId, postId)));
...
router.get('/posts/:postId/comments', async (req, res) => {
    try {
        ...
        const comments = await db
            .select()
            .from(socialComments)
            .where(and(eq(socialComments.postId, postId), eq(socialComments.isDeleted, false), eq(socialComments.moderationStatus, 'approved')))
...
router.post('/posts/:postId/comments', async (req, res) => {
    try {
        ...
        // CRITICAL: AI MODERATION - Stage 1 & 2
        const moderation = await contentModerationService.moderateContent(validated.text, 'comment', userId, 0);
        if (!moderation.isApproved) {
...
router.get('/friend-requests', async (req, res) => {
    try {
        ...
        const requests = await db
            .select()
            .from(socialFriendships)
            .where(and(eq(socialFriendships.addresseeId, userId), eq(socialFriendships.status, 'pending')));
...
router.post('/friend-requests', async (req, res) => {
    try {
        ...
        const existing = await db
            .select()
            .from(socialFriendships)
            .where(or(and(eq(socialFriendships.requesterId, userId), eq(socialFriendships.addresseeId, validated.addresseeId)), and(eq(socialFriendships.requesterId, validated.addresseeId), eq(socialFriendships.addresseeId, userId))));
...
router.post('/friend-requests/:requestId/accept', async (req, res) => {
    try {
        ...
        const [updated] = await db
            ...
            .where(and(eq(socialFriendships.id, requestId), eq(socialFriendships.addresseeId, userId), eq(socialFriendships.status, 'pending')))
            .returning();
        if (!updated) {
...
router.get('/conversations', async (req, res) => {
    try {
        ...
        const messages = await db
            ...
            .where(and(or(eq(socialDirectMessages.senderId, userId), eq(socialDirectMessages.recipientId, userId)), eq(socialDirectMessages.isDeleted, false)))
...
router.get('/conversations/:conversationId/messages', async (req, res) => {
    try {
        ...
        const messages = await db
            .select()
            .from(socialDirectMessages)
            .where(and(eq(socialDirectMessages.conversationId, conversationId), eq(socialDirectMessages.isDeleted, false), eq(socialDirectMessages.moderationStatus, 'approved')))
            .orderBy(socialDirectMessages.createdAt);
        // Mark as read
        await db
            .update(socialDirectMessages)
            .set({ isRead: true, readAt: new Date() })
            .where(and(eq(socialDirectMessages.conversationId, conversationId), eq(socialDirectMessages.recipientId, userId), eq(socialDirectMessages.isRead, false)));
...
router.post('/messages', async (req, res) => {
    try {
        ...
        if (validated.text) {
            const moderation = await contentModerationService.moderateContent(validated.text, 'message', userId, 0);
            if (!moderation.isApproved) {
