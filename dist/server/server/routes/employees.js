...
import { insertEmployeeProfileSchema } from '@shared/firestore-schema';
import { loadEmployeeProfile, requireRole } from '../middleware/roleAuth';
 ...
 */
router.get('/', requireRole(['admin', 'ops']), async (req, res) => {
    try {
        const employees = [];
        ...
        employees.sort((a, b) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0));
        res.json({ employees });
    }
    catch (error) {
 ...
 * Supports both new (employees/{uid}) and legacy (users/{uid}/employee/profile) paths
 */
router.get('/:uid', requireRole(['admin', 'ops']), async (req, res) => {
    try {
 ...
 */
router.put('/:uid', requireRole(['admin']), async (req, res) => {
    try {
        const { uid } = req.params;
 ...
 */
router.post('/:uid/suspend', requireRole(['admin']), async (req, res) => {
    try {
        const { uid } = req.params;
        const updateData = {
            status: 'suspended',
 ...
 * Updates both locations for backward compatibility
 */
router.post('/:uid/activate', requireRole(['admin']), async (req, res) => {
    try {
        const { uid } = req.params;
        const updateData = {
            status: 'active',
 ...
 */
router.post('/:uid/send-invite', requireRole(['admin']), async (req, res) => {
    try {
        const { uid } = req.params;
 ...
 */
router.post('/:uid/generate-mobile-link', requireRole(['admin']), async (req, res) => {
    try {
        const { uid } = req.params;
 ...
 * Updates both locations for backward compatibility
 */
router.delete('/:uid', requireRole(['admin']), async (req, res) => {
    try {
        const { uid } = req.params;
        const updateData = {
            status: 'inactive',
