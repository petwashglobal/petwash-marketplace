...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const success = await UserDeviceService.updateDeviceLabel(deviceId, userId, deviceLabel.trim());
        if (!success) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const ipAddress = req.headers['x-forwarded-for']?.toString().split(',')[0] ||
            req.ip ||
            req.socket.remoteAddress;
        const success = await UserDeviceService.revokeDevice(deviceId, userId, 'user_dismissed', ipAddress);
        if (!success) {
 ...
 */
router.post('/admin/:id/revoke', requireAdmin, async (req, res) => {
    try {
        const deviceId = req.params.id;
        ...
        const success = await UserDeviceService.revokeDevice(deviceId, device.userId, reason, req.ip);
        if (!success) {
            return res.status(500).json({ error: 'Failed to revoke device' });
        }
        res.json({
