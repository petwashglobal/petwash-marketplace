...
router.post('/gps-update', requireAuth, async (req, res) => {
    try {
        ...
        const data = schema.parse(req.body);
        const walkerId = req.user.uid;
        await walkSessionService.updateGPSLocation(data.walkId, walkerId, data.location);
...
router.post('/vital-update', requireAuth, async (req, res) => {
    try {
        ...
        const data = schema.parse(req.body);
        const walkerId = req.user.uid;
        await walkSessionService.updateVitalData(data.walkId, walkerId, data.vitalData);
        res.json({ success: true });
    }
    catch (error) {
