...
import { z } from "zod";
import { insertHrEmployeeSchema, insertHrPayrollSchema, insertHrTimeTrackingSchema, insertHrPerformanceReviewSchema, insertHrRecruitmentSchema, insertHrJobApplicationSchema } from "@shared/schema";
...
router.get("/employees/:id/time-tracking", requireAdmin, async (req, res) => {
    try {
        ...
        let timeEntries;
        if (start && end) {
            timeEntries = await storage.getTimeTrackingByDateRange(employeeId, start, end);
        }
        else {
...
router.get("/applications", requireAdmin, async (req, res) => {
    try {
        ...
        let applications;
        if (jobId) {
            applications = await storage.getJobApplications(parseInt(jobId));
        }
        else {
