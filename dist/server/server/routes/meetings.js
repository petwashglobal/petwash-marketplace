...
import { z } from 'zod';
import { db } from '../db';
import { crmTasks, crmMeetingAttendees, adminUsers, customers, } from '@shared/schema';
 ...
 */
async function sendMeetingInvitations(meeting, attendees, locale = 'he') {
    ...
    for (const attendee of attendees) {
        try {
          ...
          `;
                await EmailService.sendEmail(email, emailSubject, emailBody, locale);
            ...
            if (phone) {
                ...
                try {
                    await GoogleMessagingService.sendWhatsAppMessage(phone, whatsappMessage);
 ...
 */
async function sendMeetingUpdateNotifications(meeting, attendees, changes, locale = 'he') {
    const correlationId = nanoid();
 ...
 */
async function sendMeetingCancellationNotifications(meeting, attendees, locale = 'he') {
    const correlationId = nanoid();
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        // Create attendees
        const attendeeInserts = validated.attendees.map((att) => ({
        ...
        if (validated.sendInvitations) {
            await sendMeetingInvitations({ ...meeting, location: validated.location }, attendees, validated.locale);
        }
        res.status(201).json({
            success: true,
            meeting,
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        ...
        const [existing] = await db
            .select()
            .from(crmTasks)
            .where(and(eq(crmTasks.id, meetingId), eq(crmTasks.taskType, 'meeting')))
            .limit(1);
        if (!existing) {
        ...
        if (validated.scheduledStart && new Date(validated.scheduledStart).getTime() !== existing.scheduledStart?.getTime())
            changes.push('start time');
        if (validated.scheduledEnd && new Date(validated.scheduledEnd).getTime() !== existing.scheduledEnd?.getTime())
            changes.push('end time');
        if (validated.location && validated.location !== existing.notes)
        ...
        if (validated.scheduledEnd)
            updateData.scheduledEnd = new Date(validated.scheduledEnd);
        if (validated.status)
            updateData.status = validated.status;
        if (validated.location)
            updateData.notes = `Location: ${validated.location}`;
        ...
        if (validated.notifyAttendees && changes.length > 0) {
            ...
            await sendMeetingUpdateNotifications({ ...updated, location: validated.location }, attendees, changes, validated.locale);
        }
        res.json({
            success: true,
            meeting: updated,
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        if (notifyAttendees === 'true') {
            await sendMeetingCancellationNotifications(meeting, attendees, locale);
        }
        res.json({
            success: true,
 ...
 */
router.patch('/:id/attendees/:attendeeId/response', async (req, res) => {
    try {
