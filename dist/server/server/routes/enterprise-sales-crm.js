...
import { insertCrmCommunicationSchema, insertCrmDealStageSchema, insertCrmTaskSchema, insertCrmActivitySchema, } from "@shared/schema";
...
router.get("/tasks/overdue", requireAdmin, async (req, res) => {
    try {
        ...
        const tasks = await storage.getOverdueTasks(assignedTo);
        res.json(tasks);
    }
    catch (error) {
...
router.get("/tasks/upcoming", requireAdmin, async (req, res) => {
    try {
        ...
        const tasks = await storage.getUpcomingTasks(assignedTo, days ? parseInt(days) : undefined);
        res.json(tasks);
    }
    catch (error) {
...
router.post("/tasks/:id/complete", requireAdmin, async (req, res) => {
    try {
        ...
        const completed = await storage.completeTask(parseInt(req.params.id), completedBy, outcome, notes);
        res.json(completed);
    }
    catch (error) {
