...
import { authorityDocuments, providerLicenses, bookingPolicies, disputeResolutions, reviewModeration, reviewModerationRules, boardResolutions, corporateSeals, complianceTasks, complianceAuditTrail, insertAuthorityDocumentSchema, insertProviderLicenseSchema, insertBookingPolicySchema, insertDisputeResolutionSchema, insertReviewModerationSchema, insertBoardResolutionSchema, insertCorporateSealSchema, } from "@shared/schema-compliance";
 ...
 */
router.get("/authority-documents", async (req, res) => {
    try {
        const { status, documentType, displayPublicly } = req.query;
        let query = db.select().from(authorityDocuments);
        if (status) {
            query = query.where(eq(authorityDocuments.status, status));
        }
        if (documentType) {
            query = query.where(eq(authorityDocuments.documentType, documentType));
        }
        if (displayPublicly === "true") {
            query = query.where(eq(authorityDocuments.displayPublicly, true));
        }
 ...
 */
router.post("/authority-documents", async (req, res) => {
    try {
 ...
 */
router.put("/authority-documents/:id", async (req, res) => {
    try {
 ...
 */
router.delete("/authority-documents/:id", async (req, res) => {
    try {
        const id = parseInt(req.params.id);
 ...
 */
router.get("/provider-licenses", async (req, res) => {
    try {
        const { providerId, providerType, status, isMandatory } = req.query;
        let query = db.select().from(providerLicenses);
        const conditions = [];
        if (providerId) {
            conditions.push(eq(providerLicenses.providerId, parseInt(providerId)));
        }
        if (providerType) {
            conditions.push(eq(providerLicenses.providerType, providerType));
        }
        if (status) {
            conditions.push(eq(providerLicenses.status, status));
        }
        if (isMandatory === "true") {
 ...
 */
router.put("/provider-licenses/:id/verify", async (req, res) => {
    try {
        const id = parseInt(req.params.id);
 ...
 */
router.get("/provider-compliance/:providerId/:providerType", async (req, res) => {
    try {
...
router.get("/tasks", async (req, res) => {
    try {
        ...
        const conditions = [];
        if (status) {
            conditions.push(eq(complianceTasks.status, status));
        }
        if (priority) {
            conditions.push(eq(complianceTasks.priority, priority));
        }
        if (assignedTo) {
            conditions.push(eq(complianceTasks.assignedTo, parseInt(assignedTo)));
        }
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
 ...
 */
router.get("/booking-policies", async (req, res) => {
    try {
        const { serviceType, isActive } = req.query;
        let query = db.select().from(bookingPolicies);
        const conditions = [];
        if (serviceType) {
            conditions.push(eq(bookingPolicies.serviceType, serviceType));
        }
        if (isActive === "true") {
 ...
 */
router.post("/booking-policies", async (req, res) => {
    try {
...
router.get("/disputes", async (req, res) => {
    try {
        ...
        const conditions = [];
        if (status) {
            conditions.push(eq(disputeResolutions.status, status));
        }
        if (isEscalated === "true") {
 ...
 */
router.get("/corporate-seals", async (req, res) => {
    try {
        const { isActive } = req.query;
        let query = db.select().from(corporateSeals);
        if (isActive === "true") {
            query = query.where(eq(corporateSeals.isActive, true));
        }
 ...
 */
router.post("/corporate-seals", async (req, res) => {
    try {
 ...
 */
router.get("/board-resolutions", async (req, res) => {
    try {
        const { approvalStatus, isPublic } = req.query;
        let query = db.select().from(boardResolutions);
        const conditions = [];
        if (approvalStatus) {
            conditions.push(eq(boardResolutions.approvalStatus, approvalStatus));
        }
        if (isPublic === "true") {
 ...
 */
router.post("/board-resolutions", async (req, res) => {
    try {
 ...
 */
router.get("/review-moderation-rules", async (req, res) => {
    try {
        const { ruleType, isActive } = req.query;
        let query = db.select().from(reviewModerationRules);
        const conditions = [];
        if (ruleType) {
            conditions.push(eq(reviewModerationRules.ruleType, ruleType));
        }
        if (isActive === "true") {
 ...
 */
router.get("/review-moderation", async (req, res) => {
    try {
        const { status, legalReviewRequired } = req.query;
        let query = db.select().from(reviewModeration);
        const conditions = [];
        if (status) {
            conditions.push(eq(reviewModeration.status, status));
        }
 ...
 */
router.post("/review-moderation", async (req, res) => {
    try {
 ...
 */
router.put("/review-moderation/:id/approve", async (req, res) => {
    try {
        const id = parseInt(req.params.id);
