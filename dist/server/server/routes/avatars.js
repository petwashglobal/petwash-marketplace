...
import { db } from '../db';
import { petAvatars } from '@shared/schema';
...
router.get('/:avatarId', validateFirebaseToken, async (req, res) => {
    try {
        ...
        const [avatar] = await db
            .select()
            .from(petAvatars)
            .where(and(eq(petAvatars.id, avatarId), eq(petAvatars.userId, uid)));
        if (!avatar) {
...
router.patch('/:avatarId', validateFirebaseToken, async (req, res) => {
    try {
        ...
        if (req.body.petName)
            updates.petName = req.body.petName.trim();
        if (req.body.ttsVoice)
            updates.ttsVoice = req.body.ttsVoice;
        if (req.body.animationProfile)
            updates.animationProfile = req.body.animationProfile;
        if (req.body.landmarkConfig)
            updates.landmarkConfig = req.body.landmarkConfig;
        if (req.body.status)
            updates.status = req.body.status;
        // PREMIUM CUSTOMIZATION (NEW) - With Validation
        if (req.body.characterType)
            updates.characterType = req.body.characterType;
        // Outfit validation
        ...
        if (req.body.customization)
            updates.customization = req.body.customization;
        if (typeof req.body.isDefault === 'boolean') {
            ...
            if (req.body.isDefault) {
                await db
                    .update(petAvatars)
                    .set({ isDefault: false })
                    .where(and(eq(petAvatars.userId, uid), eq(petAvatars.isDefault, true)));
            }
        }
