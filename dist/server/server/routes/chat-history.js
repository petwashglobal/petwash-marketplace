...
import { z } from 'zod';
import { db } from '../db';
import { insertChatConversationSchema, insertChatMessageSchema, insertChatAttachmentSchema, insertChatAnalyticsSchema, } from '@shared/schema-chat';
import { chatConversations, chatMessages, chatAttachments, chatAnalytics } from '@shared/schema-chat';
import { eq, desc, and, lte, like } from 'drizzle-orm';
...
router.get('/conversations/franchise/:franchiseId', requireAuth, async (req, res) => {
    try {
...
router.get('/search', requireAuth, async (req, res) => {
    try {
        ...
        const messages = await db
            ...
            .from(chatMessages)
            .innerJoin(chatConversations, eq(chatMessages.conversationId, chatConversations.conversationId))
            .where(and(like(chatMessages.content, searchPattern), eq(chatConversations.userId, userId)))
            .orderBy(desc(chatMessages.createdAt))
            .limit(limit);
        res.json({
            success: true,
...
router.get('/analytics/:conversationId', requireAuth, async (req, res) => {
    try {
        const { conversationId } = req.params;
        // Verify access to conversation
        const [conversation] = await db
...
router.get('/messages/:messageId/attachments', requireAuth, async (req, res) => {
    try {
        const { messageId } = req.params;
