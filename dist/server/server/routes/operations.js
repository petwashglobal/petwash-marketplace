...
import { loadEmployeeContext, requireFranchiseScope, getFranchiseFilter, canAccessFranchise } from "../middleware/franchiseAuth";
import { db } from "../db";
...
router.get("/tasks", async (req, res) => {
    try {
        ...
        if (req.query.franchiseId && franchiseFilter === null) {
            ...
            filters.push(eq(opsTasksTable.franchiseId, parseInt(req.query.franchiseId)));
        }
        if (req.query.region) {
            filters.push(eq(opsTasksTable.region, req.query.region));
        }
        if (req.query.country) {
            filters.push(eq(opsTasksTable.country, req.query.country));
        }
        if (req.query.status) {
            filters.push(eq(opsTasksTable.status, req.query.status));
        }
        if (req.query.priority) {
            filters.push(eq(opsTasksTable.priority, req.query.priority));
        }
        if (req.query.escalationLevel) {
            filters.push(eq(opsTasksTable.escalationLevel, parseInt(req.query.escalationLevel)));
        }
        if (req.query.assignedTo) {
            filters.push(eq(opsTasksTable.assignedTo, parseInt(req.query.assignedTo)));
        }
        if (req.query.stationId) {
            filters.push(eq(opsTasksTable.stationId, req.query.stationId));
        }
        // Date range filters
        if (req.query.dueDateFrom) {
            filters.push(gte(opsTasksTable.dueDate, new Date(req.query.dueDateFrom)));
        }
        if (req.query.dueDateTo) {
            filters.push(lte(opsTasksTable.dueDate, new Date(req.query.dueDateTo)));
        }
        // Pagination
        ...
        if (!TASK_SORT_FIELDS.includes(sortBy)) {
            return res.status(400).json({
                error: "Invalid sortBy field",
                allowed: TASK_SORT_FIELDS
            });
        }
        const sortColumn = opsTasksTable[sortBy];
    ...
    try {
        ...
        const actionLogEntry = {
            ...
            details: {
                ...
                previous: Object.fromEntries(Object.keys(validated).map(key => [key, existingTask[key]])),
            },
        };
        const currentActionLog = existingTask.actionLog || [];
...
router.post("/tasks/:id/escalate", async (req, res) => {
    try {
        ...
        const currentEscalationHistory = existingTask.escalationHistory || [];
...
router.get("/incidents", async (req, res) => {
    try {
        ...
        // Additional filters (only if allowed by franchise scope)
        if (req.query.franchiseId && franchiseFilter === null) {
            filters.push(eq(opsIncidents.franchiseId, parseInt(req.query.franchiseId)));
        }
        if (req.query.region) {
            filters.push(eq(opsIncidents.region, req.query.region));
        }
        if (req.query.country) {
            filters.push(eq(opsIncidents.country, req.query.country));
        }
        if (req.query.status) {
            filters.push(eq(opsIncidents.status, req.query.status));
        }
        if (req.query.severity) {
            filters.push(eq(opsIncidents.severity, req.query.severity));
        }
        if (req.query.escalationLevel) {
            filters.push(eq(opsIncidents.escalationLevel, parseInt(req.query.escalationLevel)));
        }
        if (req.query.assignedTo) {
            filters.push(eq(opsIncidents.assignedTo, parseInt(req.query.assignedTo)));
        }
        if (req.query.stationId) {
            filters.push(eq(opsIncidents.stationId, req.query.stationId));
        }
        ...
        const offset = parseInt(req.query.offset || "0");
        // Whitelisted sorting (prevents schema inference)
        const sortBy = req.query.sortBy || "reportedAt";
        const sortOrder = req.query.sortOrder || "desc";
        if (!INCIDENT_SORT_FIELDS.includes(sortBy)) {
            return res.status(400).json({
                error: "Invalid sortBy field",
                allowed: INCIDENT_SORT_FIELDS
            });
        }
        const sortColumn = opsIncidents[sortBy];
...
router.patch("/incidents/:id", async (req, res) => {
    try {
        ...
        const actionLogEntry = {
            ...
            details: {
                changed: Object.keys(validated),
                previous: Object.fromEntries(Object.keys(validated).map(key => [key, existingIncident[key]])),
            },
        };
        const currentActionLog = existingIncident.actionLog || [];
        const updatedActionLog = [...currentActionLog, actionLogEntry];
        const [incident] = await db
            .update(opsIncidents)
            .set({
...
router.post("/incidents/:id/escalate", async (req, res) => {
    try {
        ...
        const currentEscalationHistory = existingIncident.escalationHistory || [];
        const updatedEscalationHistory = [...currentEscalationHistory, escalationEntry];
        // Build action log entry (append, don't overwrite)
        const actionLogEntry = {
            action: "escalate",
...
router.patch("/incidents/bulk", async (req, res) => {
    try {
        const bulkUpdateSchema = z.object({
            ids: z.array(z.number()).max(50),
