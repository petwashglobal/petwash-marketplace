...
const requireAdminOrViewer = (req, res, next) => {
...
router.post('/broadcast/users', validateFirebaseToken, requireAdmin, async (req, res) => {
    try {
        const { title, bodyHtml, type, ctaText, ctaUrl, locale, priority, targetUserIds, // Optional: specific user IDs
        ...
        if (targetUserIds && targetUserIds.length > 0) {
            ...
            if (segmentType === 'pet_owners') {
                ...
                userIds = Array.from(petOwnerUids);
            }
...
router.post('/broadcast/franchises', validateFirebaseToken, requireAdmin, async (req, res) => {
    try {
        const { title, bodyHtml, category, requiresAck, attachments, targetFranchiseIds, // Optional: specific franchise IDs
         } = req.body;
        const sanitizedBody = sanitizeHtml(bodyHtml, {
            allowedTags: sanitizeHtml.defaults.allowedTags.concat(['img', 'h1', 'h2']),
            allowedAttributes: {
