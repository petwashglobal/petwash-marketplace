...
import { insertAccountsPayableSchema, insertAccountsReceivableSchema, insertGeneralLedgerSchema, insertTaxReturnSchema, insertTaxPaymentSchema, insertTaxAuditLogSchema, } from "@shared/schema-finance";
...
router.post("/tax-returns", async (req, res, next) => {
    try {
        ...
        const result = await TaxComplianceService.createTaxReturn(data, userId.toString(), ipAddress, userAgent);
        // Save to database
...
router.post("/tax-returns/:id/submit", async (req, res, next) => {
    try {
        ...
        const submissionResult = await TaxComplianceService.submitTaxReturn(taxReturn, userId.toString(), ipAddress, userAgent);
...
router.post("/tax-payments", async (req, res, next) => {
    try {
        ...
        const result = await TaxComplianceService.recordTaxPayment(data, userId.toString(), ipAddress, userAgent);
        // Save to database
