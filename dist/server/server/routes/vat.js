...
router.post("/record-transaction", requireAuth, async (req, res) => {
    try {
        ...
        const entry = await VATCalculatorService.recordTransaction(platform, transactionId, baseAmount, bookingId, metadata);
        res.json({ entry });
    }
    catch (error) {
...
router.get("/platform-pl/:platform", requireAdmin, async (req, res) => {
    try {
        ...
        const pl = await VATCalculatorService.getPlatformPL(platform, start, end);
        res.json({ pl });
    }
    catch (error) {
