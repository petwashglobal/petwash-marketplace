...
import { db } from '../db';
import { contractorReviews, contractorTrustScores, reviewFlaggingRules, sitterBookings, walkBookings, pettrekTrips } from '@shared/schema';
...
router.post('/submit', requireAuth, async (req, res) => {
    try {
        ...
        const { bookingType, bookingId, reviewType, overallRating, punctualityRating, communicationRating, professionalismRating, cleanlinessRating, safetyRating, reviewText, reviewPhotos } = req.body;
        ...
        const existingReview = await db
            ...
            .where(and(eq(contractorReviews.bookingId, bookingId), eq(contractorReviews.reviewType, reviewType), eq(contractorReviews.reviewerId, userId)))
            .limit(1);
...
router.get('/:subjectId', async (req, res) => {
    try {
        ...
        const reviews = await db
            .select()
            .from(contractorReviews)
            .where(and(eq(contractorReviews.subjectId, subjectId), eq(contractorReviews.isVisible, true), eq(contractorReviews.isPublic, true)))
 ...
 */
router.get('/trust-score/:contractorId', async (req, res) => {
    try {
 ...
 */
async function updateContractorTrustScore(contractorId, contractorType) {
    try {
        // Get all reviews for this contractor
        const reviews = await db
            .select()
            .from(contractorReviews)
            .where(and(eq(contractorReviews.subjectId, contractorId), eq(contractorReviews.reviewType, 'owner_to_contractor'), eq(contractorReviews.isVisible, true)));
        const totalReviews = reviews.length;
        ...
        if (providerRecord.length > 0) {
            const bookingCount = await db
                ...
                .from(bookings)
                .where(and(eq(bookings.providerId, providerRecord[0].id), eq(bookings.status, 'completed')));
