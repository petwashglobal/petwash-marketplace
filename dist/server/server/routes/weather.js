...
import { getWeatherConditionTranslation, getRecommendationTranslation, getUIText } from '../lib/weatherTranslations';
import { getUserType, getLocalizedMetadata, } from '@shared/schema-weather-planner';
import { optionalFirebaseToken } from '../middleware/firebase-auth';
import { optionalEmployeeProfile } from '../middleware/roleAuth';
const router = express.Router();
// Supported languages
const SUPPORTED_LANGS = ['en', 'he', 'ar', 'ru', 'fr', 'es'];
...
router.get('/forecast', async (req, res) => {
    try {
        ...
        const data = await getUnifiedLocationData(location);
...
router.get('/7-day-planner', async (req, res) => {
    try {
        ...
        const sevenDayPlanner = data.daily.time.map((date, index) => {
...
// ===== HELPER FUNCTIONS =====
function calculateWashDayScore(weather) {
    let score = 100;
    ...
    if (weather.uvIndex && weather.uvIndex >= 3 && weather.uvIndex <= 7)
        score += 10;
    // Humidity penalties
...
function calculateDayWashScore(temp, weatherCode, precipProb, uvIndex, windSpeed) {
    let score = 100;
    // Temperature
    if (temp < 15)
        score -= 20;
...
function getWashDayRecommendation(score, weather, lang = 'en') {
...
router.post('/smart-advice', async (req, res) => {
    try {
        ...
        const smartAdvice = await smartWeatherAdvisor.generateSmartAdvice(operatorName || 'Operator', weatherData, appointments, language);
        res.json({
            success: true,
            location: geocoded,
            weather: weatherData,
 ...
 */
router.get('/planner', optionalFirebaseToken, optionalEmployeeProfile, async (req, res) => {
    try {
        const { location, lang } = req.query;
        const language = validateLanguage(lang);
        ...
        if (userType === 'public') {
            ...
            const weatherData = await fetch7DayForecast(location, language);
            const publicView = {
                success: true,
        ...
        if (userType === 'employee_station') {
            ...
            const stationLocation = location || 'Tel Aviv';
        ...
        if (userType === 'employee_executive') {
            ...
            const locationWeatherData = await Promise.all(mainLocations.map(loc => fetch7DayForecast(loc, language)));
 ...
 */
async function fetch7DayForecast(location, language) {
    const locale = LOCALE_MAP[language];
    // Geocode location
    const geocoded = await geocodeLocation(location);
    if (!geocoded) {
    ...
    const forecastDays = forecastData.daily.time.map((date, index) => {
    ...
    const bestWashDay = forecastDays.reduce((best, day) => day.washScore > best.washScore ? day : best, forecastDays[0]);
    return {
        location: {
            city: geocoded.city,
...
router.get('/comprehensive', async (req, res) => {
    try {
        ...
        const weather = await multiSourceWeather.getComprehensiveWeather(geocoded.latitude, geocoded.longitude);
        if (!weather) {
