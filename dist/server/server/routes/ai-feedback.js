...
router.post('/task-complete', requireAuth, async (req, res) => {
    try {
        ...
        const feedback = await EmployeeAIFeedbackService.onTaskComplete(employeeUid, taskId);
        res.json({
            success: true,
            feedback,
            message: feedback
...
router.get('/insights', requireAuth, async (req, res) => {
    try {
        const employeeUid = req.user.uid;
        const insights = await EmployeeAIFeedbackService.getPerformanceInsights(employeeUid);
        res.json({
            success: true,
            insights,
        });
