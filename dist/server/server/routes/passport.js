 ...
 */
router.post('/verify', upload.single('passport'), async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
        ...
        if (approvedCountries.length > 0) {
            const isApproved = passportOCRService.isApprovedCountry(passportData.countryCode, approvedCountries);
            if (!isApproved) {
