...
router.post('/vip-card', requireAuth, async (req, res) => {
    try {
        ...
        await WalletFraudDetection.logWalletDownload(userId, userEmail, 'vip_card', req.ip || 'unknown', req.headers['user-agent'] || 'unknown');
        // AI-assisted telemetry: record download with UA detection
        const telemetryToken = req.query.t;
        if (telemetryToken) {
...
router.post('/e-voucher', requireAuth, async (req, res) => {
    try {
        ...
        // AI-assisted telemetry: record download with UA detection
        const telemetryToken = req.query.t || await WalletTelemetryService.createSession({
            userId,
            passType: 'voucher',
            platform: 'apple'
        });
...
router.get('/pass/:linkId', async (req, res) => {
    try {
        ...
        await WalletFraudDetection.logWalletDownload(linkData.userId, userEmail, linkData.passType, req.ip || 'unknown', req.headers['user-agent'] || 'unknown');
