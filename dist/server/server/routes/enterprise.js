...
import { db } from "../db";
import { countries, franchiseTerritories, franchisees, petWashStations, stationBills, stationAssets, spareParts, stationSpareParts, maintenanceWorkOrders, stationTelemetry, stationAlerts, insertCountrySchema, insertFranchiseTerritorySchema, insertFranchiseeSchema, insertPetWashStationSchema, insertStationBillSchema, insertStationAssetSchema, insertSparePartSchema, insertMaintenanceWorkOrderSchema } from "../../shared/schema-enterprise";
...
router.get("/franchisees", requireAdmin, async (req, res) => {
    try {
        ...
        const conditions = [];
        if (status)
            conditions.push(eq(franchisees.status, status));
        if (countryId)
            conditions.push(eq(franchisees.countryId, Number(countryId)));
        if (territoryId)
            conditions.push(eq(franchisees.territoryId, Number(territoryId)));
...
router.get("/stations", requireAdmin, async (req, res) => {
    try {
        ...
        const conditions = [];
        if (franchiseeId)
            conditions.push(eq(petWashStations.franchiseeId, Number(franchiseeId)));
        if (territoryId)
            conditions.push(eq(petWashStations.territoryId, Number(territoryId)));
        if (countryId)
            conditions.push(eq(petWashStations.countryId, Number(countryId)));
        if (status)
            conditions.push(eq(petWashStations.operationalStatus, status));
        if (healthStatus)
            conditions.push(eq(petWashStations.healthStatus, healthStatus));
    ...
    try {
        ...
        const [bills, assets, alerts, latestTelemetry] = await Promise.all([
            ...
            db.select().from(stationAlerts)
                .where(and(eq(stationAlerts.stationId, station.id), eq(stationAlerts.status, 'open')))
...
router.get("/spare-parts", requireAdmin, async (req, res) => {
    try {
        ...
        const parts = category
            ? await db.select().from(spareParts).where(eq(spareParts.category, category)).orderBy(spareParts.partName)
            : await db.select().from(spareParts).orderBy(spareParts.partName);
        // Filter low stock items if requested
        if (lowStock === 'true') {
            const filtered = parts.filter(p => (p.quantityInStock || 0) <= (p.reorderPoint || 0));
...
router.get("/work-orders", requireAdmin, async (req, res) => {
    try {
        ...
        const conditions = [];
        if (stationId)
            conditions.push(eq(maintenanceWorkOrders.stationId, Number(stationId)));
        if (status)
            conditions.push(eq(maintenanceWorkOrders.status, status));
        if (priority)
            conditions.push(eq(maintenanceWorkOrders.priority, priority));
        if (technicianId)
            conditions.push(eq(maintenanceWorkOrders.assignedToTechnicianId, technicianId));
...
router.get("/analytics/global", requireAdmin, async (req, res) => {
    try {
        ...
        const [stationStats] = await db
            .select({
            totalStations: count(),
            activeStations: sql `count(*) filter (where ${petWashStations.operationalStatus} = 'active')`,
            offlineStations: sql `count(*) filter (where ${petWashStations.healthStatus} = 'offline')`,
            maintenanceStations: sql `count(*) filter (where ${petWashStations.operationalStatus} = 'maintenance')`
        })
            .from(petWashStations);
        ...
        const [franchiseeStats] = await db
            ...
            activeFranchisees: sql `count(*) filter (where ${franchisees.status} = 'active')`
        })
            .from(franchisees);
        // Get bill stats
        const [billStats] = await db
            .select({
            totalUnpaid: count(),
            totalOverdue: sql `count(*) filter (where ${stationBills.status} = 'overdue')`,
        ...
        const [alertStats] = await db
            .select({
            totalAlerts: count(),
            criticalAlerts: sql `count(*) filter (where ${stationAlerts.severity} = 'critical')`,
            warningAlerts: sql `count(*) filter (where ${stationAlerts.severity} = 'warning')`
        })
            .from(stationAlerts)
        ...
        const [workOrderStats] = await db
            .select({
            totalPending: sql `count(*) filter (where ${maintenanceWorkOrders.status} = 'pending')`,
            totalInProgress: sql `count(*) filter (where ${maintenanceWorkOrders.status} = 'in_progress')`,
            totalCompleted: sql `count(*) filter (where ${maintenanceWorkOrders.status} = 'completed')`
        })
...
router.get("/analytics/franchisee/:id", requireAdmin, async (req, res) => {
    try {
        ...
        const alerts = await db
            .select()
            .from(stationAlerts)
            .where(and(sql `${stationAlerts.stationId} = ANY(${stationIds})`, eq(stationAlerts.status, 'open')));
        res.json({
            stations,
