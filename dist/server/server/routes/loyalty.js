...
import { loyaltyProfiles, pointsTransactions, badges, userBadges, dailyChallenges, userChallenges, rewardsMarketplace, userRedemptions, referrals, } from '../../shared/schema-loyalty';
    ...
    try {
        ...
        if (!profile) {
            ...
            try {
                ...
                if (userEmail) {
                    await sendLoyaltyEnrollmentConfirmation(userEmail, firstName, 'bronze', 0, language);
 ...
 */
router.get('/badges/unlocked', async (req, res) => {
    try {
        const userId = req.firebaseUser.uid;
 ...
 */
router.post('/badges/unlock', requireAdmin, async (req, res) => {
    try {
...
router.get('/challenges/active', async (req, res) => {
    try {
        ...
        const activeChallenges = await db
            ...
            .where(and(eq(userChallenges.userId, userId), eq(userChallenges.status, 'active'), gte(userChallenges.expiresAt, now)))
...
router.post('/challenges/claim', async (req, res) => {
    try {
        ...
        const [userChallenge] = await db
            .select()
            .from(userChallenges)
            .where(and(eq(userChallenges.userId, userId), eq(userChallenges.challengeId, challengeId), eq(userChallenges.status, 'completed')))
            .limit(1);
        if (!userChallenge) {
...
router.get('/rewards', async (_req, res) => {
    try {
        ...
        const availableRewards = await db
            ...
            .where(and(eq(rewardsMarketplace.isActive, true), gte(rewardsMarketplace.validUntil, now)))
 ...
 */
router.get('/personalized-message', async (req, res) => {
    try {
        ...
        const campaigns = await db
            ...
            .where(and(eq(loyaltyCampaigns.isActive, true), gte(loyaltyCampaigns.endDate, now)))
