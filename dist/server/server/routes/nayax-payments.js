 ...
 */
function assessTransactionRisk(amount, customerUid) {
...
router.post('/settle', requireAuth, async (req, res) => {
    try {
        ...
        if (result.success) {
            try {
                await eventPublisher.publishEvent(DomainEventType.PAYMENT_CAPTURED, {
                    transactionId,
                    amount: amount || 0,
                    currency: 'ILS',
                    paymentMethod: 'nayax',
...
router.post('/cortina/inquiry', async (req, res) => {
    try {
        const { transactionId, qrCodeData, amount, deviceId, } = req.body;
