...
import { db } from "../db";
import { contractorProfiles, contractorInsurance, contractorBackgroundChecks, contractorBankDetails, contractorServiceAreas, contractorCapabilities, providerTaxCompliance, } from "@shared/schema";
...
router.post("/profile", async (req, res) => {
    try {
        const { id, displayName, legalName, email, phoneE164, whatsappOptIn, primaryCity, languageCodes, } = req.body;
...
router.post("/accept-terms", async (req, res) => {
    try {
        ...
        const updates = {};
        if (acceptTerms)
            updates.acceptedPlatformTermsAt = new Date();
        if (acceptIndependentStatus)
            updates.acceptedIndependentStatusAt = new Date();
        if (acceptPrivacyPolicy)
            updates.acceptedPrivacyPolicyAt = new Date();
...
router.post("/capability", async (req, res) => {
    try {
        ...
        const existing = await db
            ...
            .where(and(eq(contractorCapabilities.contractorId, contractorId), eq(contractorCapabilities.serviceType, serviceType)))
            .limit(1);
        if (existing.length > 0) {
            return res.status(400).json({
