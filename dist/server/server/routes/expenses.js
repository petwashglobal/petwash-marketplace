 ...
 */
router.post("/expenses/ocr-receipt", upload.single('receipt'), async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
...
router.get("/expenses", async (req, res) => {
    try {
        ...
        const reportPeriod = req.query.reportPeriod;
        let query = db.select().from(expenses).where(eq(expenses.employeeId, user.id));
        if (status) {
            query = query.where(and(eq(expenses.employeeId, user.id), eq(expenses.status, status)));
        }
        if (reportPeriod) {
            query = query.where(and(eq(expenses.employeeId, user.id), eq(expenses.reportPeriod, reportPeriod)));
        }
...
router.get("/expenses/pending-approval", async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.status(401).json({
        ...
        const whereConditions = isCEO
            ? and(eq(expenses.status, 'pending'), or(eq(expenses.approverId, user.id), eq(expenses.approverId, 'CEO_DEFAULT')))
            : and(eq(expenses.status, 'pending'), eq(expenses.approverId, user.id));
...
router.post("/expenses/seed-tax-rates", async (req, res) => {
    try {
