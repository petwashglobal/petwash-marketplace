...
import { db } from '../db';
import { sitterProfiles, walkerProfiles, pets, trainers, drivers, stations, bookingSearchFiltersSchema } from '@shared/schema';
import { eq, and, gte, sql, desc, asc, or, ilike } from 'drizzle-orm';
 ...
 * Search pet sitters
 */
async function searchSitters(filters, searchId) {
    try {
        ...
        if (filters.area) {
            conditions.push(or(ilike(sitterProfiles.city, `%${filters.area}%`), ilike(sitterProfiles.streetAddress, `%${filters.area}%`)));
        }
        ...
        const providers = results.map(sitter => {
            const housePolicies = sitter.housePolicies;
            return {
                id: sitter.id,
        ...
        logger.error('[BookingSearch] Sitter search error', { error: error.message });
 ...
 * Search dog walkers
 */
async function searchWalkers(filters, searchId) {
 ...
 */
async function searchGroomers(filters, searchId) {
    try {
        const conditions = [
 ...
 */
async function searchDrivers(filters, searchId) {
    try {
 ...
 * Search trainers
 */
async function searchTrainers(filters, searchId) {
    try {
 ...
 */
async function searchK9000Stations(filters, searchId) {
    try {
        ...
        if (filters.area) {
            conditions.push(or(ilike(stations.city, `%${filters.area}%`), ilike(stations.address, `%${filters.area}%`)));
        }
 ...
 */
function buildTrainerBadges(trainer) {
    const badges = [];
    if (trainer.verificationStatus === 'verified')
        badges.push('verified');
    if (trainer.isCertified)
        badges.push('certified');
    if ((trainer.yearsOfExperience || 0) >= 5)
        badges.push('experienced');
    if (parseFloat(trainer.averageRating || '0') >= 4.8)
 ...
 */
function buildBadges(sitter) {
    const badges = [];
    if (sitter.isVerified)
        badges.push('verified');
    if (sitter.backgroundCheckStatus === 'passed')
        badges.push('police_check');
    if (sitter.yardSize && sitter.yardSize !== 'none')
        badges.push('has_yard');
    if ((sitter.yearsOfExperience || 0) >= 5)
        badges.push('experienced');
    if (parseFloat(sitter.rating || '0') >= 4.8)
        badges.push('top_rated');
 ...
 */
function buildWalkerBadges(walker) {
    const badges = [];
    if (walker.verificationStatus === 'verified')
        badges.push('verified');
    if (walker.policeCheckVerified)
        badges.push('police_check');
    if (walker.bodyCamera)
        badges.push('body_camera');
    if (walker.gpsTracking)
        badges.push('gps_tracking');
    if ((walker.yearsOfExperience || 0) >= 5)
        badges.push('experienced');
    if ((walker.averageRating || 0) >= 4.8)
        badges.push('top_rated');
    return badges;
}
