...
import { db } from '../db';
import { walkerProfiles, walkBookings, walkGpsTracking, walkHealthData, walkBlockchainAudit, walkerReviews, walkAlerts } from '../../shared/schema';
...
router.patch('/api/walkers/:walkerId', async (req, res) => {
    try {
        ...
        if (walker.userId !== userId && !req.user?.isAdmin) {
...
router.post('/api/walkers/search', async (req, res) => {
    try {
        ...
        let query = db
            .select()
            .from(walkerProfiles)
            .where(and(eq(walkerProfiles.verificationStatus, 'verified'), eq(walkerProfiles.isAvailable, true), eq(walkerProfiles.isActive, true), gte(walkerProfiles.averageRating, minRating.toString()), 
        // Bounding box filter
        gte(walkerProfiles.currentLatitude, (latitude - latDelta).toString()), lte(walkerProfiles.currentLatitude, (latitude + latDelta).toString()), gte(walkerProfiles.currentLongitude, (longitude - lonDelta).toString()), lte(walkerProfiles.currentLongitude, (longitude + lonDelta).toString())));
...
router.post('/api/walks/book', requireLoyaltyMember, async (req, res) => {
    try {
        ...
        const { walkerId, scheduledDate, scheduledStartTime, durationMinutes, pickupLatitude, pickupLongitude, pickupAddress, petName, petBreed, petWeight, petSpecialNeeds } = req.body;
        ...
        const bookingConfirmation = await walkEliteBookingEngine.confirmBooking(bookingId, pricing, ownerId);
...
router.get('/api/users/:userId/walks', async (req, res) => {
    try {
        ...
        if (status) {
            query = query.where(eq(walkBookings.status, status));
        }
