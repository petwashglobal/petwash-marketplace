...
router.post('/redemptions', redemptionRateLimiter, async (req, res) => {
    try {
        ...
        const result = await walletService.createRedemptionSession(userId, platform, requestedAmountCents, serviceType, bookingId, stationId, deviceInfo);
...
router.post('/credits/add', async (req, res) => {
    try {
        ...
        await walletService.addCredits(targetUserId, creditType, amount, sourceType, sourceId, description);
...
router.post('/nayax/validate-code', nayaxValidationRateLimiter, async (req, res) => {
    try {
        const stationApiKey = req.headers['x-station-key'];
        if (!stationApiKey) {
