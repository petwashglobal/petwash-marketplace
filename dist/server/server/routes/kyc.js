...
import { hashIdNumber, checkDuplicateId, getKYCDocument, updateKYCDocument, validateKYCFile, getPendingKYCSubmissions, approveKYC, rejectKYC, isTrustedCountry, TRUSTED_COUNTRIES } from '../kyc';
...
router.get('/admin/document/:uid', requireAdmin, async (req, res) => {
    try {
        const { uid } = req.params;
        const kycDoc = await getKYCDocument(uid);
...
router.delete('/delete/:uid', validateFirebaseToken, loadUserRole, async (req, res) => {
    try {
        const authReq = req;
