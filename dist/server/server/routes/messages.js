...
import { db } from '../db';
import { userMessages, messageAttachments, insertUserMessageSchema } from '../../shared/schema';
...
router.get('/inbox', async (req, res) => {
    try {
        ...
        const messages = await db
            .select()
            .from(userMessages)
            .where(and(or(eq(userMessages.senderId, userId), eq(userMessages.recipientId, userId)), eq(userMessages.permanentlyDeleted, false)))
...
router.delete('/:messageId', async (req, res) => {
    try {
        ...
        if ((message.deletedBySender && message.recipientId === userId) ||
...
router.get('/unread/count', async (req, res) => {
    try {
        ...
        const unreadMessages = await db
            .select()
            .from(userMessages)
            .where(and(eq(userMessages.recipientId, userId), eq(userMessages.isRead, false), eq(userMessages.deletedByRecipient, false), eq(userMessages.permanentlyDeleted, false)));
