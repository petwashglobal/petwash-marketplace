 ...
 */
async function wasReminderRecentlySent(petId, vaccineType, vaccineDate) {
 ...
 */
async function logSentReminder(petId, petName, uid, vaccineType, vaccineDate) {
 ...
 */
export async function processVaccineReminders() {
    ...
    try {
        ...
        const remindersByOwner = new Map();
        ...
        for (const petDoc of petsSnapshot.docs) {
            try {
                ...
                const vaccineTypes = ['rabies', 'dhpp', 'lepto'];
        ...
        for (const [uid, reminder] of remindersByOwner) {
            try {
                ...
                const locale = userProfile?.preferredLanguage || 'en';
             ...
             <ul>${reminder.pets.map((p) => `<li><strong>${escapeHtml(p.petName)}:</strong> ${getVaccineNameHe(p.vaccineType)} (${formatDateHe(p.vaccineDate)})</li>`).join('')}</ul>
             ...
             <ul>${reminder.pets.map((p) => `<li><strong>${escapeHtml(p.petName)}:</strong> ${getVaccineNameEn(p.vaccineType)} (${formatDateEn(p.vaccineDate)})</li>`).join('')}</ul>
...
function getVaccineNameEn(type) {
    const names = {
        rabies: 'Rabies',
...
function getVaccineNameHe(type) {
    const names = {
        rabies: 'כלבת',
