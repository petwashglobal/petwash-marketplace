 ...
 * );
 */
export async function sendPush(token, title, body, options = {}) {
    try {
        const { data = {}, priority = NotificationPriority.HIGH, badge, sound = 'default', channelId = 'default', imageUrl, clickAction, } = options;
        // Build FCM message
        const message = {
            token,
            notification: {
 ...
 * );
 */
export async function sendMulticastPush(tokens, title, body, options = {}) {
    try {
        if (tokens.length === 0) {
 ...
 * );
 */
export async function sendTopicPush(topic, title, body, options = {}) {
    try {
        const { data = {}, priority = NotificationPriority.NORMAL, badge, sound = 'default', imageUrl, } = options;
        const message = {
            topic,
            notification: {
                title,
