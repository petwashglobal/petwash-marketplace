...
export class BackgroundJobProcessor {
    static jobLocks = new Map(); // Per-task locking
    static retryDelays = [1000, 5000, 15000, 60000]; // 1s, 5s, 15s, 1m
    /**
     * Acquire lock for a specific job - prevents overlapping executions
     */
    static async acquireLock(jobName) {
     ...
     */
    static async processReminderWithRetry(reminderId) {
        try {
            ...
            const serviceDetails = reminder.serviceDetails;
     ...
     */
    static async processBirthdayDiscounts() {
            ...
            for (const userDoc of usersSnapshot.docs) {
                try {
                    ...
                    if (birthMonth === todayMonth && birthDay === todayDay) {
                        ...
                        const voucher = await createBirthdayVoucher(uid, profileData?.email || '', currentYear, profileData?.petName || profileData?.dogName);
                        // Send birthday email
     ...
     */
    static async runDataIntegrityCheck() {
        ...
        try {
            ...
            await recordCronExecution(jobName, result.overallStatus !== 'critical', result.overallStatus === 'critical'
     ...
     */
    static async processObservances() {
     ...
     */
    static async processVaccineReminders() {
     ...
     */
    static async autoReleaseEscrowPayouts() {
        try {
    ...
    static async performDailyBackup() {
        ...
        try {
            ...
            try {
                ...
                await googleDriveBackupService.backupJSON(backupSummary, `PetWash_Daily_Summary_${new Date().toISOString().split('T')[0]}`, { includeTimestamp: false });
     ...
     */
    static async triggerReminderProcess() {
        const dueReminders = await storage.getPendingReminders();
        let processed = 0;
        let errors = 0;
        for (const reminder of dueReminders) {
     ...
     */
    static async triggerBirthdayProcess() {
    ...
    static async testReminder(reminderId) {
        ...
        const results = { customerData, appointmentData };
        // Test email reminder
        if (reminder.reminderType === 'email' || reminder.reminderType === 'both') {
     ...
     */
    static async generateDailyRevenueReport() {
        try {
            ...
            await EmailService.sendRevenueReport('daily', periodStr, excelBuffer, pdfBuffer, csvContent, { totalRevenue: data.totalRevenue, totalTransactions: data.totalTransactions });
     ...
     */
    static async generateMonthlyRevenueReport() {
        try {
            ...
            await EmailService.sendRevenueReport('monthly', periodStr, excelBuffer, pdfBuffer, csvContent, { totalRevenue: data.totalRevenue, totalTransactions: data.totalTransactions });
     ...
     */
    static async generateYearlyRevenueReport() {
        try {
            ...
            await EmailService.sendRevenueReport('yearly', periodStr, excelBuffer, pdfBuffer, csvContent, { totalRevenue: data.totalRevenue, totalTransactions: data.totalTransactions });
     ...
     */
    static async generateMonthlyVATDeclaration() {
        try {
     ...
     */
    static async generateManualReport(type, referenceDate) {
        ...
        await EmailService.sendRevenueReport(type, periodStr, excelBuffer, pdfBuffer, csvContent, { totalRevenue: data.totalRevenue, totalTransactions: data.totalTransactions });
        return paths;
    }
    /**
     * Perform weekly code backup to Google Cloud Storage
     */
    static async performWeeklyCodeBackup() {
        try {
     ...
     */
    static async performFirestoreExport() {
        try {
     ...
     */
    static async checkLegalCompliance() {
        try {
            ...
            const reviewsNeedingAttention = await db.select()
                ...
                .where(and(or(
            // Overdue reviews
            and(lt(legalComplianceReviews.nextReviewDue, now), eq(legalComplianceReviews.reviewStatus, 'pending')), 
            // Reviews due within 60 days that haven't been reminded recently
            and(gte(legalComplianceReviews.nextReviewDue, now), lt(legalComplianceReviews.nextReviewDue, reminderThreshold60Days), or(isNull(legalComplianceReviews.reminderSentAt), lt(legalComplianceReviews.reminderSentAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)) // 30 days ago
            )))))
     ...
     */
    static async checkIsraeliCompliance() {
        try {
     ...
     */
    static async checkSecurityUpdates() {
        try {
     ...
     */
    static async cleanupMonitoringData() {
        try {
     ...
     */
    static async processWalletTelemetryAbandonment() {
        try {
     ...
     */
    static async processWeatherNotifications() {
        try {
