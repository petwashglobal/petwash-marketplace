...
import { insertGiftCardSchema, insertWashHistorySchema, insertCustomerSchema, insertCustomerPetSchema, insertCrmCommunicationSchema, updateCrmLeadSchema, crmLeadCreationSchema, updateCrmOpportunitySchema, crmOpportunityCreationSchema, updateCrmTaskSchema, crmTaskCreationSchema, insertCrmDealStageSchema, insertCrmEmailTemplateSchema, updateCrmEmailTemplateSchema, insertCrmSmsTemplateSchema, updateCrmSmsTemplateSchema, insertCrmAppointmentReminderSchema, updateCrmAppointmentReminderSchema, insertCrmCommunicationLogSchema, updateCrmCommunicationLogSchema, subscriptionProducts, subscriptionBoxTypes, customerSubscriptions, subscriptionShipments, aiProductRecommendations, customers, nayaxTransactions, hrDocuments, washHistory, customerPets, users } from "@shared/schema";
import { z } from "zod";
...
import { apiLimiter, paymentLimiter, adminLimiter, uploadLimiter } from './middleware/rateLimiter';
import { loginRateLimitMiddleware, recordFailedLogin, clearLoginAttempts } from './middleware/loginRateLimiter';
import { verifyAppCheckTokenOptional } from './middleware/appCheckMiddleware';
...
export async function registerRoutes(app) {
    ...
    app.get('/payment-status', (req, res) => {
        const isNayaxConfigured = !!(process.env.NAYAX_API_KEY &&
...
`;
    ...
    app.post('/api/consent/oauth', requireAuth, async (req, res) => {
        try {
    ...
    app.post('/api/simple-auth/login', loginRateLimitMiddleware, async (req, res) => {
        try {
            ...
            if (!isValid) {
                ...
                recordFailedLogin(identifier);
                const rateLimit = req.loginRateLimit;
    ...
    app.put('/api/profile', async (req, res) => {
        try {
            ...
            const { firstName, lastName, phone, dateOfBirth, petName, petBreed, petAge, petWeight, address, city, postcode, country, marketingOptIn } = req.body;
            ...
            if (petName !== undefined)
                updates.petName = String(petName).trim();
            if (petBreed !== undefined)
                updates.petBreed = String(petBreed).trim();
            if (petAge !== undefined)
                updates.petAge = String(petAge).trim();
            if (petWeight !== undefined)
                updates.petWeight = String(petWeight).trim();
            if (address !== undefined)
                updates.address = String(address).trim();
            if (city !== undefined)
                updates.city = String(city).trim();
            if (postcode !== undefined)
                updates.postcode = String(postcode).trim();
            if (country !== undefined)
                updates.country = String(country).trim();
    ...
    app.get('/api/greeting', async (req, res) => {
        try {
            ...
            const preferredLanguage = req.query.language || profileData?.preferredLanguage || 'en';
    ...
    app.post('/api/user/delete', async (req, res) => {
        try {
            ...
            await deleteUserData({ ...req, firebaseUser: decoded, session: {} }, res);
        }
        catch (error) {
    ...
    app.put('/api/users/me', async (req, res) => {
        try {
            ...
            const uid = decoded.uid;
            // Validate and extract allowed fields
            const { firstName, lastName, phone, dateOfBirth, address, city, postcode, country, marketingOptIn } = req.body;
            // Build update object (only include provided fields)
            const updates = {};
            if (firstName !== undefined)
            ...
            if (postcode !== undefined)
                updates.postcode = postcode.trim();
            if (country !== undefined)
    ...
    app.post('/api/webauthn/register/options', webauthnLimiter, async (req, res) => {
        try {
            ...
            const { generateRegistrationOptionsForUser, } = await import('./webauthn/service');
            const result = await generateRegistrationOptionsForUser(decoded.uid, decoded.email || '', isAdmin, req, res);
            if (!result.success) {
    ...
    app.post('/api/webauthn/register/verify', webauthnLimiter, async (req, res) => {
        ...
        try {
            ...
            const result = await verifyAndStoreRegistration(response, req, res);
            if (!result.verified) {
    ...
    app.delete('/api/webauthn/credentials/:credentialId', webauthnLimiter, async (req, res) => {
        ...
        try {
            ...
            // Send security alert
            await alertPasskeyRevoked(decoded.uid, req.params.credentialId, decoded.email || undefined, deviceLabel);
            const lang = (await import('./lib/i18n')).getLanguage(req);
            const { webauthnMessages, t } = await import('./lib/i18n');
    ...
    app.get('/api/auth/tiktok/callback', async (req, res) => {
        try {
            ...
            if (!storedState || !storedVerifier || storedState !== state) {
                logger.error('[TikTok OAuth] State mismatch', { stored: storedState?.substring(0, 8), received: state.substring(0, 8) });
    ...
    app.get('/api/debug/webauthn', requireAdmin, async (req, res) => {
        try {
            ...
            if (uid) {
                const snapshot = await firestoreDb
                    ...
                    .collection('devices')
                    .get();
                userCredentials = snapshot.docs.map((doc) => ({
    ...
    app.post('/api/gift-cards/redeem', requireAuth, async (req, res) => {
        try {
            const { code } = req.body;
    ...
    app.get('/api/gift-cards', requireAdmin, async (req, res) => {
        const correlationId = crypto.randomUUID();
...
<body>
    ...
    // Wash history
    app.get('/api/wash-history', requireAuth, verifyAppCheckTokenOptional, async (req, res) => {
        try {
            const customerId = req.session?.customerId;
    ...
    app.post('/api/wash-history', requireAuth, async (req, res) => {
        try {
            const customerId = req.session?.customerId;
            const customer = await storage.getCustomer(customerId);
            if (!customer) {
    ...
    app.post('/api/nayax/payment', paymentLimiter, async (req, res) => {
        try {
    ...
    app.get('/api/admin/stations/alerts', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/stations/alerts/:alertId/acknowledge', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/stations/:stationId/faults', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/stations/faults/:faultId/resolve', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/stations/:stationId/simulate-fault', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/stations/:stationId/ack-alerts', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/stations/:stationId/alerts', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/stations/:stationId/maintenance', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/monitoring/test', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/monitoring/tests', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/monitoring/run-all-tests', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/webhooks/nayax', express.raw({ type: 'application/json' }), async (req, res) => {
        try {
            const signature = req.headers['x-nayax-signature'];
            if (!signature) {
            ...
            const { verifyWebhookSignature, logWebhookEvent, markEventProcessed, updateTransactionStatus, getTransaction, createVoucher } = await import('./nayaxFirestoreService');
            // Verify signature
    ...
    app.post('/api/test-purchase', async (req, res) => {
        try {
            ...
            const qrCode = await QRCodeService.generateVoucherQRCode(selectedPackageId, testTransaction.voucherCode, pkg.washCount, new Date(Date.now() + 365 * 24 * 60 * 60 * 1000) // Expires in 1 year
            );
            // Send purchase confirmation email
            logger.info('Sending purchase confirmation email...');
            const emailSent = await EmailService.sendPurchaseConfirmation(taxInvoice, testTransaction.voucherCode);
    ...
    app.post('/api/multi-service-gift', async (req, res) => {
        try {
            ...
            const { value, currency, recipientName, recipientEmail, senderName, senderEmail, message, eligibleServices } = parseResult.data;
            ...
            if (paymentMethod === 'credit_card' || paymentMethod === 'nayax') {
                ...
                if (discount > 0 && discountAmount > 0) {
                    if (discountType === 'birthday_coupon' && birthdayCoupon.birthdayYear) {
                        ...
                        await markBirthdayCouponUsed(userId, washHistory.id.toString(), discountAmount, Number(pkg.price), finalPrice, packageId, birthdayCoupon.birthdayYear);
                    }
                    else {
    ...
    app.get('/api/my-vouchers', requireAuth, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.post('/api/vouchers/purchase', async (req, res) => {
        ...
        try {
            ...
            await EmailService.sendVoucherPurchaseEmail(emailRecipient, result.codePlain, result.codeLast4, data.amount.toFixed(2), data.currency, data.expiresAt ? new Date(data.expiresAt) : null, 'he');
    ...
    app.post('/api/vouchers/claim', requireAuth, verifyAppCheckTokenOptional, voucherClaimLimiter, async (req, res) => {
        const correlationId = crypto.randomUUID();
        try {
            const schema = z.object({
                code: z.string().min(1)
            ...
            const user = await storage.getUser(userId);
            if (user?.email) {
                await EmailService.sendVoucherClaimEmail(user.email, result.voucher.codeLast4, result.voucher.remainingAmount, result.voucher.currency, 'he');
            }
    ...
    app.post('/api/vouchers/redeem', requireAuth, verifyAppCheckTokenOptional, paymentLimiter, async (req, res) => {
        const correlationId = crypto.randomUUID();
        try {
            const schema = z.object({
    ...
    // Get user's vouchers (authenticated)
    app.get('/api/vouchers/my-vouchers', requireAuth, verifyAppCheckTokenOptional, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.get('/api/admin/vouchers', requireAdmin, async (req, res) => {
        try {
            ...
            if (search) {
                ...
                vouchers = vouchers.filter(v => v.codeLast4?.toLowerCase().includes(searchLower) ||
    ...
    app.get('/api/admin/vouchers/:id', requireAdmin, async (req, res) => {
        try {
            const voucherId = req.params.id;
    ...
    app.post('/api/admin/vouchers/generate-test', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/vouchers/:id/void', requireAdmin, async (req, res) => {
        try {
            const voucherId = req.params.id;
     ...
     */
    app.get('/api/admin/analytics/stations', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/dashboard/stats', requireAdmin, async (req, res) => {
        try {
            ...
            const revenueResult = await db
                .select({
                total: sql `COALESCE(SUM(CAST(${nayaxTransactions.amount} AS DECIMAL)), 0)`,
            })
                .from(nayaxTransactions)
                .where(and(gte(nayaxTransactions.completedAt, startOfMonth), eq(nayaxTransactions.status, 'settled'), sql `CAST(${nayaxTransactions.amount} AS DECIMAL) > 0` // Positive amounts only
            ));
    ...
    app.get('/api/admin/nayax/config', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/nayax/transactions', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/reports/nayax/daily', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/backups/code', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/backups/firestore', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/backups/status', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/backups/logs', requireAdmin, async (req, res) => {
        try {
            const { db } = await import('./lib/firebase-admin');
            const limit = req.query.limit ? parseInt(req.query.limit) : 50;
            const type = req.query.type; // 'code' or 'firestore'
    ...
    app.post('/api/admin/nayax/seed-test-data', requireAdmin, async (req, res) => {
        try {
            const { db } = await import('./lib/firebase-admin');
    ...
    app.post('/api/admin/nayax/backfill-metadata', requireAdmin, async (req, res) => {
        try {
            const { db } = await import('./lib/firebase-admin');
    ...
    app.post('/api/smart-receipts', async (req, res) => {
        try {
            const { userId, packageId, customerEmail, customerName, paymentMethod, originalAmount, discountApplied, finalTotal, nayaxTransactionId, locationName, washDuration } = req.body;
    ...
    app.get('/api/users/:userId/receipts', requireAuth, async (req, res) => {
        try {
    ...
    app.get('/api/crm/dashboard/overview', requireAdmin, async (req, res) => {
        try {
            ...
            const totalRevenue = allUsers.reduce((sum, user) => sum + parseFloat(user.totalSpent || '0'), 0);
    ...
    // CRM Sales Pipeline
    app.get('/api/crm/dashboard/pipeline', requireAdmin, async (req, res) => {
        try {
            ...
            const pipelineData = dealStages.map(stage => {
                ...
                const totalValue = stageOpportunities.reduce((sum, opp) => sum + parseFloat(opp.estimatedValue || '0'), 0);
                return {
                    id: stage.id,
                    name: stage.name,
            ...
            res.json({
                ...
                totalPipelineValue: Math.round(opportunities.reduce((sum, opp) => sum + parseFloat(opp.estimatedValue || '0'), 0))
            });
        }
        catch (error) {
    ...
    app.get('/api/crm/dashboard/customer-analytics', requireAdmin, async (req, res) => {
        try {
    ...
    // CRM Lead Management
    app.get('/api/crm/dashboard/leads', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/dashboard/communications', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/dashboard/marketing', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/dashboard/revenue', requireAdmin, async (req, res) => {
        try {
            ...
            const totalUserSpending = allUsers.reduce((sum, user) => sum + parseFloat(user.totalSpent || '0'), 0);
            ...
            const recentWashes = await db
                ...
                .from(washHistory)
                .where(and(gte(washHistory.createdAt, thirtyDaysAgo), eq(washHistory.status, 'completed')))
    ...
    app.get('/api/crm/leads', requireAdmin, async (req, res) => {
        try {
            const { status, source, assignedTo, search, page = 1, limit = 25, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
            ...
            if (search) {
                ...
                filteredLeads = leads.filter(lead => lead.firstName.toLowerCase().includes(searchTerm) ||
    ...
    // Update lead
    app.put('/api/crm/leads/:id', requireAdmin, async (req, res) => {
        try {
            const leadId = parseInt(req.params.id);
    ...
    app.post('/api/crm/leads/:id/convert', requireAdmin, async (req, res) => {
        try {
            const leadId = parseInt(req.params.id);
    ...
    app.post('/api/crm/leads/:id/opportunities', requireAdmin, async (req, res) => {
        try {
            const leadId = parseInt(req.params.id);
    ...
    app.post('/api/crm/leads/:id/communications', requireAdmin, async (req, res) => {
        try {
            const leadId = parseInt(req.params.id);
    ...
    app.post('/api/crm/leads/:id/tasks', requireAdmin, async (req, res) => {
        try {
            const leadId = parseInt(req.params.id);
    ...
    // Get lead analytics
    app.get('/api/crm/leads/analytics', requireAdmin, async (req, res) => {
        try {
            ...
            if (startDate || endDate) {
                filteredLeads = leads.filter(lead => {
                    ...
                    if (startDate && createdDate < new Date(startDate))
                        return false;
                    if (endDate && createdDate > new Date(endDate))
                        return false;
                    return true;
                });
            }
            ...
            const totalEstimatedValue = filteredLeads.reduce((sum, lead) => sum + parseFloat(lead.estimatedMonthlyValue || '0'), 0);
    ...
    app.get('/api/crm/analytics', requireAdmin, async (req, res) => {
        try {
            ...
            const wonValue = wonOpportunities.reduce((sum, opp) => sum + parseFloat(opp.estimatedValue || '0'), 0);
            // Lead source distribution
            ...
            const avgCustomerValue = customers.reduce((sum, customer) => sum + parseFloat(customer.totalSpent || '0'), 0) / totalCustomers || 0;
            res.json({
                leads: {
                    total: totalLeads,
    ...
    app.get('/api/crm/opportunities', requireAdmin, async (req, res) => {
        try {
            ...
            if (stage)
                filters.dealStageId = parseInt(stage);
            if (assignedTo)
                filters.assignedTo = assignedTo;
    ...
    app.get('/api/crm/opportunities/:id', requireAdmin, async (req, res) => {
        try {
    ...
    // Update opportunity
    app.put('/api/crm/opportunities/:id', requireAdmin, async (req, res) => {
        try {
            const opportunityId = parseInt(req.params.id);
    ...
    app.get('/api/crm/deal-stages', requireAdmin, async (req, res) => {
        try {
    ...
    // Create deal stage
    app.post('/api/crm/deal-stages', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/tasks', requireAdmin, async (req, res) => {
        try {
            const { status, priority, assignedTo, leadId, opportunityId, taskType, page = 1, limit = 25 } = req.query;
            const filters = {};
            if (status)
                filters.status = status;
            if (priority)
            ...
            if (opportunityId)
                filters.opportunityId = parseInt(opportunityId);
            if (taskType)
    ...
    // Update task
    app.put('/api/crm/tasks/:id', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/customers', requireAdmin, async (req, res) => {
        try {
            const { page = 1, pageSize = 25, search = '', loyaltyTier = '', customerValue = '', verificationStatus = '', location = '', petType = '', sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
    ...
    app.get('/api/admin/customers/:id/wash-history', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
            ...
            const detailedHistory = washHistory.map((wash) => ({
                id: wash.id,
    ...
    app.get('/api/admin/customers/:id/communications', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
    ...
    app.post('/api/admin/customers/:id/communications', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
    ...
    app.get('/api/admin/customers/:id/pets', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
    ...
    app.post('/api/admin/customers/:id/pets', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
    ...
    app.patch('/api/admin/customers/:id/pets/:petId', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
    ...
    app.delete('/api/admin/customers/:id/pets/:petId', requireAdmin, async (req, res) => {
        try {
            const customerId = req.params.id;
            const petId = req.params.petId;
            const adminUser = req.adminUser;
    ...
    app.get('/api/crm/communications/stats', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/crm/communications/email-templates', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/communications/email-templates', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/communications/email-templates/:id', requireAdmin, async (req, res) => {
        try {
    ...
    app.patch('/api/crm/communications/email-templates/:id', requireAdmin, async (req, res) => {
        try {
            const templateId = parseInt(req.params.id);
    ...
    app.delete('/api/crm/communications/email-templates/:id', requireAdmin, async (req, res) => {
        try {
            const templateId = parseInt(req.params.id);
    ...
    app.get('/api/crm/communications/email-templates/default/:category', requireAdmin, async (req, res) => {
        try {
    ...
    // =================== SMS TEMPLATES ===================
    // Create SMS template
    app.post('/api/crm/communications/sms-templates', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/communications/sms-templates', requireAdmin, async (req, res) => {
        try {
            const { category, isActive, createdBy, page = 1, limit = 25 } = req.query;
            const offset = (parseInt(page) - 1) * parseInt(limit);
    ...
    app.get('/api/crm/communications/sms-templates/:id', requireAdmin, async (req, res) => {
        try {
            const templateId = parseInt(req.params.id);
    ...
    // Update SMS template
    app.patch('/api/crm/communications/sms-templates/:id', requireAdmin, async (req, res) => {
        try {
            const templateId = parseInt(req.params.id);
    ...
    app.delete('/api/crm/communications/sms-templates/:id', requireAdmin, async (req, res) => {
        try {
            const templateId = parseInt(req.params.id);
    ...
    app.get('/api/crm/communications/sms-templates/default/:category', requireAdmin, async (req, res) => {
        try {
            const category = req.params.category;
    ...
    app.post('/api/crm/communications/appointment-reminders', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/communications/appointment-reminders', requireAdmin, async (req, res) => {
        try {
            const { customerId, userId, status, appointmentDate, reminderType, isScheduled, page = 1, limit = 25 } = req.query;
            const offset = (parseInt(page) - 1) * parseInt(limit);
    ...
    app.get('/api/crm/communications/appointment-reminders/:id', requireAdmin, async (req, res) => {
        try {
    ...
    app.patch('/api/crm/communications/appointment-reminders/:id', requireAdmin, async (req, res) => {
        try {
            const reminderId = parseInt(req.params.id);
    ...
    app.post('/api/crm/communications/appointment-reminders/:id/cancel', requireAdmin, async (req, res) => {
        try {
            const reminderId = parseInt(req.params.id);
            const { reason } = req.body;
            const adminId = req.session?.adminId;
    ...
    app.get('/api/crm/communications/appointment-reminders/pending', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/communications/appointment-reminders/scheduled', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/crm/communications/logs', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/crm/communications/logs/communication/:communicationId', requireAdmin, async (req, res) => {
        try {
    ...
    // Track email open
    app.post('/api/crm/communications/logs/track/email-open', requireAdmin, async (req, res) => {
        try {
    ...
    // Track email click
    app.post('/api/crm/communications/logs/track/email-click', requireAdmin, async (req, res) => {
        try {
            const { communicationId, logId } = req.body;
    ...
    app.patch('/api/crm/communications/logs/:id', requireAdmin, async (req, res) => {
        try {
    ...
    // =================== BULK OPERATIONS ===================
    // Send bulk emails
    app.post('/api/crm/communications/bulk/send-email', requireAdmin, async (req, res) => {
        try {
    ...
    // Send bulk SMS
    app.post('/api/crm/communications/bulk/send-sms', requireAdmin, async (req, res) => {
        try {
            const { templateId, recipients, customData } = req.body;
            if (!templateId || !recipients || !Array.isArray(recipients)) {
                return res.status(400).json({ message: "Template ID and recipients array required" });
    ...
    app.get('/api/locations', apiLimiter, async (req, res) => {
        try {
            ...
            const stationsData = await db
                ...
                averageRating: sql `5.0`, // Default rating, will be calculated from reviews
                // Location data
                ...
                .where(and(eq(stations.isActive, true), eq(stations.status, 'operational')))
    ...
    app.get('/api/admin/platform-status', requireAdmin, adminLimiter, async (req, res) => {
        try {
    ...
    app.get('/api/admin/company-registration', requireAdmin, async (req, res) => {
        try {
    ...
    // SECURITY FIX: Added requireAdmin middleware (was RBAC bypass vulnerability)
    app.post('/api/admin/send-backend-invitation', requireAdmin, async (req, res) => {
        try {
            ...
            const success = await sendBackendTeamInvitation(recipientEmail, recipientName, 'Nir Hadad', 'Nir.H@PetWash.co.il', personalMessage, ['Nir.H@PetWash.co.il'] // CC to owner
            );
            if (success) {
                res.json({
                    success: true,
    ...
    app.post('/api/email/send-partner-invitation', async (req, res) => {
        try {
            ...
            const success = await sendPartnerInvitation(partnerEmail, partnerName, role, ccEmails);
            if (success) {
    ...
    app.post('/api/email/send-partner-invitation-hebrew', async (req, res) => {
        try {
            ...
            for (const partner of partners) {
                ...
                const success = await sendPartnerInvitationHebrew(email, name, role, ccEmails);
                results.push({
                    partner: name,
                    email,
                    success
    ...
    app.post('/api/email/welcome', async (req, res) => {
        try {
            ...
            const success = await sendWelcomeEmail(email, firstName, petName, petType, language || 'en');
            res.json({
                success,
                message: success
    ...
    const { validateBirthdayVoucher, getUserBirthdayVouchers, redeemBirthdayVoucher } = await import('./birthdayVoucher');
    const { BackgroundJobProcessor } = await import('./backgroundJobs');
    // Manual trigger for birthday processing (admin/testing only)
    // SECURITY FIX: Added requireAdmin middleware (was RBAC bypass vulnerability)
    app.post('/api/admin/trigger-birthdays', requireAdmin, async (req, res) => {
        try {
    ...
    // SECURITY FIX: Added requireAdmin middleware (was RBAC bypass vulnerability)
    app.post('/api/admin/trigger-observances', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/users/create-profile', async (req, res) => {
        try {
            ...
            const uid = decoded.uid;
            const { firstName, lastName, email, phone, dob, country, language, loyaltyProgram, reminders, marketing, pushNotifications, acceptedTerms, consentTimestamp } = req.body;
            // Validate required fields
    ...
    app.post('/api/internal/invitations', requireFirebaseAuth, async (req, res) => {
        try {
            ...
            const { email, firstName, lastName, phone, roleCode, // STAFF, CONTRACTOR, FRANCHISEE, MANAGER, etc.
            department, franchiseeId, stationIds, notes, expiresInDays = 7 // Default 7-day expiry
             } = req.body;
            if (!email || !roleCode) {
            ...
            await EmailService.sendInternalInvitation(email, firstName || '', roleCode, inviteUrl, adminEmail);
    ...
    app.post('/api/welcome-email', async (req, res) => {
        try {
            ...
            // Send welcome email
            const { EmailService } = await import('./emailService');
            const emailSent = await EmailService.sendWelcomeEmail(email, firstName || '', language || 'he');
            if (emailSent) {
    ...
    app.post('/api/admin/test/add-wash', requireAdmin, async (req, res) => {
        try {
            const { uid } = req.body;
            if (!uid) {
    ...
    // SECURITY FIX: Added requireAdmin middleware (defense in depth with Firebase token check)
    app.post('/api/admin/test/grant-coupon', requireAdmin, async (req, res) => {
        try {
            const { uid } = req.body;
            if (!uid) {
                return res.status(400).json({ success: false, error: 'UID required' });
    ...
    // SECURITY FIX: Added requireAdmin middleware (defense in depth with Firebase token check)
    app.post('/api/admin/test/send-sample-email', requireAdmin, async (req, res) => {
        try {
            ...
            if (emailType === 'welcome') {
                success = await EmailService.sendWelcomeEmail(adminEmail, 'Admin Test', language);
            }
    ...
    app.post('/api/admin/revenue/generate-report', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/backup/trigger', requireAdmin, async (req, res) => {
        try {
            const { BackgroundJobProcessor } = await import('./backgroundJobs');
    ...
    app.get('/api/admin/system-logs/activity', requireAdmin, async (req, res) => {
        try {
            const { limit = 100, offset = 0 } = req.query;
            const logs = await storage.getAdminActivityLogs(undefined, // all admins
            parseInt(limit));
            res.json({
                success: true,
                logs,
    ...
    app.get('/api/admin/system-logs/workflow', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/legal/compliance', requireAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/admin/legal/reviews', requireAdmin, async (req, res) => {
        try {
            ...
            if (documentType) {
                query = query.where(eq(legalComplianceReviews.documentType, documentType));
            }
            if (status) {
                query = query.where(eq(legalComplianceReviews.reviewStatus, status));
            }
    ...
    app.post('/api/admin/legal/reviews', requireAdmin, async (req, res) => {
        try {
    ...
    app.patch('/api/admin/legal/reviews/:id', requireAdmin, async (req, res) => {
        try {
            const { legalComplianceReviews } = await import('@shared/schema');
            const { db } = await import('../db');
    ...
    app.get('/api/admin/legal/versions', requireAdmin, async (req, res) => {
        try {
            ...
            if (documentType) {
                query = query.where(eq(legalDocumentVersions.documentType, documentType));
            }
    ...
    app.post('/api/admin/legal/versions', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/admin/legal/remind', requireAdmin, async (req, res) => {
        try {
    ...
    app.post('/api/track/interaction', async (req, res) => {
        try {
            ...
            if (interactionData.inputValue && (interactionData.elementType === 'password' ||
    ...
    app.get('/api/admin/interactions', requireAdmin, async (req, res) => {
        try {
            const { userInteractionLogs } = await import('@shared/schema');
            const { db } = await import('../db');
            const { desc, eq, and, gte, lte } = await import('drizzle-orm');
            const { userId, sessionId, interactionType, page, startDate, endDate, limit = 1000, offset = 0 } = req.query;
            let conditions = [];
            if (userId)
                conditions.push(eq(userInteractionLogs.userId, userId));
            if (sessionId)
                conditions.push(eq(userInteractionLogs.sessionId, sessionId));
            if (interactionType)
                conditions.push(eq(userInteractionLogs.interactionType, interactionType));
            if (page)
                conditions.push(eq(userInteractionLogs.page, page));
            if (startDate)
                conditions.push(gte(userInteractionLogs.timestamp, new Date(startDate)));
            if (endDate)
                conditions.push(lte(userInteractionLogs.timestamp, new Date(endDate)));
    ...
    app.get('/api/admin/interactions/analytics', requireAdmin, async (req, res) => {
        try {
            const { userInteractionLogs } = await import('@shared/schema');
            const { db } = await import('../db');
            ...
            const pageVisits = await db
                .select({
                page: userInteractionLogs.page,
                count: sql `count(distinct ${userInteractionLogs.sessionId})::int`,
            })
                .from(userInteractionLogs)
                .where(gte(userInteractionLogs.timestamp, startDate))
    ...
    app.get('/api/subscription-products', async (req, res) => {
        try {
            ...
            const conditions = [];
            if (category)
                conditions.push(eq(subscriptionProducts.category, category));
            if (petType)
                conditions.push(or(eq(subscriptionProducts.petType, petType), eq(subscriptionProducts.petType, 'both')));
            if (ageGroup)
                conditions.push(or(eq(subscriptionProducts.ageGroup, ageGroup), eq(subscriptionProducts.ageGroup, 'all')));
            if (sizeGroup)
                conditions.push(or(eq(subscriptionProducts.sizeGroup, sizeGroup), eq(subscriptionProducts.sizeGroup, 'all')));
            if (conditions.length > 0) {
                query = db.select().from(subscriptionProducts).where(and(eq(subscriptionProducts.isActive, true), ...conditions));
            }
    ...
    app.post('/api/subscriptions', requireAuth, async (req, res) => {
        try {
            ...
            const boxType = await db
                .select()
                .from(subscriptionBoxTypes)
                .where(and(eq(subscriptionBoxTypes.id, validatedData.boxTypeId), eq(subscriptionBoxTypes.isActive, true)))
                .limit(1);
            if (!boxType.length) {
    ...
    app.get('/api/subscriptions/:id', requireAuth, async (req, res) => {
        try {
            ...
            const [result] = await db
                ...
                .from(customerSubscriptions)
                .leftJoin(subscriptionBoxTypes, eq(customerSubscriptions.boxTypeId, subscriptionBoxTypes.id))
                .where(and(eq(customerSubscriptions.id, subscriptionId), eq(customerSubscriptions.userId, userId)))
                .limit(1);
            if (!result) {
     ...
     * Requires authentication
     */
    app.post('/api/subscriptions/:id/ai-recommendations', requireAuth, async (req, res) => {
        try {
            const subscriptionId = parseInt(req.params.id);
            const userId = req.user.uid;
...
Select exactly ${boxType.itemCount} products that match the pet's profile, age, size, and dietary needs. Prioritize variety across categories (food, treats, toys, etc.).`;
     ...
     * Requires authentication
     */
    app.get('/api/subscriptions/:id/shipments', requireAuth, async (req, res) => {
        try {
            const subscriptionId = parseInt(req.params.id);
            const userId = req.user.uid;
            // Verify subscription belongs to user
  ...
  <div class="container">
    ...
    app.post('/api/pet-care/schedule-wash', async (req, res) => {
        try {
            ...
            const targetDateIndex = weatherData.daily.time.findIndex((d) => d === date);
            let weatherForecast;
            let dataSnapshot = {};
    ...
    app.get('/api/loyalty/user-profile', async (req, res) => {
        try {
            ...
            const washesCountResult = await db
                ...
                .from(washHistory)
                .where(and(eq(washHistory.userId, userId), eq(washHistory.status, 'completed')));
            const totalWashes = washesCountResult[0]?.count || 0;
            // Calculate tier based on REAL wash count (using loyalty tier logic)
            let tier = 'new';
            if (totalWashes >= 25)
                tier = 'platinum';
            else if (totalWashes >= 10)
                tier = 'gold';
            else if (totalWashes >= 3)
                tier = 'silver';
    ...
    app.get('/api/greeting/personalized', requireAuth, async (req, res) => {
        try {
            ...
            const greetingUserData = {
                ...
                preferredLanguage: (userData.preferredLanguage || 'he'),
