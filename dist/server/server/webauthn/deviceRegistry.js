 ...
 */
export function calculateDeviceTrustScore(credential, factors) {
    let score = 50; // Start at neutral
    // Age factor (0-20 points): older devices are more trusted
    if (factors.ageInDays) {
        if (factors.ageInDays > 90)
            score += 20;
        else if (factors.ageInDays > 30)
            score += 15;
        else if (factors.ageInDays > 7)
            score += 10;
        else
            score += 5;
    }
    ...
    if (factors.usageFrequency !== undefined) {
        if (factors.usageFrequency > 50)
            score += 25;
        else if (factors.usageFrequency > 20)
            score += 20;
        else if (factors.usageFrequency > 5)
            score += 10;
        else
            score += 5;
    }
    ...
    if (factors.failureRate !== undefined) {
        if (factors.failureRate === 0)
            score += 10;
        else if (factors.failureRate < 0.05)
            score += 5;
        else if (factors.failureRate < 0.1)
            score += 0;
        else if (factors.failureRate < 0.2)
            score -= 10;
        else
            score -= 30;
    }
    // Backup status (0-10 points): backed up devices are slightly more trusted
    if (factors.isBackedUp)
        score += 10;
    // Attestation validation (0-15 points)
    if (factors.hasValidAttestation)
        score += 15;
    // Consistent location (0-10 points)
    if (factors.consistentLocation)
        score += 10;
    // Consistent user agent (0-10 points)
    if (factors.consistentUserAgent)
        score += 10;
 ...
 */
export function getDeviceIcon(credential) {
 ...
 */
export async function registerDevice(uid, isAdmin, credentialData, userAgent, ipAddress, origin) {
 ...
 */
export async function updateDeviceOnAuth(uid, isAdmin, credId, newCounter, ipAddress, userAgent) {
    const collectionPath = isAdmin ? 'employees' : 'users';
    const deviceRef = db
        .collection(collectionPath)
        .doc(uid)
 ...
 */
export async function recordAuthFailure(uid, isAdmin, credId, reason) {
    const collectionPath = isAdmin ? 'employees' : 'users';
    const deviceRef = db
        .collection(collectionPath)
        .doc(uid)
 ...
 * Revoke a device
 */
export async function revokeDevice(uid, isAdmin, credId, reason, revokedBy, reasonText) {
    const collectionPath = isAdmin ? 'employees' : 'users';
    const deviceRef = db
        .collection(collectionPath)
        .doc(uid)
 ...
 */
export async function getUserDevices(uid, isAdmin) {
    const collectionPath = isAdmin ? 'employees' : 'users';
 ...
 * Rename a device
 */
export async function renameDevice(uid, isAdmin, credId, newName) {
    const collectionPath = isAdmin ? 'employees' : 'users';
    await db
        .collection(collectionPath)
        .doc(uid)
 ...
 * Set device icon
 */
export async function setDeviceIcon(uid, isAdmin, credId, icon) {
    const collectionPath = isAdmin ? 'employees' : 'users';
    await db
        .collection(collectionPath)
        .doc(uid)
 ...
 */
export function checkReAuthRequired(lastAuthAt, gracePeriodMinutes = 5) {
    if (!lastAuthAt)
        return true;
    const now = Date.now();
 ...
 */
export async function getDeviceUsagePattern(uid, isAdmin, credId) {
    const collectionPath = isAdmin ? 'employees' : 'users';
    ...
    const device = deviceDoc.data();
    // Calculate metrics
    const ageInDays = Math.floor((Date.now() - device.createdAt.toMillis()) / (1000 * 60 * 60 * 24));
