 ...
 */
export function generateWebAuthnCsrfToken() {
 ...
 */
export function ensureWebAuthnSession(req, res, next) {
 ...
 */
export function setWebAuthnCsrfToken(req, res, next) {
 ...
 */
export function verifyWebAuthnCsrfToken(req, res, next) {
    const tokenFromHeader = req.headers['x-webauthn-csrf-token'];
    ...
    if (!crypto.timingSafeEqual(Buffer.from(providedToken), Buffer.from(sessionToken))) {
