...
import { webauthnConfig, getRpId, getExpectedOrigin, isOriginAllowed, storeChallengeInCookie, retrieveChallengeFromCookie, clearChallengeFromCookie, getAuthenticatorSelection, getSupportedAlgorithms, isAttestationFormatAllowed, validateAppleAttestation, validateAndroidAttestation } from './config';
import { registerDevice, updateDeviceOnAuth, recordAuthFailure, getUserDevices, renameDevice as registryRenameDevice, setDeviceIcon as registrySetDeviceIcon, revokeDevice, checkReAuthRequired, logAuthEvent } from './deviceRegistry';
import { webauthnMessages, getLanguage, bilingualError } from '../lib/i18n';
/**
 * Generate registration options for a user (customer or employee)
 */
export async function generateRegistrationOptionsForUser(uid, email, isAdmin, req, res) {
    try {
 ...
 */
export async function verifyAndStoreRegistration(response, req, res) {
    try {
        const lang = getLanguage(req);
        const rpId = getRpId(req);
        const origin = getExpectedOrigin(req);
        ...
        const registeredCredential = await registerDevice(challengeData.uid, isAdmin, {
 ...
 */
export async function generateAuthenticationOptionsForEmail(email, req, res) {
    try {
        const lang = getLanguage(req);
        const rpId = getRpId(req);
        const origin = getExpectedOrigin(req);
        ...
        if (!usersSnapshot.empty) {
            ...
            const credsSnapshot = await db
                ...
                .get();
            credentials = credsSnapshot.docs.map((doc) => doc.data());
        }
        ...
        const trustedCredentials = credentials.filter(cred => cred.trustScore >= webauthnConfig.deviceTrustThreshold);
 ...
 */
export async function verifyAuthenticationAndGetUser(response, req, res) {
    try {
        const lang = getLanguage(req);
        const rpId = getRpId(req);
        const origin = getExpectedOrigin(req);
        ...
        await updateDeviceOnAuth(uid, isAdmin, credentialId, verification.authenticationInfo.newCounter, ipAddress, userAgent);
        // Clear challenge cookie
        clearChallengeFromCookie(res);
 ...
 */
export async function getUserCredentials(uid, isAdmin) {
    return getUserDevices(uid, isAdmin);
}
/**
 * Delete a credential
 */
export async function deleteUserCredential(uid, credentialId, isAdmin) {
 ...
 * Rename a credential
 */
export async function renameUserCredential(uid, credentialId, newName, isAdmin) {
    await registryRenameDevice(uid, isAdmin, credentialId, newName);
}
/**
 * Set credential icon
 */
export async function setUserCredentialIcon(uid, credentialId, icon, isAdmin) {
    await registrySetDeviceIcon(uid, isAdmin, credentialId, icon);
}
/**
 * Check if re-authentication is required for sensitive action
 */
export function requireReAuth(lastAuthAt) {
