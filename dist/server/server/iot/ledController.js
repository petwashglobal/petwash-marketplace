...
(function (LedState) {
    LedState["OFF"] = "OFF";
    LedState["AVAILABLE"] = "AVAILABLE";
    LedState["DRIVER_ON_ROUTE"] = "DRIVER_ON_ROUTE";
    LedState["IN_USE"] = "IN_USE";
    LedState["MAINTENANCE"] = "MAINTENANCE";
    LedState["ERROR"] = "ERROR";
    LedState["PAUSED"] = "PAUSED";
})(LedState || (LedState = {}));
/**
 * ===================================================================
 ...
 * ===================================================================
 */
function mapStateToVisual(state) {
    switch (state) {
...
function decideLedState(context) {
...
function buildLedStatus(stationId, state, source, prev, reason) {
...
export class K9000HardwareBridge {
    async sendLedCommand(stationId, status) {
        try {
...
export class LedService {
    ...
    async setManual(stationId, requestedState, overrideMinutes, actorEmail) {
        try {
            const current = await this.getStatus(stationId);
            const expiresAt = overrideMinutes && overrideMinutes > 0
    ...
    manualExpired(status) {
        if (!status.manualOverride || !status.manualExpiresAt)
            return false;
        return new Date(status.manualExpiresAt).getTime() < Date.now();
    }
    async applyAutomation(stationId, ctx) {
        try {
            const current = await this.getStatus(stationId);
            ...
            const fullCtx = { ...ctx, lastLed: current };
    ...
    async markError(stationId, reason) {
        try {
            const current = await this.getStatus(stationId);
    ...
    async markOffline(stationId, reason = 'Station offline') {
        try {
            const current = await this.getStatus(stationId);
    ...
    async saveStatus(status, previous) {
        const data = {
    ...
    async logCommandHistory(stationId, commandType, previous, current, triggeredBy) {
        try {
...
export function createLedRouter(deps) {
    ...
    router.get('/led', requireAuth, async (req, res) => {
        try {
    ...
    router.get('/stations/:stationId/led', requireAuth, async (req, res) => {
        try {
    ...
    router.post('/stations/:stationId/led/manual', requireAdmin, async (req, res) => {
        try {
            const stationId = parseInt(req.params.stationId, 10);
            const { state, overrideMinutes } = req.body;
            if (!state || !Object.values(LedState).includes(state)) {
                return res.status(400).json({ error: 'Invalid or missing LED state' });
            }
            const actorEmail = req.user?.email || 'unknown@petwash.co.il';
            const status = await ledService.setManual(stationId, state, typeof overrideMinutes === 'number' ? overrideMinutes : null, actorEmail);
            res.json(status);
        }
        catch (err) {
    ...
    router.post('/stations/:stationId/led/manual/clear', requireAdmin, async (req, res) => {
        try {
            const stationId = parseInt(req.params.stationId, 10);
 ...
 */
export function wireLedAutomation(eventBus) {
    const hardwareBridge = new K9000HardwareBridge();
    const ledService = new LedService(hardwareBridge);
    // Wash session events
    eventBus.subscribe('wash.started', async (event) => {
        try {
    ...
    eventBus.subscribe('wash.completed', async (event) => {
        try {
            const stationId = event.data?.stationId;
            if (!stationId)
                return;
    ...
    eventBus.subscribe('transport.assigned', async (event) => {
        try {
            const stationId = event.data?.stationId;
            if (!stationId)
                return;
    ...
    eventBus.subscribe('transport.pickup', async (event) => {
        try {
            const stationId = event.data?.stationId;
            if (!stationId)
                return;
    ...
    eventBus.subscribe('station.offline', async (event) => {
        try {
            const stationId = event.data?.stationId;
            if (!stationId)
                return;
    ...
    eventBus.subscribe('station.error', async (event) => {
        try {
            const stationId = event.data?.stationId;
