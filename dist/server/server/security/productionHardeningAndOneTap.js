...
function signOneTapToken(payload, ttlSec = 600) {
    if (!MOBILE_LINK_SECRET)
        throw new Error("MOBILE_LINK_SECRET not set");
...
function verifyOneTapToken(token) {
    if (!MOBILE_LINK_SECRET)
        throw new Error("MOBILE_LINK_SECRET not set");
...
export function applySecurityAndOneTap(ctx) {
    ...
    */
    app.post("/api/ops/one-tap/create", requireAdmin, async (req, res) => {
        try {
            if (!BASE_URL)
                return res.status(400).json({ error: "BASE_URL not configured" });
            const { uid, email, redirect } = (req.body || {});
            let targetUid = uid;
            if (!targetUid && email) {
    ...
    */
    app.get("/ops/one-tap", async (req, res) => {
        try {
    ...
    */
    app.get("/ops/one-tap-employee", async (req, res) => {
        try {
