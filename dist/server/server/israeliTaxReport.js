...
export class IsraeliTaxReportService {
     ...
     */
    static generateBilingualTaxReport(data) {
        const formatCurrency = (amount) => `₪${amount.toFixed(2)}`;
        const formatDate = (date) => date.toLocaleDateString('he-IL');
        const formatDateEn = (date) => date.toLocaleDateString('en-US');
        return `
<!DOCTYPE html>
<html lang="he">
<head>
...
<body>
     ...
     */
    static async sendTaxReportToAccountant(data, recipientEmail = 'Support@PetWash.co.il') {
        try {
            if (!process.env.SENDGRID_API_KEY) {
