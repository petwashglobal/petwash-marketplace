...
export async function beginRegistration(user) {
    ...
    `);
        const existingCreds = credResult.rows[0]?.credentials || [];
 ...
 */
export async function completeRegistration(userId, registrationResponse) {
    try {
 ...
 */
export async function beginLogin(userEmail) {
    try {
 ...
 * Called after user successfully scans Face ID / fingerprint
 */
export async function completeLogin(userId, authenticationResponse) {
    try {
    ...
    `);
        ...
        const credential = userData.credentials?.find((c) => c.credId === credId);
        if (!credential) {
        ...
        const updatedCredentials = userData.credentials.map((c) => c.credId === credential.credId
 ...
 */
export async function removeCredential(userId, credId) {
    try {
        const result = await db.execute(sql `
      SELECT credentials
      FROM users
