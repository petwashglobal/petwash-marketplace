 ...
 */
function matchesDateRule(rule, date) {
 ...
 */
function generateVoucherCode(template, uid) {
 ...
 */
function generateIdempotencyKey(template, uid) {
    const year = new Date().getFullYear();
    return template
        .replace('{UID}', uid)
        .replace('{YYYY}', year.toString());
 ...
 */
async function sendObservanceMessage(uid, event, userLocale, voucherCode) {
    try {
 ...
 */
async function createObservanceVoucher(uid, code, event) {
    try {
        const now = new Date();
 ...
 */
async function isAlreadySent(uid, idempotencyKey) {
    try {
 ...
 */
async function markAsSent(uid, idempotencyKey) {
    try {
        await adminDb
            .collection('observance_tracking')
            .doc(`${uid}_${idempotencyKey}`)
 ...
 */
export async function processObservancesForLocale(locale) {
    let sent = 0;
    let errors = 0;
    let skipped = 0;
    try {
        ...
        const matchingEvents = config.events.filter(event => event.active && matchesDateRule(event.rule, today));
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            for (const event of matchingEvents) {
                ...
                // Send inbox message
                const messageSent = await sendObservanceMessage(uid, event, userLocale, voucherCode);
                if (messageSent) {
 ...
 */
export async function processAllObservances() {
