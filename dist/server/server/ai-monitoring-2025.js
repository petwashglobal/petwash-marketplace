 ...
 */
export function trackRequestPerformance(req, responseTime, statusCode) {
 ...
 */
async function detectAnomalies(key, current) {
 ...
 */
async function reportAnomaly(anomaly) {
 ...
 * Self-healing system
 */
async function attemptAutoFix(issueType, context) {
...
export async function runHealthCheck() {
    ...
    const status = criticalFailures.length > 0 ? 'critical' :
...
// EXPORTS
// ============================================
export function getAnomaliesReport() {
    ...
    for (const [_, history] of Array.from(performanceHistory.entries())) {
        ...
        if (recent.length > 0) {
            const avg = {
                ...
                avgResponseTime: average(recent.map((m) => m.avgResponseTime)),
                p95ResponseTime: percentile(recent.map((m) => m.avgResponseTime), 95),
                p99ResponseTime: percentile(recent.map((m) => m.avgResponseTime), 99),
                errorRate: average(recent.map((m) => m.errorRate)),
                requestsPerMinute: average(recent.map((m) => m.requestsPerMinute)),
                timestamp: new Date()
            };
            report.push(avg);
        }
