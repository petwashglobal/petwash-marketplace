import { users, customers, customerPets, washPackages, eVouchers, eVoucherRedemptions, washHistory, coupons, userCoupons, adminUsers, adminActivityLogs, inventoryItems, hrDocuments, loyaltyAnalytics, smartWashReceipts, 
// CRM tables
crmLeads, crmCommunications, crmDealStages, crmOpportunities, crmTasks, crmActivities, crmCampaigns, crmCampaignTargets, crmCampaignMetrics, crmCustomerSegments, crmCustomerSegmentMembers, crmCustomerInsights, crmTouchpoints, 
// Communication Center tables
crmEmailTemplates, crmSmsTemplates, crmAppointmentReminders, crmCommunicationLogs, 
// Enterprise Corporate tables
jvPartners, jvContracts, suppliers, supplierContracts, supplierPayments, supplierQualityScores, stationRegistry, 
// Enterprise HR tables
hrEmployees, hrPayroll, hrTimeTracking, hrPerformanceReviews, hrRecruitment, hrJobApplications, } from "@shared/schema";
import { opsTasksTable, opsIncidents, opsSlaTracking, } from "@shared/schema-operations";
import { logisticsWarehouses, logisticsInventory, logisticsFulfillmentOrders, } from "@shared/schema-logistics";
import { accountsPayable, accountsReceivable, generalLedger, taxReturns, taxPayments, taxAuditLogs, } from "@shared/schema-finance";
import { policyDocuments, policyAcknowledgments, complianceCertifications, } from "@shared/schema-policy";
import { franchisees, franchiseRoyaltyPayments, } from "@shared/schema-franchise";
import { db } from "./db";
...
export class DatabaseStorage {
    ...
    async unsubscribeAll(email) {
        try {
            ...
            if (user) {
                const existingSuppression = user.suppressionList || {};
                const existingPrefs = user.communicationPreferences || {};
                ...
                await db
                    .update(users)
                    .set({
                    suppressionList: mergedSuppression,
            ...
            if (customer) {
                const existingSuppression = customer.suppressionList || {};
                const existingPrefs = customer.communicationPreferences || {};
                const mergedSuppression = { ...existingSuppression };
                allChannels.forEach(channel => {
                    mergedSuppression[channel] = true;
                });
     ...
     */
    async updateChannelSuppression(email, channels, suppress = true) {
        try {
            const [user] = await db.select().from(users).where(eq(users.email, email)).limit(1);
            const [customer] = await db.select().from(customers).where(eq(customers.email, email)).limit(1);
            if (!user && !customer) {
            ...
            if (user) {
                ...
                if (!suppress) {
                    ...
                    allChannelsNowEnabled = ['email', 'sms', 'whatsapp', 'push'].every(ch => mergedSuppression[ch] === false || !mergedSuppression[ch]);
                }
     ...
     */
    async addToSuppressionList(email, channels = ['all']) {
        if (channels.includes('all')) {
            return this.unsubscribeAll(email);
        }
        else {
            return this.updateChannelSuppression(email, channels, true);
        }
    }
    ...
    async getCustomersWithFilters(filters) {
        const { searchTerm, loyaltyTier, verificationStatus, location, petType, sortBy = 'createdAt', sortOrder = 'desc', limit = 25, offset = 0 } = filters;
        let query = db.select().from(customers);
        let countQuery = db.select({ count: sql `count(*)` }).from(customers);
        // Apply filters
        const conditions = [];
        if (searchTerm) {
            const searchCondition = or(like(customers.firstName, `%${searchTerm}%`), like(customers.lastName, `%${searchTerm}%`), like(customers.email, `%${searchTerm}%`));
    ...
    async updateCustomerPet(id, updates) {
        const [pet] = await db
            .update(customerPets)
            .set({ ...updates, updatedAt: new Date() })
    ...
    async findVoucherByHash(codeHash) {
    ...
    async updateEVoucher(id, updates) {
    ...
    async redeemVoucher(data) {
        ...
        const existing = await db
            ...
            .where(and(eq(eVoucherRedemptions.voucherId, data.voucherId), eq(eVoucherRedemptions.nayaxSessionId, data.nayaxSessionId)));
        ...
        try {
            const result = await db.execute(sql `SELECT redeem_voucher_atomic(${data.voucherId}::uuid, ${data.amount}::numeric) as success`);
    ...
    async getMyVouchers(ownerUid, options) {
    ...
    async getVoucherByIdForOwner(voucherId, ownerUid) {
    ...
    async createEVoucherRedemption(redemption) {
        ...
        try {
            ...
            const result = await db.transaction(async (tx) => {
                ...
                const updateResult = await tx
                    ...
                    .where(and(eq(eVouchers.id, existingVoucher.id), eq(eVouchers.code, data.code), eq(eVouchers.isActive, true), sql `${eVouchers.remainingWashes} >= ${data.washesRequested}`, // Atomic check
    ...
    async createGiftCard(giftCard) {
        return await this.createEVoucher(giftCard);
    }
    async getGiftCard(codeHash) {
        return await this.getEVoucherByCodeHash(codeHash);
    }
    async redeemGiftCard(codeHash, userId) {
    ...
    async getCustomerWashHistory(customerId) {
    ...
    async getCommunicationLogByMessageId(messageId) {
        const [log] = await db
            .select()
    ...
    async getCommunicationLogsByEmail(email) {
        ...
        const communications = await db
            ...
            .where(or(user ? eq(crmCommunications.userId, user.id) : sql `false`, customer ? eq(crmCommunications.customerId, customer.id) : sql `false`));
    ...
    // Admin activity logs
    async createAdminActivityLog(logData) {
        const [log] = await db.insert(adminActivityLogs).values(logData).returning();
        return log;
    }
    async getAdminActivityLogs(adminId, limit = 100) {
    ...
    async getInventoryItems(location) {
    ...
    async getLowStockItems(location) {
        const query = db.select().from(inventoryItems);
        if (location) {
            query.where(and(eq(inventoryItems.location, location), eq(inventoryItems.currentStock, inventoryItems.minStock)));
        }
        else {
    ...
    async updateLoyaltyAnalytics(userId, updates) {
    ...
    // Smart Wash Receipts
    async createSmartReceipt(receipt) {
    ...
    async getSmartReceiptByTransactionId(transactionId) {
    ...
    async getUserSmartReceipts(userId, limit = 10) {
        return await db
            .select()
            .from(smartWashReceipts)
    ...
    async updateSmartReceipt(transactionId, updates) {
    ...
    async convertLeadToCustomer(leadId, customerId) {
        const [lead] = await db
            .update(crmLeads)
            .set({
    ...
    async getEntityCommunications(entityType, entityId) {
        let query = db.select().from(crmCommunications);
        switch (entityType) {
            case 'lead':
                query = query.where(eq(crmCommunications.leadId, entityId));
                break;
            case 'customer':
                query = query.where(eq(crmCommunications.customerId, entityId));
                break;
            case 'user':
                query = query.where(eq(crmCommunications.userId, entityId));
                break;
        }
        return await query.orderBy(desc(crmCommunications.createdAt));
    }
    // Deal Stages
    async createDealStage(stageData) {
    ...
    async updateDealStage(id, updates) {
        const [stage] = await db
    ...
    async getOpportunitiesByAssignee(assigneeId) {
        return await db
            .select()
    ...
    async getForecastData(assigneeId, timeframe = 'this_month') {
    ...
    async completeTask(id, completedBy, outcome, notes) {
        const [task] = await db
            .update(crmTasks)
            .set({
            status: 'completed',
    ...
    async getUpcomingTasks(assigneeId, days = 7) {
        const now = new Date();
    ...
    async getEntityActivities(entityType, entityId) {
        ...
        switch (entityType) {
            case 'lead':
                query = query.where(eq(crmActivities.leadId, entityId));
                break;
            case 'customer':
                query = query.where(eq(crmActivities.customerId, entityId));
                break;
            case 'user':
                query = query.where(eq(crmActivities.userId, entityId));
                break;
            case 'opportunity':
                query = query.where(eq(crmActivities.opportunityId, entityId));
                break;
        }
    ...
    // Campaign Metrics
    async createCampaignMetrics(metricsData) {
    ...
    async updateCampaignMetrics(campaignId, updates) {
        const [metrics] = await db
    ...
    async updateCustomerSegment(id, updates) {
        const [segment] = await db
    ...
    async refreshSegmentMembers(segmentId) {
    ...
    async addCustomerToSegment(segmentId, customerId, userId) {
    ...
    async removeCustomerFromSegment(segmentId, customerId, userId) {
        let whereCondition;
        if (customerId) {
            whereCondition = and(eq(crmCustomerSegmentMembers.segmentId, segmentId), eq(crmCustomerSegmentMembers.customerId, customerId));
        }
        else if (userId) {
            whereCondition = and(eq(crmCustomerSegmentMembers.segmentId, segmentId), eq(crmCustomerSegmentMembers.userId, userId));
        }
        else {
            return false;
        }
    ...
    async getSegmentMembers(segmentId) {
        return await db
            ...
            .where(and(eq(crmCustomerSegmentMembers.segmentId, segmentId), eq(crmCustomerSegmentMembers.isActive, true)))
            .orderBy(desc(crmCustomerSegmentMembers.addedAt));
    }
    async getCustomerSegments(customerId, userId) {
        ...
        const results = await db
            ...
            .from(crmCustomerSegments)
            .innerJoin(crmCustomerSegmentMembers, eq(crmCustomerSegments.id, crmCustomerSegmentMembers.segmentId))
            .where(and(whereCondition, eq(crmCustomerSegmentMembers.isActive, true)));
        return results;
    }
    // Customer Insights
    async createCustomerInsights(insightsData) {
    ...
    async getCustomerInsights(customerId, userId) {
        let whereCondition;
        if (customerId) {
    ...
    async updateCustomerInsights(customerId, userId, updates) {
        let whereCondition;
        if (customerId) {
            whereCondition = eq(crmCustomerInsights.customerId, customerId);
        }
    ...
    async calculateCustomerInsights(customerId, userId) {
        if (!customerId && !userId) {
            throw new Error('Either customerId or userId must be provided');
        }
        // Get customer data
    ...
    async getCustomersAtRisk(riskLevel) {
    ...
    async getCustomerJourney(customerId, userId) {
        let whereCondition;
        if (customerId) {
    ...
    async getSessionTouchpoints(sessionId) {
        return await db
            .select()
            .from(crmTouchpoints)
    ...
    async getDefaultEmailTemplate(category) {
        const [template] = await db
            .select()
            .from(crmEmailTemplates)
            .where(and(eq(crmEmailTemplates.category, category), eq(crmEmailTemplates.isDefault, true), eq(crmEmailTemplates.isActive, true)));
        return template;
    }
    // SMS Templates
    ...
    async getDefaultSmsTemplate(category) {
        const [template] = await db
            .select()
            .from(crmSmsTemplates)
            .where(and(eq(crmSmsTemplates.category, category), eq(crmSmsTemplates.isDefault, true), eq(crmSmsTemplates.isActive, true)));
        return template;
    }
    ...
    async getAppointmentReminder(id) {
        const [reminder] = await db
            .select()
    ...
    async updateAppointmentReminder(id, updates) {
        const [reminder] = await db
    ...
    async cancelAppointmentReminder(id, cancelledBy, reason) {
        const [reminder] = await db
            .update(crmAppointmentReminders)
            .set({
            isCancelled: true,
    ...
    async getScheduledReminders(cutoffDate) {
        const cutoff = cutoffDate || new Date();
        return await db
            .select()
            .from(crmAppointmentReminders)
            .where(and(eq(crmAppointmentReminders.status, 'scheduled'), eq(crmAppointmentReminders.isScheduled, true), eq(crmAppointmentReminders.isCancelled, false), lte(crmAppointmentReminders.scheduledSendTime, cutoff)))
    ...
    async getPendingReminders() {
        ...
        return await db
            .select()
            .from(crmAppointmentReminders)
            .where(and(eq(crmAppointmentReminders.status, 'scheduled'), eq(crmAppointmentReminders.isScheduled, true), eq(crmAppointmentReminders.isCancelled, false), lte(crmAppointmentReminders.scheduledSendTime, now)))
            .orderBy(crmAppointmentReminders.scheduledSendTime);
    }
    // Communication Logs
    ...
    async getCommunicationLogsByCommunication(communicationId) {
        return await db
            .select()
            .from(crmCommunicationLogs)
    ...
    async trackEmailOpen(communicationId, logId) {
    ...
    async trackEmailClick(communicationId, logId) {
        const targetLogId = logId || (await this.getCommunicationLogsByCommunication(communicationId))?.[0]?.id;
        if (!targetLogId) {
            throw new Error('Communication log not found');
        }
    ...
    async getCommunicationStats(dateFrom, dateTo) {
        ...
        const totalCommunications = await db
            ...
            .from(crmCommunications)
            .where(and(gte(crmCommunications.createdAt, from), lte(crmCommunications.createdAt, to)));
        // Email and SMS communications
        const emailsSent = await db
            .select({ count: sql `count(*)` })
            .from(crmCommunications)
            .where(and(eq(crmCommunications.communicationType, 'email'), gte(crmCommunications.createdAt, from), lte(crmCommunications.createdAt, to)));
        const smsSent = await db
            .select({ count: sql `count(*)` })
            .from(crmCommunications)
            .where(and(eq(crmCommunications.communicationType, 'sms'), gte(crmCommunications.createdAt, from), lte(crmCommunications.createdAt, to)));
        // Email engagement stats
        const emailsOpened = await db
            .select({ count: sql `count(*)` })
            .from(crmCommunicationLogs)
            .where(and(eq(crmCommunicationLogs.opened, true), gte(crmCommunicationLogs.createdAt, from), lte(crmCommunicationLogs.createdAt, to)));
        const emailsClicked = await db
            .select({ count: sql `count(*)` })
            .from(crmCommunicationLogs)
            .where(and(eq(crmCommunicationLogs.clicked, true), gte(crmCommunicationLogs.createdAt, from), lte(crmCommunicationLogs.createdAt, to)));
        // SMS delivery stats
        const smsDelivered = await db
            .select({ count: sql `count(*)` })
            .from(crmCommunicationLogs)
            .where(and(eq(crmCommunicationLogs.deliveryStatus, 'delivered'), gte(crmCommunicationLogs.createdAt, from), lte(crmCommunicationLogs.createdAt, to)));
        // Template usage
        const emailTemplatesUsed = await db
            .select({ count: sql `count(*)` })
            .from(crmEmailTemplates)
            .where(and(sql `${crmEmailTemplates.timesUsed} > 0`, gte(crmEmailTemplates.lastUsed || crmEmailTemplates.createdAt, from), lte(crmEmailTemplates.lastUsed || crmEmailTemplates.createdAt, to)));
        const smsTemplatesUsed = await db
            .select({ count: sql `count(*)` })
            .from(crmSmsTemplates)
            .where(and(sql `${crmSmsTemplates.timesUsed} > 0`, gte(crmSmsTemplates.lastUsed || crmSmsTemplates.createdAt, from), lte(crmSmsTemplates.lastUsed || crmSmsTemplates.createdAt, to)));
        // Reminder stats
        const activeReminders = await db
            .select({ count: sql `count(*)` })
            .from(crmAppointmentReminders)
            .where(and(eq(crmAppointmentReminders.status, 'scheduled'), eq(crmAppointmentReminders.isCancelled, false)));
        const completedReminders = await db
            .select({ count: sql `count(*)` })
            .from(crmAppointmentReminders)
            .where(and(or(eq(crmAppointmentReminders.status, 'sent'), eq(crmAppointmentReminders.status, 'delivered')), gte(crmAppointmentReminders.createdAt, from), lte(crmAppointmentReminders.createdAt, to)));
        return {
    ...
    async createBoardMember(data) { throw new Error("Not implemented"); }
    async updateBoardMember(id, updates) { throw new Error("Not implemented"); }
    // Board Meetings - Stub implementations
    async getAllBoardMeetings() { return []; }
    async getUpcomingBoardMeetings() { return []; }
    async createBoardMeeting(data) { throw new Error("Not implemented"); }
    async updateBoardMeeting(id, updates) { throw new Error("Not implemented"); }
    async addMeetingAttendee(meetingId, memberId, attended) { throw new Error("Not implemented"); }
    ...
    async getSupplierQualityScores(supplierId) {
    ...
    async createSupplierQualityScore(data) {
    ...
    async getStationByCanonicalId(canonicalId) {
    ...
    async updateStationRevenue(id, revenue, washes) {
    ...
    // HR Time Tracking
    async getEmployeeTimeTracking(employeeId) {
    ...
    async getTimeTrackingByDateRange(employeeId, start, end) {
        return await db.select().from(hrTimeTracking)
            .where(and(eq(hrTimeTracking.employeeId, employeeId), gte(hrTimeTracking.clockInTime, new Date(start)), lte(hrTimeTracking.clockInTime, new Date(end))))
    ...
    async clockOut(id, clockOutTime) {
    ...
    async approveTimeEntry(id, approvedBy) {
        const [entry] = await db.update(hrTimeTracking)
            .set({
    ...
    async getPendingReviews() {
        return await db.select().from(hrPerformanceReviews)
            .where(or(eq(hrPerformanceReviews.status, "draft"), eq(hrPerformanceReviews.status, "completed")))
            .orderBy(desc(hrPerformanceReviews.reviewDate));
    }
    ...
    async acknowledgeReview(id, signature) {
        const [review] = await db.update(hrPerformanceReviews)
            .set({
    ...
    async updateJobApplicationStatus(id, status, updates = {}) {
    ...
    async getOpsTasks(filters = {}) {
        let query = db.select().from(opsTasksTable);
        const conditions = [];
        if (filters.taskId)
            conditions.push(eq(opsTasksTable.taskId, filters.taskId));
        if (filters.assignedTo)
            conditions.push(eq(opsTasksTable.assignedTo, filters.assignedTo));
        if (filters.status)
            conditions.push(eq(opsTasksTable.status, filters.status));
        if (filters.priority)
            conditions.push(eq(opsTasksTable.priority, filters.priority));
        if (filters.category)
            conditions.push(eq(opsTasksTable.category, filters.category));
        if (filters.stationId)
            conditions.push(eq(opsTasksTable.stationId, filters.stationId));
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
    ...
    async updateOpsTask(id, updates) {
    ...
    async completeOpsTask(id, completedBy, notes) {
    ...
    async getOverdueOpsTasks() {
        return await db.select().from(opsTasksTable)
            .where(and(lt(opsTasksTable.dueDate, new Date()), or(eq(opsTasksTable.status, 'pending'), eq(opsTasksTable.status, 'in_progress'))))
    ...
    async createIncident(incident) {
        ...
        const targetMinutes = slaMinutes[inc.severity] || 480;
        const startTime = new Date();
    ...
    async getIncidents(filters = {}) {
        ...
        if (filters.incidentId)
            conditions.push(eq(opsIncidents.incidentId, filters.incidentId));
        if (filters.severity)
            conditions.push(eq(opsIncidents.severity, filters.severity));
        if (filters.status)
            conditions.push(eq(opsIncidents.status, filters.status));
        if (filters.category)
            conditions.push(eq(opsIncidents.category, filters.category));
        if (filters.stationId)
            conditions.push(eq(opsIncidents.stationId, filters.stationId));
        if (filters.assignedTo)
            conditions.push(eq(opsIncidents.assignedTo, filters.assignedTo));
        if (filters.slaBreach !== undefined)
            conditions.push(eq(opsIncidents.slaBreach, filters.slaBreach));
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
    ...
    async resolveIncident(id, resolvedBy, resolution, preventiveMeasures) {
        const [incident] = await db.update(opsIncidents)
            .set({
            status: 'resolved',
            resolution,
    ...
    async closeIncident(id, closedBy) {
        const [incident] = await db.update(opsIncidents)
            .set({
            status: 'closed',
            closedAt: new Date(),
    ...
    async escalateIncident(id, escalatedBy, escalationNotes) {
        const [incident] = await db.update(opsIncidents)
            .set({
            status: 'escalated',
            updatedAt: new Date(),
    ...
    async createSlaTracking(sla) {
        const [tracking] = await db.insert(opsSlaTracking).values(sla).returning();
        return tracking;
    }
    async getSlaTracking(id) {
        const [tracking] = await db.select().from(opsSlaTracking).where(eq(opsSlaTracking.id, id));
        return tracking;
    }
    async getSlaTrackings(filters = {}) {
        let query = db.select().from(opsSlaTracking);
        const conditions = [];
        if (filters.entityType)
            conditions.push(eq(opsSlaTracking.entityType, filters.entityType));
        if (filters.entityId)
            conditions.push(eq(opsSlaTracking.entityId, filters.entityId));
        if (filters.slaType)
            conditions.push(eq(opsSlaTracking.slaType, filters.slaType));
        if (filters.status)
            conditions.push(eq(opsSlaTracking.status, filters.status));
        if (filters.isBreach !== undefined)
            conditions.push(eq(opsSlaTracking.isBreach, filters.isBreach));
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
    ...
    async updateSlaTracking(id, updates) {
    ...
    async completeSlaTracking(id, completedTime) {
        const [tracking] = await db.select().from(opsSlaTracking).where(eq(opsSlaTracking.id, id));
        if (!tracking)
            throw new Error("SLA tracking not found");
    ...
    async getSlaBreaches(entityType) {
        let query = db.select().from(opsSlaTracking).where(eq(opsSlaTracking.isBreach, true));
        if (entityType) {
            query = query.where(and(eq(opsSlaTracking.isBreach, true), eq(opsSlaTracking.entityType, entityType)));
        }
        return await query.orderBy(desc(opsSlaTracking.createdAt));
    }
    async getSlaMetrics(entityType) {
        let query = db.select().from(opsSlaTracking);
        if (entityType) {
            query = query.where(eq(opsSlaTracking.entityType, entityType));
        }
        const slas = await query;
        const totalSlas = slas.length;
        const breachCount = slas.filter((s) => s.isBreach).length;
        const breachRate = totalSlas > 0 ? (breachCount / totalSlas) * 100 : 0;
        const completedSlas = slas.filter((s) => s.completedTime);
        const avgActualMinutes = completedSlas.length > 0
            ? completedSlas.reduce((sum, s) => sum + (s.actualMinutes || 0), 0) / completedSlas.length
            : 0;
        const responseSlas = slas.filter((s) => s.slaType === 'response_time' && s.completedTime);
        const avgResponseTime = responseSlas.length > 0
            ? responseSlas.reduce((sum, s) => sum + (s.actualMinutes || 0), 0) / responseSlas.length
            : 0;
        const resolutionSlas = slas.filter((s) => s.slaType === 'resolution_time' && s.completedTime);
        const avgResolutionTime = resolutionSlas.length > 0
            ? resolutionSlas.reduce((sum, s) => sum + (s.actualMinutes || 0), 0) / resolutionSlas.length
            : 0;
        return {
            totalSlas,
            breachCount,
    ...
    async getWarehouses(filters) {
        ...
        query = query.orderBy(desc(logisticsWarehouses.createdAt));
        if (filters?.limit) {
            query = query.limit(filters.limit);
        }
        if (filters?.offset) {
    ...
    async getInventoryItems(filters) {
        ...
        if (filters?.searchTerm) {
            conditions.push(or(like(logisticsInventory.productName, `%${filters.searchTerm}%`), like(logisticsInventory.sku, `%${filters.searchTerm}%`)));
        }
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
        query = query.orderBy(logisticsInventory.productName);
        if (filters?.limit) {
            query = query.limit(filters.limit);
        }
        if (filters?.offset) {
    ...
    async adjustInventoryQuantity(id, quantityChange, notes) {
    ...
    async getExpiringItems(daysThreshold = 30) {
        ...
        const items = await db.select()
            .from(logisticsInventory)
            .where(and(sql `${logisticsInventory.expiryDate} IS NOT NULL`, lte(logisticsInventory.expiryDate, thresholdDate.toISOString().split('T')[0])))
    ...
    async getFulfillmentOrders(filters) {
        ...
        query = query.orderBy(desc(logisticsFulfillmentOrders.orderDate));
        if (filters?.limit) {
            query = query.limit(filters.limit);
        }
        if (filters?.offset) {
    ...
    async updateFulfillmentOrder(id, updates) {
    ...
    async shipFulfillmentOrder(id, trackingNumber, carrier) {
    ...
    async deliverFulfillmentOrder(id) {
    ...
    async cancelFulfillmentOrder(id, reason) {
    ...
    async createAccountsPayable(data) {
        const results = await db.insert(accountsPayable).values(data).returning();
        return results[0];
    }
    async updateAccountsPayable(id, updates) {
    ...
    async deleteAccountsPayable(id) {
    ...
    async getOverduePayables() {
        ...
        return await db.select()
            ...
            .where(and(lt(accountsPayable.dueDate, today.toISOString().split('T')[0]), or(eq(accountsPayable.paymentStatus, 'pending'), eq(accountsPayable.paymentStatus, 'overdue'))))
            .orderBy(accountsPayable.dueDate);
    }
    async getPayablesBySupplier(supplierId) {
        return await db.select()
            .from(accountsPayable)
            .where(eq(accountsPayable.supplierId, supplierId))
            .orderBy(desc(accountsPayable.invoiceDate));
    }
    async getPayablesByStatus(status) {
        return await db.select()
            .from(accountsPayable)
            .where(eq(accountsPayable.paymentStatus, status))
            .orderBy(desc(accountsPayable.dueDate));
    }
    async markPayableAsPaid(id, paymentDate, paymentMethod, paymentReference) {
        const results = await db.update(accountsPayable)
            .set({
            paymentStatus: 'paid',
    ...
    async getAccountsReceivableById(id) {
    ...
    async updateAccountsReceivable(id, updates) {
    ...
    async deleteAccountsReceivable(id) {
    ...
    async getOverdueReceivables() {
        ...
        return await db.select()
            ...
            .where(and(lt(accountsReceivable.dueDate, today.toISOString().split('T')[0]), or(eq(accountsReceivable.paymentStatus, 'pending'), eq(accountsReceivable.paymentStatus, 'overdue'), eq(accountsReceivable.paymentStatus, 'partial'))))
            .orderBy(accountsReceivable.dueDate);
    }
    async getReceivablesByCustomer(customerId) {
        return await db.select()
            .from(accountsReceivable)
            .where(eq(accountsReceivable.customerId, customerId))
            .orderBy(desc(accountsReceivable.invoiceDate));
    }
    async getReceivablesByStatus(status) {
        return await db.select()
            .from(accountsReceivable)
            .where(eq(accountsReceivable.paymentStatus, status))
            .orderBy(desc(accountsReceivable.dueDate));
    }
    async recordReceivablePayment(id, amount, paymentDate, paymentMethod) {
    ...
    async updateTaxReturn(id, updates) {
    ...
    async getTaxReturnsByPeriod(taxYear, taxPeriod) {
        return await db.select()
            .from(taxReturns)
            .where(and(eq(taxReturns.taxYear, taxYear), eq(taxReturns.taxPeriod, taxPeriod)))
            .orderBy(desc(taxReturns.dueDate));
    }
    async getTaxReturnsByStatus(status) {
        return await db.select()
            .from(taxReturns)
            .where(eq(taxReturns.status, status))
            .orderBy(desc(taxReturns.dueDate));
    }
    async submitTaxReturn(id, submittedBy) {
        const results = await db.update(taxReturns)
            .set({
            status: 'submitted',
    ...
    async createTaxPayment(data) {
        const results = await db.insert(taxPayments).values(data).returning();
        return results[0];
    }
    async getTaxPaymentsByReturn(taxReturnId) {
        return await db.select()
            .from(taxPayments)
            .where(eq(taxPayments.taxReturnId, taxReturnId))
            .orderBy(desc(taxPayments.paymentDate));
    }
    async getTaxPaymentsByDateRange(startDate, endDate) {
        return await db.select()
            .from(taxPayments)
            .where(and(gte(taxPayments.paymentDate, startDate), lte(taxPayments.paymentDate, endDate)))
            .orderBy(taxPayments.paymentDate);
    }
    async getAllTaxAuditLogs() {
        return await db.select().from(taxAuditLogs).orderBy(desc(taxAuditLogs.timestamp));
    }
    async getTaxAuditLogById(id) {
        const results = await db.select().from(taxAuditLogs).where(eq(taxAuditLogs.id, id));
        return results[0];
    }
    async createTaxAuditLog(data) {
        const results = await db.insert(taxAuditLogs).values(data).returning();
        return results[0];
    }
    async getTaxAuditLogsByEntity(entityType, entityId) {
        return await db.select()
            .from(taxAuditLogs)
            .where(and(eq(taxAuditLogs.entityType, entityType), eq(taxAuditLogs.entityId, entityId)))
            .orderBy(desc(taxAuditLogs.timestamp));
    }
    async getTaxAuditLogsByUser(userId) {
        return await db.select()
            .from(taxAuditLogs)
            .where(eq(taxAuditLogs.userId, userId))
            .orderBy(desc(taxAuditLogs.timestamp));
    }
    async getTaxAuditLogsByDateRange(startDate, endDate) {
        return await db.select()
            .from(taxAuditLogs)
            .where(and(gte(taxAuditLogs.timestamp, startDate), lte(taxAuditLogs.timestamp, endDate)))
    ...
    async recordPolicyAcknowledgment(data) {
        const results = await db.insert(policyAcknowledgments).values(data).returning();
        return results[0];
    }
    async getPolicyAcknowledgments(policyId) {
        return await db.select()
    ...
    async createComplianceCertification(data) {
        const results = await db.insert(complianceCertifications).values(data).returning();
        return results[0];
    }
    async updateComplianceCertification(id, updates) {
    ...
    async recordRoyaltyPayment(id, paymentData) {
        const result = await db.update(franchiseRoyaltyPayments)
            .set({
            paymentStatus: 'paid',
