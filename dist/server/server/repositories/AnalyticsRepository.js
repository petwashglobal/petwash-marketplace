...
export class AnalyticsRepository {
    ...
    async getMetricsByPlatform(platform, startDate, endDate) {
        return await db.select()
            ...
            .where(and(eq(platformMetrics.platform, platform), gte(platformMetrics.date, startDate), lte(platformMetrics.date, endDate)))
            .orderBy(desc(platformMetrics.date));
    }
    async getAllPlatformMetrics(startDate, endDate) {
        return await db.select()
            .from(platformMetrics)
            .where(and(gte(platformMetrics.date, startDate), lte(platformMetrics.date, endDate)))
            .orderBy(desc(platformMetrics.date));
    }
    async calculateDailyMetrics(platform, date) {
        ...
        const transactionStats = await db.select({
            revenue: sql `COALESCE(SUM(CAST(${walletTransactions.amount} AS DECIMAL)), 0)::text`,
            transactions: sql `COUNT(*)::int`,
            avgValue: sql `COALESCE(AVG(CAST(${walletTransactions.amount} AS DECIMAL)), 0)::text`,
        }).from(walletTransactions)
            .where(and(eq(walletTransactions.platform, platform), eq(walletTransactions.type, 'debit'), gte(walletTransactions.createdAt, startOfDay), lte(walletTransactions.createdAt, endOfDay)));
        // Calculate active users
        const activeUsersResult = await db.select({
            count: sql `COUNT(DISTINCT ${userActivity.userId})::int`,
        }).from(userActivity)
            .where(and(eq(userActivity.platform, platform), gte(userActivity.timestamp, startOfDay), lte(userActivity.timestamp, endOfDay)));
        return {
    ...
    async getRevenueByPlatform(startDate, endDate) {
        return await db.select({
            platform: platformMetrics.platform,
            totalRevenue: sql `SUM(CAST(${platformMetrics.revenue} AS DECIMAL))::text`,
            totalTransactions: sql `SUM(${platformMetrics.transactions})::int`,
        }).from(platformMetrics)
            .where(and(gte(platformMetrics.date, startDate), lte(platformMetrics.date, endDate)))
            .groupBy(platformMetrics.platform);
    }
    async getGrowthMetrics(platform, days = 30) {
    ...
    async getTopPerformingPlatforms(limit = 5) {
        ...
        return await db.select({
            ...
            totalRevenue: sql `SUM(CAST(${platformMetrics.revenue} AS DECIMAL))::text`,
            totalTransactions: sql `SUM(${platformMetrics.transactions})::int`,
            avgConversionRate: sql `AVG(CAST(${platformMetrics.conversionRate} AS DECIMAL))::text`,
        }).from(platformMetrics)
    ...
    async getUserLifetimeValue(userId) {
        const result = await db.select({
            ...
            .where(and(eq(walletTransactions.userId, userId), eq(walletTransactions.type, 'debit')));
    ...
    async getChurnPrediction(userId) {
        ...
        if (!profile[0] || !profile[0].lastActivityDate)
            return 0.5;
