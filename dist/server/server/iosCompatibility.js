...
export function iosCompatibilityMiddleware(req, res, next) {
...
export function iosSSLBypass(req, res, next) {
    const userAgent = req.headers['user-agent'] || '';
    const isIOS = /iPhone|iPad|iPod/i.test(userAgent);
    const host = req.get('host') || '';
...
export function iosErrorHandler(err, req, res, next) {
    const userAgent = req.headers['user-agent'] || '';
    const isIOS = /iPhone|iPad|iPod/i.test(userAgent);
...
export function detectIOSDevice(userAgent) {
