...
const BIOMETRIC_BUCKET_NAME = process.env.BIOMETRIC_BUCKET_NAME || `${PROJECT_ID}.firebasestorage.app`;
const BIOMETRIC_PREFIX = process.env.BIOMETRIC_PREFIX || "biometric-certificates/";
let storage = null;
...
function initializeStorage() {
    ...
    if (!process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
        throw new Error("[BiometricStorage] Missing FIREBASE_SERVICE_ACCOUNT_KEY env var");
    }
    try {
        ...
        throw new Error(`[BiometricStorage] Failed to parse FIREBASE_SERVICE_ACCOUNT_KEY: ${error.message}`);
    }
}
/**
...
export async function ensureBiometricStorage() {
    try {
        ...
        if (!exists) {
            logger.warn(`[BiometricStorage] Bucket "${BIOMETRIC_BUCKET_NAME}" does not exist. ` +
                `Create it in Firebase Console → Storage. ` +
                `Expected name: "${PROJECT_ID}.firebasestorage.app"`);
            logger.warn("[BiometricStorage] Biometric certificate uploads will be disabled until bucket is created");
            return;
        }
        ...
        const hasSameRule = existingRules.some((r) => r.action?.type === "Delete" &&
        ...
        if (!hasSameRule) {
            ...
            logger.info(`[BiometricStorage] ✅ Lifecycle rule configured on ${BIOMETRIC_BUCKET_NAME}: delete "${BIOMETRIC_PREFIX}" objects after 1 day`);
        }
        else {
            logger.info(`[BiometricStorage] ✅ Lifecycle rule already configured on ${BIOMETRIC_BUCKET_NAME}`);
        }
        isInitialized = true;
    }
    catch (error) {
        ...
        logger.warn("[BiometricStorage] Biometric certificate uploads will be disabled. " +
 ...
 */
export async function uploadBiometricObject(objectName, buffer, contentType) {
    if (!isInitialized) {
        throw new Error("[BiometricStorage] Storage not initialized. Biometric uploads are disabled.");
    }
    const storageClient = initializeStorage();
    const bucket = storageClient.bucket(BIOMETRIC_BUCKET_NAME);
    ...
    logger.info(`[BiometricStorage] ✅ Uploaded biometric object: ${BIOMETRIC_PREFIX}${objectName}`);
    return file.name;
}
/**
 ...
 * @param objectName - Name of the file (without prefix)
 */
export async function deleteBiometricObject(objectName) {
    if (!isInitialized) {
    ...
    try {
        await file.delete();
        logger.info(`[BiometricStorage] ✅ Deleted biometric object: ${BIOMETRIC_PREFIX}${objectName}`);
    }
    catch (error) {
        if (error.code === 404) {
            logger.warn(`[BiometricStorage] Object not found (already deleted): ${BIOMETRIC_PREFIX}${objectName}`);
        }
        else {
            throw error;
        }
 ...
 */
export function isBiometricStorageAvailable() {
