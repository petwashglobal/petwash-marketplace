 ...
 */
async function getAuthSuccessRate() {
    try {
...
async function getP95Latency() {
    try {
        ...
        if (!latencyMetric?.values)
            return 0;
        // Extract latency values
        const latencies = [];
        latencyMetric.values.forEach((v) => {
            if (v.value)
                latencies.push(v.value);
        });
...
export async function runCanaryChecks() {
    ...
    const canaryPassed = healthzResult.passed &&
    ...
    const shouldRollback = consecutiveHealthFailures >= HEALTH_CHECK_CONFIG.consecutiveFailuresThreshold ||
 ...
 */
export async function monitorCanaryDeployment(durationMs = 300000) {
 ...
 */
export function resetCanaryCounters() {
    consecutiveHealthFailures = 0;
    consecutiveReadinessFailures = 0;
