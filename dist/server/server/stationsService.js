 ...
 */
export async function getMergedMonitoringConfig(stationId) {
    const globalConfig = await getMonitoringConfig();
    try {
        const stationDoc = await db.collection('stations').doc(stationId).get();
        const overrides = stationDoc.data()?.monitoringOverrides;
        if (!overrides)
            return globalConfig;
        return {
            ...globalConfig,
 ...
 */
export function isBusyHours(config, now = new Date()) {
    try {
        // Parse time strings
 ...
 */
export async function shouldSuppressLowActivity(stationId, config) {
    if (!config.weatherSuppression.enabled)
        return false;
    try {
 ...
 */
export async function logStationEvent(event) {
    try {
...
// ==================== STATION CRUD ====================
export async function getStationByTerminalId(terminalId) {
    try {
...
export async function getAllStations(filters) {
    try {
        ...
        if (filters?.q) {
            ...
            stations = stations.filter(s => s.label.toLowerCase().includes(q) ||
 ...
 */
export async function updateStationHeartbeat(stationId, source = 'ping') {
    try {
        const updates = {
 ...
 */
export async function updateStationStatus(stationId) {
    try {
 ...
 */
export async function updateAllStationStatuses() {
    try {
 ...
 */
async function createOfflineAlert(stationId, minutesOffline) {
 ...
 */
async function createLowActivityAlert(stationId, minutesInactive, isBusyHours) {
    // Check if alert already exists (deduplication)
 ...
 */
async function handleOfflineRecovery(stationId) {
    try {
...
async function getActiveAlertByType(stationId, type) {
    try {
...
async function autoResolveOfflineAlert(stationId) {
    try {
        const alert = await getActiveAlertByType(stationId, 'offline');
        if (alert && alert.id) {
            await resolveAlert(alert.id);
...
export async function acknowledgeAlerts(stationId, types) {
    try {
...
export async function getStationAlerts(stationId, limit = 100) {
    try {
        const snapshot = await db.collection('station_alerts')
            .where('stationId', '==', stationId)
...
// ==================== ECU FAULTS ====================
export async function createECUFault(fault) {
    try {
        const faultData = {
            ...fault,
            createdAt: Timestamp.now(),
...
export async function resolveECUFault(faultId, adminUid, notes) {
    try {
...
async function hasActiveECUFault(stationId) {
    try {
...
// ==================== EMAIL ALERTS ====================
async function sendOfflineEmail(stationId, minutesOffline) {
    try {
...
async function sendRecoveryEmail(stationId) {
    try {
        const { EmailService } = await import('./emailService');
        const station = await getStation(stationId);
        if (!station)
...
// ==================== UPTIME CALCULATION ====================
export async function calculateStationUptime(stationId, period) {
    try {
...
export async function updateAllStationUptime() {
    try {
        const stations = await getAllStations();
 ...
 */
export async function sendOfflineReminderEmails() {
    try {
        const config = await getMonitoringConfig();
 ...
 */
export async function setMaintenanceMode(stationId, enabled, reason, adminUid) {
    try {
        const updates = {
 ...
 */
async function autoResolveAllAlerts(stationId) {
    try {
        const snapshot = await db.collection('station_alerts')
            .where('stationId', '==', stationId)
            .where('resolved', '==', false)
 ...
 */
export async function simulateHeartbeats(stationId, heartbeats) {
 ...
 */
export async function setTestWeather(stationId, tempC, precipitation) {
 ...
 */
export async function setTestConfigOverride(stationId, override) {
    await db.collection('stations').doc(stationId).update({
 ...
 */
export async function resetTestStation(stationId) {
 ...
 */
export async function runMonitoringTest(testCase) {
    try {
        ...
        if (testCase.inputs.weather) {
            await setTestWeather(testCase.inputs.stationId, testCase.inputs.weather.tempC, testCase.inputs.weather.precipitation);
        }
        // Simulate heartbeats
        ...
        if (testCase.inputs.maintenanceMode) {
            await setMaintenanceMode(testCase.inputs.stationId, testCase.inputs.maintenanceMode.enabled, testCase.inputs.maintenanceMode.reason, 'test-admin');
        }
 ...
 */
export function getAcceptanceTestCases(testStationId = 'TEST-001') {
    const now = new Date();
 ...
 */
export async function runAllAcceptanceTests(testStationId = 'TEST-001') {
 ...
 */
export async function getStationAnalyticsFor24Hours() {
    try {
        const now = new Date();
