...
async function getAccessToken() {
    ...
    try {
        const response = await fetch('https://' + hostname + '/api/v2/connection?include_secrets=true&connector_names=hubspot', {
            headers: {
...
async function enqueueRetry(taskId, fn, attempts = 0) {
    const maxAttempts = 3;
...
export async function syncUserToHubSpot(data) {
    ...
    const syncTask = async () => {
        try {
            ...
            if (data.lastname)
                contactProperties.lastname = data.lastname;
            if (data.phone)
                contactProperties.phone = data.phone;
            // Standard fields
            if (data.uid)
                contactProperties.company = `Pet Wash User ${data.uid}`;
            if (data.lang)
                contactProperties.hs_language = data.lang === 'he' ? 'he' : 'en';
            if (data.dob)
                contactProperties.date_of_birth = data.dob;
            if (data.country)
                contactProperties.country = data.country;
            // Custom properties (these need to be created in HubSpot first)
            if (data.uid)
                contactProperties.petwash_uid = data.uid;
            if (data.loyaltyProgram !== undefined)
                contactProperties.petwash_loyalty = data.loyaltyProgram;
            if (data.reminders !== undefined)
                contactProperties.petwash_reminders = data.reminders;
            if (data.marketing !== undefined)
                contactProperties.petwash_marketing = data.marketing;
            if (data.consent !== undefined)
                contactProperties.petwash_consent = data.consent;
            if (data.consentTimestamp)
                contactProperties.consent_timestamp = data.consentTimestamp;
            // Pet information custom properties
            if (data.petName)
                contactProperties.petwash_pet_name = data.petName;
            if (data.petBreed)
                contactProperties.petwash_pet_breed = data.petBreed;
            if (data.petAge)
                contactProperties.petwash_pet_age = data.petAge;
            if (data.petWeight)
                contactProperties.petwash_pet_weight = data.petWeight;
            ...
            try {
                ...
                if (createError.code === 409 || createError.body?.category === 'CONFLICT') {
                    ...
                    if (existingIdMatch) {
                        ...
                        const response = await hubspotClient.crm.contacts.basicApi.update(existingId, { properties: contactProperties });
                    ...
                    if (searchResponse.results.length > 0) {
                        const response = await hubspotClient.crm.contacts.basicApi.update(searchResponse.results[0].id, { properties: contactProperties });
...
export async function trackHubSpotEvent(email, eventName, properties) {
