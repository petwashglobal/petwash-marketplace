...
async function voidExpiredPayments() {
    try {
        ...
        const expiredPayments = await db.select()
            .from(paymentIntents)
            .where(and(eq(paymentIntents.status, 'authorized'), lt(paymentIntents.createdAt, fifteenMinutesAgo)))
        ...
        const results = await Promise.allSettled(filteredPayments.map(async (paymentIntent) => {
            try {
                const result = await NayaxJobDispatchPaymentService.voidPayment(paymentIntent.id, 'Booking expired - no operator accepted');
                if (result.success) {
