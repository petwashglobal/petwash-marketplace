    ...
    try {
        ...
        for (const partner of activePartners) {
            const agreements = await db.query.partnerAgreements.findMany({
                where: and(eq(partnerAgreements.partnerId, partner.id), eq(partnerAgreements.isActive, true)),
            });
        ...
        const settlementResults = await Promise.allSettled(partnersWithAgreements.map(async ({ partner, agreementsCount }) => {
            try {
                ...
                const settlement = await FinanceSettlementService.generateSettlement(partner.id, periodStart, periodEnd, 'monthly');
        ...
        // 4. Collect results
        const successful = settlementResults.filter(r => r.status === 'fulfilled' && r.value.success);
        const failed = settlementResults.filter(r => r.status === 'rejected' || (r.status === 'fulfilled' && !r.value.success));
