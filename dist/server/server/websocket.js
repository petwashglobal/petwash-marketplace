...
async function handleAuth(client, payload) {
    const { token } = payload || {};
    if (!token) {
        client.ws.send(JSON.stringify({
            type: 'auth_failed',
...
async function sendStationsSnapshot(client) {
    try {
...
async function sendTelemetrySnapshot(client, payload) {
    try {
...
async function sendAlertsSnapshot(client, payload) {
    try {
...
export function broadcastTelemetryUpdate(stationId, telemetry) {
...
export function broadcastAlert(stationId, alert) {
    const message = JSON.stringify({
        type: 'alert',
        stationId,
        data: alert,
...
export function broadcastStationStatusChange(stationId, status) {
    const message = JSON.stringify({
...
async function handleMessagingAuth(client, payload) {
...
async function handleConversationSubscribe(client, payload) {
...
function handleConversationUnsubscribe(client, payload) {
    const { conversationIds } = payload || {};
...
export function broadcastNewMessage(conversationId, message, participantUids) {
...
export function notifyConversationUpdate(userId, conversationId, update) {
    const payload = JSON.stringify({
