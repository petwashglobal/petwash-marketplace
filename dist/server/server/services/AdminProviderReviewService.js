 ...
 */
import { db } from '../db';
import { providerApprovalQueue, } from '@shared/schema';
...
class AdminProviderReviewService {
     ...
     */
    async createApplication(providerId, platform, priority = 'normal') {
        try {
            // Check if already in queue
            const existing = await db
                .select()
                .from(providerApprovalQueue)
                .where(and(eq(providerApprovalQueue.providerId, providerId), eq(providerApprovalQueue.platform, platform)))
                .limit(1);
            if (existing[0]) {
     ...
     */
    async getQueue(status, platform, limit = 50) {
        try {
            ...
            ELSE 4 
          END`, desc(providerApprovalQueue.createdAt))
                .limit(limit);
        }
        catch (error) {
     ...
     */
    async assignToReviewer(applicationId, reviewerId) {
        try {
            await db
     ...
     */
    async updateChecklistItem(applicationId, itemKey, approved, reviewerId) {
        try {
            const updateData = {
                [itemKey]: approved,
                updatedAt: new Date(),
     ...
     */
    async rejectApplication(applicationId, reviewerId, rejectionReason) {
        try {
            const review = await this.getApplicationReview(applicationId);
            if (!review) {
                return {
     ...
     */
    async putOnHold(applicationId, reviewerId, reason) {
        try {
            await db
                .update(providerApprovalQueue)
                .set({
     ...
     */
    async isProviderApproved(providerId, platform) {
        try {
            const application = await db
                .select()
                .from(providerApprovalQueue)
                .where(and(eq(providerApprovalQueue.providerId, providerId), eq(providerApprovalQueue.platform, platform), eq(providerApprovalQueue.status, 'approved')))
                .limit(1);
     ...
     */
    async getApprovedProviders(platform, limit = 100) {
        try {
            return await db
                .select()
                .from(providerApprovalQueue)
                .where(and(eq(providerApprovalQueue.platform, platform), eq(providerApprovalQueue.status, 'approved')))
