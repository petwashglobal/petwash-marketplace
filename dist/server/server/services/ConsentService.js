...
export class ConsentService {
     ...
     */
    async getConsentStatus(userId, verificationId) {
        try {
            let query = db
                .select()
                .from(biometricConsents)
                .where(and(eq(biometricConsents.userId, userId), eq(biometricConsents.isRevoked, false)))
            ...
            if (verificationId) {
                query = db
                    .select()
                    .from(biometricConsents)
                    .where(and(eq(biometricConsents.userId, userId), eq(biometricConsents.verificationId, verificationId), eq(biometricConsents.isRevoked, false)))
                    .orderBy(biometricConsents.createdAt)
                    .limit(1);
            }
     ...
     */
    async revokeConsent(userId, reason) {
        try {
            await db
     ...
     */
    async validateConsentForProcessing(userId, requireBiometric = true) {
        try {
     ...
     */
    async verifyAuditIntegrity(consentId) {
        try {
