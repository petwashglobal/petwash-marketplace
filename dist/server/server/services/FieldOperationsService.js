...
import { fieldUpdates, fieldUpdatePhotos, staffDevices, stations, locations } from '@shared/schema';
...
export class FieldOperationsService {
     ...
     */
    static async createFieldUpdate(params, photos) {
        try {
     ...
     */
    static async uploadPhotos(fieldUpdateId, files) {
        try {
     ...
     */
    static async getTimelineForStation(stationId) {
        try {
            ...
            const updatesWithPhotos = await Promise.all(updates.map(async (update) => {
     ...
     */
    static async getFieldUpdatesForTask(taskId) {
        try {
            const updates = await db
                .select()
                .from(fieldUpdates)
     ...
     */
    static generateWazeUrl(latitude, longitude) {
        return `waze://?ll=${latitude},${longitude}&navigate=yes`;
    }
    /**
     * Generate Google Maps URL
     */
    static generateGoogleMapsUrl(latitude, longitude) {
        return `https://www.google.com/maps?q=${latitude},${longitude}`;
    }
    /**
     * Get mobile-optimized station summary
     */
    static async getStationSummaryForMobile(stationId) {
        try {
            ...
            const wazeUrl = this.generateWazeUrl(Number(station.location.latitude), Number(station.location.longitude));
            const googleMapsUrl = this.generateGoogleMapsUrl(Number(station.location.latitude), Number(station.location.longitude));
            const summary = {
                id: station.id,
     ...
     */
    static async getNearbyStations(params) {
        try {
        ...
        LIMIT 50
      `);
            const summaries = nearbyStations.rows.map((row) => ({
                id: row.id,
     ...
     */
    static async registerDevice(device) {
        try {
            // Check if device already exists
            const [existing] = await db
                .select()
                .from(staffDevices)
                .where(and(eq(staffDevices.userId, device.userId), eq(staffDevices.platform, device.platform)))
                .limit(1);
            if (existing) {
     ...
     */
    static async updatePushToken(deviceId, pushToken) {
        try {
            const [updated] = await db
                .update(staffDevices)
                .set({
