 ...
 */
import { db } from '../db';
import { auditLedger, voucherRedemptions, discountUsageLog, merkleSnapshots, } from '@shared/schema';
...
export class AuditLedgerService {
    ...
    static async recordDiscountUsage(params) {
        try {
            ...
            const whereCondition = params.oneTimePerUser !== false
                ? and(eq(discountUsageLog.discountCode, params.discountCode), eq(discountUsageLog.userId, params.userId))
    ...
    static async getEntityAuditTrail(entityType, entityId) {
        try {
            const records = await db
                .select()
                .from(auditLedger)
                .where(and(eq(auditLedger.entityType, entityType), eq(auditLedger.entityId, entityId)))
                .orderBy(desc(auditLedger.createdAt));
            return records;
        }
        catch (error) {
     ...
     */
    static async verifyChainIntegrity(startBlock, endBlock) {
        try {
            const query = db.select().from(auditLedger).orderBy(auditLedger.blockNumber);
            const records = startBlock && endBlock
                ? await query.where(and(gte(auditLedger.blockNumber, startBlock), lte(auditLedger.blockNumber, endBlock)))
                : await query;
            if (records.length === 0) {
                return {
                    isValid: true,
            ...
            for (let i = 0; i < records.length; i++) {
                ...
                if (previousRecord) {
                    if (record.previousHash !== previousRecord.currentHash) {
                        errors.push(`Block ${record.blockNumber}: Hash mismatch. ` +
                ...
                if (record.currentHash !== expectedHash) {
                    errors.push(`Block ${record.blockNumber}: Record hash invalid. ` +
     ...
     */
    static async createDailySnapshot(date = new Date()) {
        try {
            ...
            const records = await db
                .select()
                .from(auditLedger)
                .where(and(gte(auditLedger.createdAt, startOfDay), lte(auditLedger.createdAt, endOfDay)))
     ...
     */
    static async getFraudMonitoringStats(timeframe = 'today') {
        try {
            const now = new Date();
            let startDate;
            switch (timeframe) {
            ...
            const highRiskEvents = await db
                .select()
                .from(auditLedger)
                .where(and(gte(auditLedger.createdAt, startDate), gte(auditLedger.fraudScore, 50)))
            ...
            const suspiciousUsers = await db
                ...
                avgFraudScore: sql `avg(${auditLedger.fraudScore})`,
                maxFraudScore: sql `max(${auditLedger.fraudScore})`,
            })
                .from(auditLedger)
                .where(gte(auditLedger.createdAt, startDate))
