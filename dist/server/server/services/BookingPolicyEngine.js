...
import { bookingPolicies, disputeResolutions, } from "@shared/schema-compliance";
...
export class BookingPolicyEngineService {
     ...
     */
    async getApplicablePolicy(serviceType, country = "IL") {
        const policies = await db
            .select()
            .from(bookingPolicies)
            .where(and(eq(bookingPolicies.serviceType, serviceType), eq(bookingPolicies.isActive, true)))
            .limit(1);
        if (policies.length === 0) {
            ...
            const fallback = await db
                .select()
                .from(bookingPolicies)
                .where(and(eq(bookingPolicies.serviceType, "all"), eq(bookingPolicies.isActive, true)))
                .limit(1);
     ...
     */
    async calculateCancellation(serviceType, bookingAmount, bookingDate, cancellationDate = new Date(), country = "IL") {
        // Get policy
        ...
        const rules = policy.cancellationRules;
        let refundPercent = 0;
     ...
     */
    async processAutoRefund(bookingId, refundAmount, refundMethod = "original_payment") {
     ...
     * Resolve dispute
     */
    async resolveDispute(disputeId, resolution, resolutionHe, refundAmount, status, resolvedBy) {
     ...
     */
    async checkDisputeSLA(disputeId) {
        const [dispute] = await db
            .select()
     ...
     */
    async escalateDispute(disputeId, escalationReason) {
        const [updated] = await db
            .update(disputeResolutions)
            .set({
            isEscalated: true,
     ...
     */
    getRefundCalculation(originalAmount, refundPercent, cancellationFee, currency = "ILS") {
     ...
     */
    async validateCancellation(bookingId, userId) {
