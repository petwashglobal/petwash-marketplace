...
export class NayaxMonitoringService {
    ...
    static async monitorPendingTransactions() {
        ...
        try {
            ...
            const stuckTransactions = await db
                ...
                .where(and(inArray(nayaxTransactions.status, ['initiated', 'authorized', 'vend_pending']), lt(nayaxTransactions.createdAt, thirtyMinutesAgo)));
     ...
     * Runs every hour
     */
    static async monitorOfflineStations() {
        ...
        try {
            ...
            const terminals = [];
            if (NAYAX_TERMINAL_ID_MAIN)
                terminals.push(NAYAX_TERMINAL_ID_MAIN);
            if (NAYAX_TERMINAL_ID_SECONDARY)
                terminals.push(NAYAX_TERMINAL_ID_SECONDARY);
    ...
    static async generateDailyReport() {
        ...
        try {
            ...
            const transactions = await db
                .select()
                .from(nayaxTransactions)
                .where(and(lt(nayaxTransactions.createdAt, today), lt(yesterday, nayaxTransactions.createdAt)));
            const stats = {
