import { db } from '../db';
import { providerApplications, contractorViolations, contractorReviews, contractorTrustScores } from '@shared/schema';
import { eq, and } from 'drizzle-orm';
import { logger } from '../lib/logger';
/**
 * Calculate vetting score based on background checks and certifications
 */
async function calculateVettingScore(contractorId) {
    try {
        ...
        if (application.providerType === 'sitter' || application.providerType === 'walker') {
            ...
            if (application.petFirstAidCertUrl &&
            ...
            if (application.drivingRecordStatus === 'clean' &&
        ...
        if (application.insuranceCertUrl &&
 ...
 */
async function calculateReviewScore(contractorId) {
    try {
        ...
        const flaggedReviews = reviews.filter((review) => review.isFlagged && !review.adminReviewed).length;
 ...
 */
async function calculateViolationScore(contractorId) {
    try {
        // Get all violations for contractor
        const violations = await db
            .select()
            .from(contractorViolations)
            .where(and(eq(contractorViolations.contractorId, contractorId), eq(contractorViolations.status, 'confirmed') // Only count confirmed violations
        ));
 ...
 */
export async function calculateTrustScores(contractorId) {
    try {
        ...
        const internalRiskScore = vetting.score * 0.3 + reviews.score * 0.4 + violations.score * 0.3;
 ...
 */
export async function updateContractorTrustScores(contractorId) {
    try {
 ...
 */
export async function triggerTrustScoreUpdate(contractorId, reason) {
    try {
