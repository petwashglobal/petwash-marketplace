import { db } from "../db";
import { taxRateHistory } from "../../shared/schema";
import { eq, and, lte, gte, isNull, or, desc } from "drizzle-orm";
export class TaxRateService {
    async getCurrentTaxRates(asOfDate) {
        const queryDate = asOfDate || new Date();
        const queryDateStr = queryDate.toISOString().split('T')[0];
        const rates = await db
            .select()
            .from(taxRateHistory)
            .where(and(eq(taxRateHistory.isActive, true), lte(taxRateHistory.effectiveFrom, queryDateStr), or(isNull(taxRateHistory.effectiveTo), gte(taxRateHistory.effectiveTo, queryDateStr))))
    ...
    async getCurrentVATRate(category = 'standard', asOfDate) {
        const queryDate = asOfDate || new Date();
        const queryDateStr = queryDate.toISOString().split('T')[0];
        const rates = await db
            .select()
            .from(taxRateHistory)
            .where(and(eq(taxRateHistory.taxType, 'vat'), eq(taxRateHistory.category, category), eq(taxRateHistory.isActive, true), lte(taxRateHistory.effectiveFrom, queryDateStr), or(isNull(taxRateHistory.effectiveTo), gte(taxRateHistory.effectiveTo, queryDateStr))))
            .orderBy(desc(taxRateHistory.effectiveFrom))
            .limit(1);
        if (rates.length === 0) {
    ...
    async getVATRateByExpenseCategory(expenseCategory, asOfDate) {
    ...
    calculateVATAmounts(grossAmount, vatRate) {
