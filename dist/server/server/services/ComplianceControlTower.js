...
import { complianceTasks, authorityDocuments, providerLicenses, complianceAuditTrail, } from "@shared/schema-compliance";
import { createHash } from "crypto";
import { nanoid } from "nanoid";
/**
 * Compliance risk levels
 */
export var ComplianceRiskLevel;
...
export class ComplianceControlTowerService {
    ...
    async runComplianceMonitoring() {
        ...
        const expiredDocs = await db
            ...
            .where(and(eq(authorityDocuments.status, "active"), lt(authorityDocuments.expiryDate, now.toISOString().split("T")[0])));
        for (const doc of expiredDocs) {
            issues.push({
                ...
                affectedServices: doc.applicableServices,
            });
        ...
        const expiringDocs = await db
            .select()
            .from(authorityDocuments)
            .where(and(eq(authorityDocuments.status, "active"), gte(authorityDocuments.expiryDate, now.toISOString().split("T")[0]), lt(authorityDocuments.expiryDate, thirtyDaysFromNow.toISOString().split("T")[0])));
        for (const doc of expiringDocs) {
            const daysUntilExpiry = Math.ceil((new Date(doc.expiryDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
            issues.push({
        ...
        const expiredLicenses = await db
            ...
            .where(and(eq(providerLicenses.status, "active"), lt(providerLicenses.expiryDate, now.toISOString().split("T")[0])));
        ...
        const expiringLicenses = await db
            .select()
            .from(providerLicenses)
            .where(and(eq(providerLicenses.status, "active"), gte(providerLicenses.expiryDate, now.toISOString().split("T")[0]), lt(providerLicenses.expiryDate, thirtyDaysFromNow.toISOString().split("T")[0])));
        for (const license of expiringLicenses) {
            const daysUntilExpiry = Math.ceil((new Date(license.expiryDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
            issues.push({
        ...
        const overdueTasks = await db
            ...
            .where(and(or(eq(complianceTasks.status, "pending"), eq(complianceTasks.status, "in_progress")), lt(complianceTasks.dueDate, now)));
        ...
        const pendingTasksResult = await db
            ...
            .from(complianceTasks)
            .where(or(eq(complianceTasks.status, "pending"), eq(complianceTasks.status, "in_progress")));
        const criticalTasksResult = await db
            .select({ count: sql `count(*)` })
            .from(complianceTasks)
            .where(and(eq(complianceTasks.priority, "critical"), or(eq(complianceTasks.status, "pending"), eq(complianceTasks.status, "in_progress"), eq(complianceTasks.status, "overdue"))));
     ...
     */
    async checkProviderCompliance(providerId, providerType) {
        const now = new Date();
        const licenses = await db
            .select()
            .from(providerLicenses)
            .where(and(eq(providerLicenses.providerId, providerId), eq(providerLicenses.providerType, providerType)));
        const issues = [];
     ...
     */
    async suspendProvider(providerId, providerType, reason, relatedEntityId) {
     ...
     */
    async sendExpiryReminder(entityId, entityType, daysUntilExpiry) {
        ...
        if (entity[0]?.lastReminderSent) {
            const daysSinceLastReminder = Math.ceil((Date.now() - new Date(entity[0].lastReminderSent).getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    calculateOverallRisk(issues, criticalTasks) {
     ...
     */
    calculateProviderRiskLevel(issues, expiredLicenses, expiringSoon) {
        const criticalIssues = issues.filter(i => i.severity === ComplianceRiskLevel.CRITICAL).length;
     ...
     */
    getRequiredLicenses(providerType) {
        const requirements = {
