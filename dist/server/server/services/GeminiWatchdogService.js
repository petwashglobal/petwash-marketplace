...
import { db } from '../db';
import { watchdogIssues, watchdogUserStruggles, watchdogAutoFixes, watchdogCheckoutMonitoring, watchdogRegistrationMonitoring } from '../../shared/schema-gemini-watchdog';
...
class GeminiWatchdogService {
    ...
    isRunning = false;
    logBuffer = [];
    userStruggles = new Map();
    BUFFER_SIZE = 100; // Analyze logs in batches of 100
     ...
     */
    async analyzeLogsBatch() {
        if (!this.genAI || this.logBuffer.length === 0)
            return;
...
Respond in JSON format:
{
  ...
  "userImpact": {"affectedUsers": number, "severity": "high|medium|low", "description": "..."}
     ...
     */
    async analyzeErrorImmediate(entry) {
        if (!this.genAI)
            return;
        try {
...
Respond in JSON:
{
  ...
  "immediateAction": "..."
    ...
    async detectUserStruggles() {
        try {
            ...
            const recentStruggles = await db.query.watchdogUserStruggles.findMany({
                where: (struggles, { eq, and, gt }) => and(eq(struggles.resolved, false), gt(struggles.detectedAt, new Date(Date.now() - 3600000))),
                limit: 50
            });
     ...
     */
    async analyzeUserStruggle(struggle) {
        if (!this.genAI)
            return;
        try {
...
Respond in JSON:
{
  ...
  "urgency": "critical|high|medium|low"
     ...
     */
    async analyzeCheckoutFailure(params) {
        if (!this.genAI)
            return;
        try {
...
Respond in JSON:
{
  ...
  "preventionTip": "..."
     ...
     */
    async analyzeRegistrationFailure(params) {
        if (!this.genAI)
            return;
        try {
...
Respond in JSON:
{
  ...
  "risks": "..."
    ...
    async runAutoFixes() {
        try {
            ...
            const openIssues = await db.query.watchdogIssues.findMany({
                where: and(eq(watchdogIssues.status, 'open'), eq(watchdogIssues.autoFixAttempted, false)),
                limit: 5
            });
     ...
     */
    async getStatus() {
        const [issuesCount] = await db.select({ count: sql `count(*)` })
            .from(watchdogIssues)
            .where(eq(watchdogIssues.status, 'open'));
        const [strugglesCount] = await db.select({ count: sql `count(*)` })
            .from(watchdogUserStruggles)
            .where(eq(watchdogUserStruggles.resolved, false));
        const [autoFixesCount] = await db.select({ count: sql `count(*)` })
