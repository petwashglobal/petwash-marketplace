...
export class PetTrekDispatchService {
    MAX_DISPATCH_DRIVERS = 3; // Send to top 3 drivers
    MAX_SEARCH_RADIUS = 15; // km
    DISPATCH_EXPIRY_SECONDS = 30; // Auto-expire after 30s
    /**
     ...
     */
    isVehicleCompatible(vehicleCapacity, petSize) {
     ...
     */
    async findNearbyDrivers(pickupLat, pickupLon, petSize, maxRadius = this.MAX_SEARCH_RADIUS) {
        try {
            // Get online, vetted, available TRANSPORT drivers with location
            const drivers = await db
                .select()
                .from(pettrekProviders)
                .where(and(eq(pettrekProviders.isOnline, true), eq(pettrekProviders.isAvailable, true), eq(pettrekProviders.isVetted, true), eq(pettrekProviders.offersTransport, true) // CRITICAL: Only transport providers
            ));
            ...
            const driversWithDistance = drivers
                ...
                const distanceKm = this.calculateDistance(pickupLat, pickupLon, parseFloat(driver.lastKnownLatitude), parseFloat(driver.lastKnownLongitude));
                ...
                const distanceKm = this.calculateDistance(pickupLat, pickupLon, driverLat, driverLon);
                ...
                if (driver.distanceKm > maxRadius)
                    return false;
                return true;
            })
                .sort((a, b) => {
     ...
     */
    async dispatchTrip(tripId, pickupLat, pickupLon, petSize) {
        try {
            // Find nearby drivers
            ...
            const dispatchRecords = await Promise.all(driversToDispatch.map(async (driver) => {
     ...
     */
    async sendPushNotifications(drivers, tripId) {
     ...
     */
    async acceptTrip(dispatchRecordId, providerId) {
        try {
            ...
            await db
                ...
                .where(and(eq(pettrekDispatchRecords.tripId, record.tripId), sql `${pettrekDispatchRecords.id} != ${dispatchRecordId}`));
     ...
     */
    async declineTrip(dispatchRecordId, reason, notes) {
        try {
            await db
                .update(pettrekDispatchRecords)
                .set({
    ...
    async expireOldDispatches() {
        try {
            const result = await db
                ...
                .where(and(eq(pettrekDispatchRecords.responseStatus, 'pending'), eq(pettrekDispatchRecords.isExpired, false), sql `${pettrekDispatchRecords.expiresAt} < NOW()`));
