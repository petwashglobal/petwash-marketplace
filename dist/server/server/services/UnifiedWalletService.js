...
export class UnifiedWalletService {
     ...
     * Add funds to wallet
     */
    async addFunds(userId, amount, platform, description) {
        try {
            // Ensure wallet exists
            await this.getBalance(userId);
            // Update balance and record transaction atomically
            const result = await walletRepository.updateBalanceAndRecordTransaction(userId, amount.toFixed(2), 'credit', platform, description);
     ...
     */
    async deductFunds(userId, amount, platform, description, referenceId) {
        try {
            // Update balance and record transaction atomically (includes balance check)
            const result = await walletRepository.updateBalanceAndRecordTransaction(userId, amount.toFixed(2), 'debit', platform, description, referenceId);
            const newBalance = parseFloat(result.balance.balance);
            // Publish event
            await eventBus.publish({
     ...
     */
    async transfer(fromUserId, toUserId, amount, platform, description) {
        try {
     ...
     */
    async getPlatformSpending(userId) {
        try {
