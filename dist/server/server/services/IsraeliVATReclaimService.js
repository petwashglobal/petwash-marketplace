...
export class IsraeliVATReclaimService {
     ...
     */
    static async calculateMonthlyVAT(year, month) {
     ...
     */
    static async calculateOutputVAT(periodStart, periodEnd) {
     ...
     */
    static async calculateInputVAT(periodStart, periodEnd) {
        ...
        const expenseRecords = await db
            ...
            .where(and(eq(israeliExpenses.taxYear, year), eq(israeliExpenses.taxMonth, month), eq(israeliExpenses.status, 'approved') // Only approved expenses
        ));
        // Calculate totals
     ...
     */
    static async getFirestoreRevenue(periodStart, periodEnd) {
        try {
     ...
     */
    static async getRevenueBreakdown(periodStart, periodEnd) {
        const breakdown = {
            'wash_stations': 0,
            'sitter_suite': 0,
            'walk_my_pet': 0,
     ...
     */
    static async getExpenseBreakdown(periodStart, periodEnd) {
        // Extract year and month from period
        const year = periodStart.getFullYear();
        const month = periodStart.getMonth() + 1;
        const expenseRecords = await db
            .select()
            .from(israeliExpenses)
            .where(and(eq(israeliExpenses.taxYear, year), eq(israeliExpenses.taxMonth, month), eq(israeliExpenses.status, 'approved')));
        const breakdown = {};
     ...
     */
    static async generateMonthlyDeclaration(year, month) {
        ...
        const existing = await db
            ...
            .where(and(eq(israeliVatDeclarations.taxYear, year), eq(israeliVatDeclarations.taxMonth, month)));
     ...
     */
    static async submitToTaxAuthority(declarationId, autoSubmit = false) {
        try {
     ...
     */
    static generateVATReportHTML(vatCalculation) {
        const formatCurrency = (amount) => `₪${amount.toLocaleString('he-IL', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
