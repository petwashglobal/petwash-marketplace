 ...
 */
async function geocodeLocationFallback(locationQuery) {
    try {
 ...
 */
async function getGoogleWeather(latitude, longitude, targetDate) {
    if (!GOOGLE_WEATHER_API_KEY) {
        return null;
    }
    try {
        ...
        const recommendation = generatePetWashRecommendation(tempCelsius, condition, humidity, windSpeed, uvIndex);
 ...
 */
function mapGoogleWeatherCodeToIcon(code) {
    const iconMap = {
        'clear_day': 'sun',
        'clear_night': 'moon',
        'cloudy': 'cloud',
 ...
 */
async function getOpenMeteoWeather(latitude, longitude, targetDate) {
    try {
 ...
 */
export function generateNavigationLinks(latitude, longitude, address, placeName) {
 ...
 */
function getWeatherRecommendation(weatherCode, temperature) {
 ...
 */
export async function getUnifiedLocationData(locationQuery, targetDate) {
    ...
    const navigation = generateNavigationLinks(location.latitude, location.longitude, location.formattedAddress, location.city);
    return {
        location,
        weather,
        navigation,
