 ...
 */
async function _geocodeAddressUncached(address, language = 'he') {
    try {
        ...
        const cityComponent = result.address_components?.find(c => c.types.includes('locality') || c.types.includes('administrative_area_level_1'));
 ...
 */
async function _reverseGeocodeUncached(lat, lng, language = 'he') {
    try {
        if (!GOOGLE_MAPS_API_KEY) {
            logger.warn('[MapsService] Google Maps API key not configured');
            return null;
 ...
 */
export async function searchPlaces(query, language = 'he', location, radius) {
    try {
        if (!GOOGLE_MAPS_API_KEY) {
            logger.warn('[MapsService] Google Maps API key not configured');
            return null;
 ...
 */
export async function validateAddress(address, language = 'he') {
