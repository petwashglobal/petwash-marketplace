 ...
 */
async function initializeAccountingSheets() {
 ...
 */
async function createAccountingSpreadsheet() {
 ...
 */
async function initializeAccountingHeaders(ssId) {
    const headers = {
 ...
 */
async function classifyTransactionWithAI(booking) {
    if (!ai) {
        return {
 ...
 */
export async function exportBookingsToSheets(fromDate, includeAIClassification = true) {
    const errors = [];
 ...
 */
export async function generateComplianceReport(year, month) {
    const startDate = new Date(year, month - 1, 1);
    const endDate = new Date(year, month, 0, 23, 59, 59);
    const bookings = await db.select().from(bookingRequests)
        .where(and(gte(bookingRequests.createdAt, startDate), isNotNull(bookingRequests.paymentHeldAt)));
    const completedBookings = bookings.filter(b => ['completed', 'reviewed'].includes(b.status));
    const escrowBookings = bookings.filter(b => b.paymentHeldAt && !b.paymentReleasedAt);
 ...
 */
export async function exportComplianceReport(year, month) {
    const initialized = await initializeAccountingSheets();
    if (!initialized || !spreadsheetId) {
        return {
            success: false,
...
async function getBookingCount(year, month) {
    const startDate = new Date(year, month - 1, 1);
 ...
 */
export async function exportEscrowStatus() {
    const initialized = await initializeAccountingSheets();
    if (!initialized || !spreadsheetId) {
        return {
            success: false,
