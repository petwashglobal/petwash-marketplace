...
import { providerRateCards, serviceAddons, quoteRequests } from "../../shared/schema";
...
function isIsraeliHoliday(date) {
...
function countHolidayDays(startDate, endDate) {
    let count = 0;
    const current = new Date(startDate);
    while (current < endDate) {
        if (isIsraeliHoliday(current))
            count++;
        current.setDate(current.getDate() + 1);
    }
    return count;
...
export async function calculateQuote(input) {
    const rateCard = await db
        ...
        .where(and(eq(providerRateCards.providerId, input.providerId), eq(providerRateCards.platform, input.platform), eq(providerRateCards.serviceType, input.serviceType), eq(providerRateCards.isActive, true)))
        .limit(1);
    if (!rateCard.length) {
        return null;
    }
    ...
    const petTypePricing = rate.petTypePricing || {};
    ...
    if (input.requestedAddons && input.requestedAddons.length > 0) {
        const addonPricing = rate.addonPricing || {};
...
export async function createQuoteRequest(input, customerId) {
...
export async function getAvailableAddons(platform, serviceType) {
...
export async function getProviderRateCard(providerId, platform, serviceType) {
    const rateCard = await db
        .select()
        .from(providerRateCards)
        .where(and(eq(providerRateCards.providerId, providerId), eq(providerRateCards.platform, platform), eq(providerRateCards.serviceType, serviceType)))
        .limit(1);
    return rateCard[0] || null;
}
export function formatPrice(cents, currency = 'ILS') {
