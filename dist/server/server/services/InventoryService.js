import { db } from '../db';
import { supplies, stationSupplies, inventoryRefills, stations, } from '@shared/schema';
...
export class InventoryService {
     ...
     */
    async addStationSupply(stationId, supplyId, initialLevel = 0) {
        try {
            ...
            const existing = await db
                ...
                .where(and(eq(stationSupplies.stationId, stationId), eq(stationSupplies.supplyId, supplyId)))
                .limit(1);
     ...
     */
    async updateStationSupplyLevel(stationSupplyId, newLevel, userId) {
        try {
     ...
     */
    async refillStationSupply(stationSupplyId, amount, userId, notes) {
        try {
            ...
            await eventPublisher.publishEvent(DomainEventType.INVENTORY_REFILLED, {
                stationSupplyId,
    ...
    async getLowStockStations() {
        try {
            const lowStockItems = await db
                ...
                .where(sql `${stationSupplies.currentLevel} < COALESCE(${stationSupplies.reorderThreshold}, ${supplies.reorderThreshold}, 10)`);
    ...
    async checkLowStock(stationId) {
        try {
            const lowStockItems = await db
                ...
                .from(stationSupplies)
                .leftJoin(supplies, eq(stationSupplies.supplyId, supplies.id))
                .where(and(eq(stationSupplies.stationId, stationId), sql `${stationSupplies.currentLevel} < COALESCE(${stationSupplies.reorderThreshold}, ${supplies.reorderThreshold}, 10)`));
            // Get station details
            const [station] = await db
                .select()
                .from(stations)
            ...
            for (const item of lowStockItems) {
                await eventPublisher.publishEvent(DomainEventType.INVENTORY_LOW, {
     ...
     */
    async generatePurchaseOrder(stationIds) {
        try {
            let query = db
                .select({
                stationId: stationSupplies.stationId,
            ...
            if (stationIds && stationIds.length > 0) {
                query = query.where(and(sql `${stationSupplies.currentLevel} < COALESCE(${stationSupplies.reorderThreshold}, ${supplies.reorderThreshold}, 10)`, sql `${stationSupplies.stationId} IN (${sql.join(stationIds.map(id => sql `${id}`), sql `, `)})`));
            }
