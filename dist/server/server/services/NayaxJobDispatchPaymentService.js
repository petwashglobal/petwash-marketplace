...
export class NayaxJobDispatchPaymentService {
    ...
    static DEFAULT_AUTH_EXPIRY_MINUTES = 30; // Default hold duration
    /**
     ...
     */
    static async autoVoidExpiredAuthorizations() {
        try {
            ...
            const expiringIntents = await db.select()
                .from(paymentIntents)
                .where(and(eq(paymentIntents.status, 'authorized'), lt(paymentIntents.expiresAt, new Date(Date.now() + 5 * 60 * 1000))));
            const errors = [];
            let voidedCount = 0;
    ...
    // ==================== NAYAX API CALLS ====================
    static async callNayaxAuthorizeAPI(request) {
        try {
    ...
    static async callNayaxCaptureAPI(request) {
        try {
    ...
    static async callNayaxVoidAPI(request) {
        try {
