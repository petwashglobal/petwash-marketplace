 ...
 */
import { calculateWalkFees, validateWalkFeeCalculation } from '../utils/walkFeeCalculator';
import { logger } from '../lib/logger';
import { nanoid } from 'nanoid';
// ==================== NAYAX WALK MARKETPLACE SERVICE ====================
export class NayaxWalkMarketplaceService {
    static NAYAX_API_URL = process.env.NAYAX_API_URL || process.env.NAYAX_BASE_URL || 'https://api.nayax.com/spark/v1';
    static NAYAX_API_KEY = process.env.NAYAX_API_KEY;
    static NAYAX_TERMINAL_ID = process.env.NAYAX_TERMINAL_ID;
    static CURRENCY = 'ILS'; // Israeli Shekel ONLY
    /**
     ...
     */
    static async processWalkPayment(params) {
        try {
     ...
     */
    static async processWalkerPayout(params) {
        try {
     ...
     */
    static calculateWalkTotal(basePriceCents) {
        return calculateWalkFees(basePriceCents);
    }
    /**
     * Refund walk payment (if cancelled before completion)
     */
    static async refundWalkPayment(nayaxTransactionId, amountCents) {
        try {
