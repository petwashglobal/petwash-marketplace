...
class ElectronicInvoicingService {
    ...
    determineInvoiceType(totalAmount, customerTaxId) {
        if (customerTaxId && customerTaxId.length > 0) {
            return 'B2B';
        }
        return 'B2C';
    }
    requiresElectronicInvoicing(invoiceType, totalAmount) {
    ...
    async generateComplianceReport(startDate, endDate) {
        ...
        const b2bOver25k = filtered.filter(inv => inv.invoiceType === 'B2B' && parseFloat(inv.totalAmount) >= THRESHOLD_B2B_ELECTRONIC);
        const submitted = b2bOver25k.filter(inv => inv.itaSubmissionStatus === 'submitted' || inv.itaSubmissionStatus === 'accepted');
        const pending = b2bOver25k.filter(inv => inv.itaSubmissionStatus === 'pending');
        const failed = b2bOver25k.filter(inv => inv.itaSubmissionStatus === 'error' || inv.itaSubmissionStatus === 'rejected');
        return {
            period: {
