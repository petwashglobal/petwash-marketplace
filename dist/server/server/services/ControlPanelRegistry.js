 ...
 */
export async function userHasRole(userId, roleKey, scopeType, scopeId) {
    const query = db
        .select()
        .from(userRoles)
        .innerJoin(roles, eq(userRoles.roleId, roles.id))
    ...
    if (scopeType && scopeId) {
        query.where(and(eq(userRoles.userId, userId), eq(roles.key, roleKey), eq(userRoles.scopeType, scopeType), eq(userRoles.scopeId, scopeId)));
    }
 ...
 */
export async function userHasAnyRole(userId, roleKeys) {
    const userRoleRecords = await db
        .select()
        .from(userRoles)
        .innerJoin(roles, eq(userRoles.roleId, roles.id))
 ...
 * Assign role to user
 */
export async function assignRoleToUser(userId, roleKey, scopeType, scopeId, grantedBy) {
    ...
    const existing = await db
        .select()
        .from(userRoles)
        .where(and(eq(userRoles.userId, userId), eq(userRoles.roleId, role[0].id), scopeType ? eq(userRoles.scopeType, scopeType) : eq(userRoles.scopeType, null), scopeId ? eq(userRoles.scopeId, scopeId) : eq(userRoles.scopeId, null)));
 ...
 */
export async function revokeRoleFromUser(userId, userRoleId) {
