 ...
 */
import { db } from '../db';
import { washHistory, sitterBookings, walkBookings, pettrekTrips, israeliExpenses } from '@shared/schema';
...
export class ManagementAnalyticsService {
     ...
     */
    async getComprehensiveMetrics(startDate, endDate, type) {
        try {
     ...
     */
    async getK9000Metrics(startDate, endDate, prevStartDate, prevEndDate) {
        // Current period
        const current = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${washHistory.finalPrice} AS NUMERIC)), 0)`,
            totalTransactions: sql `COUNT(*)`,
        })
            .from(washHistory)
            .where(and(gte(washHistory.createdAt, startDate), lte(washHistory.createdAt, endDate)));
        ...
        const previous = await db
            ...
            .from(washHistory)
            .where(and(gte(washHistory.createdAt, prevStartDate), lte(washHistory.createdAt, prevEndDate)));
     ...
     */
    async getSitterSuiteMetrics(startDate, endDate, prevStartDate, prevEndDate) {
        // Current period
        const current = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${sitterBookings.totalPrice} AS NUMERIC)), 0)`,
            platformCommission: sql `COALESCE(SUM(CAST(${sitterBookings.platformFee} AS NUMERIC)), 0)`,
            providerPayouts: sql `COALESCE(SUM(CAST(${sitterBookings.sitterPayout} AS NUMERIC)), 0)`,
            totalTransactions: sql `COUNT(*)`,
        })
            .from(sitterBookings)
            .where(and(gte(sitterBookings.createdAt, startDate), lte(sitterBookings.createdAt, endDate), eq(sitterBookings.status, 'completed')));
        // Previous period
        const previous = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${sitterBookings.totalPrice} AS NUMERIC)), 0)`,
            ...
            .from(sitterBookings)
            .where(and(gte(sitterBookings.createdAt, prevStartDate), lte(sitterBookings.createdAt, prevEndDate), eq(sitterBookings.status, 'completed')));
        const totalRevenue = Number(current[0]?.totalRevenue || 0);
     ...
     * Walk My Pet™ Metrics
     */
    async getWalkMyPetMetrics(startDate, endDate, prevStartDate, prevEndDate) {
        // Current period
        const current = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${walkBookings.totalPrice} AS NUMERIC)), 0)`,
            platformCommission: sql `COALESCE(SUM(CAST(${walkBookings.platformCommission} AS NUMERIC)), 0)`,
            providerPayouts: sql `COALESCE(SUM(CAST(${walkBookings.walkerPayout} AS NUMERIC)), 0)`,
            totalTransactions: sql `COUNT(*)`,
        })
            .from(walkBookings)
            .where(and(gte(walkBookings.createdAt, startDate), lte(walkBookings.createdAt, endDate), eq(walkBookings.status, 'completed')));
        // Previous period
        const previous = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${walkBookings.totalPrice} AS NUMERIC)), 0)`,
            ...
            .from(walkBookings)
            .where(and(gte(walkBookings.createdAt, prevStartDate), lte(walkBookings.createdAt, prevEndDate), eq(walkBookings.status, 'completed')));
        const totalRevenue = Number(current[0]?.totalRevenue || 0);
        const platformCommission = Number(current[0]?.platformCommission || 0);
        const providerPayouts = Number(current[0]?.providerPayouts || 0);
        const totalTransactions = Number(current[0]?.totalTransactions || 0);
     ...
     */
    async getPetTrekMetrics(startDate, endDate, prevStartDate, prevEndDate) {
        // Current period
        const current = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${pettrekTrips.finalFare} AS NUMERIC)), 0)`,
            platformCommission: sql `COALESCE(SUM(CAST(${pettrekTrips.platformCommission} AS NUMERIC)), 0)`,
            providerPayouts: sql `COALESCE(SUM(CAST(${pettrekTrips.driverPayout} AS NUMERIC)), 0)`,
            totalTransactions: sql `COUNT(*)`,
        })
            .from(pettrekTrips)
            .where(and(gte(pettrekTrips.createdAt, startDate), lte(pettrekTrips.createdAt, endDate), eq(pettrekTrips.status, 'completed')));
        // Previous period
        const previous = await db
            .select({
            totalRevenue: sql `COALESCE(SUM(CAST(${pettrekTrips.finalFare} AS NUMERIC)), 0)`,
            ...
            .from(pettrekTrips)
            .where(and(gte(pettrekTrips.createdAt, prevStartDate), lte(pettrekTrips.createdAt, prevEndDate), eq(pettrekTrips.status, 'completed')));
        const totalRevenue = Number(current[0]?.totalRevenue || 0);
        const platformCommission = Number(current[0]?.platformCommission || 0);
        const providerPayouts = Number(current[0]?.providerPayouts || 0);
        const totalTransactions = Number(current[0]?.totalTransactions || 0);
    ...
    async getExpenses(startDate, endDate) {
        const expenses = await db
            ...
            totalAmount: sql `COALESCE(SUM(CAST(${israeliExpenses.totalAmount} AS NUMERIC)), 0)`,
        })
            .from(israeliExpenses)
            .where(and(gte(israeliExpenses.createdAt, startDate), lte(israeliExpenses.createdAt, endDate), eq(israeliExpenses.status, 'approved')))
     ...
     */
    async generateForecast(services, currentPeriod) {
        try {
