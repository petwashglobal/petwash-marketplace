 ...
 */
import { db } from '../db';
import { providerPoliceChecks, providerApprovalQueue, } from '@shared/schema';
...
class PoliceCheckService {
    ...
    BADGE_VALIDITY_MONTHS = 6;
    /**
     * Submit a new police check document for verification
     */
    async submitPoliceCheck(providerId, documentUrl, documentFileName, documentIssuedAt) {
        try {
            // Check for existing pending/under_review checks
            const existingChecks = await db
                .select()
                .from(providerPoliceChecks)
                .where(and(eq(providerPoliceChecks.providerId, providerId), sql `${providerPoliceChecks.status} IN ('pending', 'under_review')`));
     ...
     */
    async submitBiometricOnboarding(providerId, documents) {
        ...
        try {
            ...
            const biometricResult = await biometricService.verifyIdentity(documents.selfieUrl, documents.idDocumentUrl);
     ...
     */
    async getProviderPoliceCheckStatus(providerId) {
        try {
     ...
     */
    async approvePoliceCheck(checkId, reviewerId, reviewNotes) {
        try {
     ...
     */
    async rejectPoliceCheck(checkId, reviewerId, rejectionReason) {
        try {
            const check = await db
                .select()
                .from(providerPoliceChecks)
     ...
     */
    async getPendingChecks(limit = 50) {
        try {
            return await db
                .select()
                .from(providerPoliceChecks)
                .where(sql `${providerPoliceChecks.status} IN ('pending', 'under_review')`)
                .orderBy(desc(providerPoliceChecks.createdAt))
                .limit(limit);
        }
        catch (error) {
     ...
     */
    async getExpiringChecks(daysBeforeExpiry = 30) {
        try {
            ...
            return await db
                .select()
                .from(providerPoliceChecks)
                .where(and(eq(providerPoliceChecks.status, 'approved'), lt(providerPoliceChecks.expiresAt, expiryThreshold)))
    ...
    async markExpiredChecks() {
        try {
            ...
            const result = await db
                ...
                .where(and(eq(providerPoliceChecks.status, 'approved'), lt(providerPoliceChecks.expiresAt, now)))
                .returning();
            if (result.length > 0) {
     ...
     */
    async verifyProviderBadge(providerId) {
        try {
