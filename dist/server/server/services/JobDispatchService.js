...
export class JobDispatchService {
     ...
     */
    static async createJobOffer(params) {
        try {
     ...
     */
    static async findNearbyOperators(params) {
        try {
            ...
            const operators = await db.select()
                ...
                .where(and(eq(operatorPresence.platform, params.platform), eq(operatorPresence.status, "online")))
            ...
            const nearbyOperators = operators.filter(op => {
                if (!op.currentLocation)
                    return false;
                const distance = this.calculateDistance(params.location.latitude, params.location.longitude, op.currentLocation.latitude, op.currentLocation.longitude);
     ...
     */
    static async notifyOperators(jobOfferId, operators, jobDetails) {
     ...
     * - Start job timer
     */
    static async acceptJobOffer(jobOfferId, operatorId) {
        try {
            ...
            const [updatedOffer] = await db.update(jobOffers)
                ...
                .where(and(eq(jobOffers.id, jobOfferId), sql `${jobOffers.status} IN ('pending', 'offered')` // CRITICAL: Only update if still available
            ))
                .returning();
     ...
     */
    static async rejectJobOffer(jobOfferId, operatorId, reason) {
        try {
            ...
            const [updatedOffer] = await db.update(jobOffers)
                ...
                .where(and(eq(jobOffers.id, jobOfferId), sql `${jobOffers.status} IN ('pending', 'offered')` // Only reject if still available
            ))
                .returning();
            if (!updatedOffer) {
     ...
     */
    static async getOperatorJobOffers(operatorId, platform) {
        try {
            const offers = await db.select()
                .from(jobOffers)
                .where(and(eq(jobOffers.platform, platform), eq(jobOffers.status, "offered")))
     ...
     */
    static async updateOperatorPresence(operatorId, platform, status, location) {
        try {
