...
export class CDPService {
    ...
    async getCustomer360(userId) {
        try {
            ...
            const profile = {
                ...
                lifecycleStage: dbProfile.lifecycleStage,
    ...
    async updateProfile(userId, updates) {
        try {
            const dbUpdates = {};
            if (updates.totalSpending !== undefined)
                dbUpdates.totalSpending = updates.totalSpending.toFixed(2);
            if (updates.totalTransactions !== undefined)
                dbUpdates.totalTransactions = updates.totalTransactions;
            if (updates.platformsUsed !== undefined)
                dbUpdates.platformsUsed = updates.platformsUsed;
            if (updates.favoriteService !== undefined)
                dbUpdates.favoriteService = updates.favoriteService;
            if (updates.loyaltyTier !== undefined)
                dbUpdates.loyaltyTier = updates.loyaltyTier;
            if (updates.lifecycleStage !== undefined)
                dbUpdates.lifecycleStage = updates.lifecycleStage;
            if (updates.firstPurchaseDate !== undefined)
                dbUpdates.firstPurchaseDate = updates.firstPurchaseDate;
            if (updates.lastActivityDate !== undefined)
                dbUpdates.lastActivityDate = updates.lastActivityDate;
            if (updates.segments !== undefined)
                dbUpdates.segments = updates.segments;
            if (updates.predictedLifetimeValue !== undefined)
                dbUpdates.predictedLifetimeValue = updates.predictedLifetimeValue.toFixed(2);
            if (updates.predictedChurnProbability !== undefined)
                dbUpdates.predictedChurnProbability = updates.predictedChurnProbability.toFixed(2);
            if (updates.location !== undefined)
                dbUpdates.location = updates.location;
            if (updates.language !== undefined)
                dbUpdates.preferredLanguage = updates.language;
     ...
     */
    async calculateChurnProbability(userId) {
        try {
     ...
     */
    async getCustomerJourney(userId, limit = 100) {
        try {
