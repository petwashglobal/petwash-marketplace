...
import { db } from "../db";
import { providers, locations } from "@shared/schema";
import { eq, and } from "drizzle-orm";
import { logger } from "../lib/logger";
const ai = new GoogleGenAI({
    apiKey: process.env.AI_INTEGRATIONS_GEMINI_API_KEY || process.env.GEMINI_API_KEY || ""
});
const SERVICE_TYPE_TO_PLATFORM = {
...
export class GeminiMatchingService {
    ...
    defaultRadius = 15;
    maxResults = 20;
    async findMatchingProviders(clientLocation, serviceRequest, radiusKm) {
        ...
        try {
            ...
            const providersWithLocations = await db
                ...
                .where(and(eq(providers.platformId, platformId), eq(providers.isActive, true), eq(providers.isAvailable, true)))
                .limit(100);
            ...
            for (const row of providersWithLocations) {
                ...
                if (location?.latitude && location?.longitude) {
                    distance = calculateDistance(clientLocation.latitude, clientLocation.longitude, location.latitude, location.longitude);
                }
                else {
            ...
            if (topMatches.length > 0) {
                aiSummary = await this.generateAIRecommendations(topMatches.slice(0, 5), serviceRequest, clientLocation);
            }
    ...
    async generateAIRecommendations(providers, request, location) {
        try {
            ...
            const providerSummary = providers.map((p, i) => `${i + 1}. ${p.displayName} - ${p.distance}km away, ${p.averageRating}★ rating, ${p.totalBookings} bookings, ${p.isVerified ? 'verified' : 'pending verification'}`).join('\n');
...
Respond in ${langName}. Write a brief, friendly 2-3 sentence recommendation explaining why the top provider(s) would be a great match. Be warm and professional. Do NOT use emojis.`;
    ...
    async findNearestProvider(clientLocation, serviceType) {
        const result = await this.findMatchingProviders(clientLocation, { serviceType: serviceType }, this.maxRadius);
        if (result.success && result.matches.length > 0) {
            return result.matches[0];
        }
        return null;
    }
    async searchWithAI(query, clientLocation, preferredLanguage = 'en') {
        try {
...
If unclear, respond with: pet_sitting`;
            ...
            const detectedService = (response.text || 'pet_sitting').trim().toLowerCase();
            ...
            return this.findMatchingProviders(clientLocation, { serviceType, preferredLanguage, notes: query });
        }
        catch (error) {
            logger.error('[GeminiMatching] AI search failed', { error });
            return this.findMatchingProviders(clientLocation, { serviceType: 'pet_sitting', preferredLanguage });
        }
    }
}
