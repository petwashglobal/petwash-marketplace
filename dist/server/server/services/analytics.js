 ...
 */
async function getRevenueForRange(startDate, endDate, stationId) {
    try {
        let query = db.collection('nayax_transactions')
 ...
 * FIXED: Now accepts optional stationId filter
 */
async function getTransactionCountForRange(startDate, endDate, status, stationId) {
    try {
        let query = db.collection('nayax_transactions')
            .where('createdAt', '>=', startDate)
            .where('createdAt', '<=', endDate);
 ...
 */
async function getActiveCustomersForRange(startDate, endDate) {
    try {
 ...
 */
async function getNewCustomersForRange(startDate, endDate) {
    try {
...
export async function getAnalyticsOverview() {
    ...
    try {
        ...
        const [
        // Revenue metrics
        revenueToday, revenueThisWeek, revenueThisMonth, revenueLastMonth, revenueThisYear, 
        // Customer metrics
        totalCustomersSnapshot, activeCustomersThisMonth, newCustomersThisMonth, newCustomersLastMonth, 
        // Transaction metrics
        totalTransactionsSnapshot, completedTransactionsSnapshot, pendingTransactionsSnapshot, failedTransactionsSnapshot, 
        // Station metrics
        allStationsSnapshot, 
        // Loyalty metrics
        loyaltySnapshot,] = await Promise.all([
            // Revenue
        ...
        // Process stations
        const stations = allStationsSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
        }));
...
export async function getStationPerformanceAnalytics() {
    ...
    try {
        ...
        const performanceData = await Promise.all(stationsSnapshot.docs.map(async (doc) => {
