 ...
 */
export async function performWeeklyCodeBackup() {
    ...
    try {
        ...
        await execAsync(`tar -czf ${localPath} \
        ...
        -C ${process.cwd()} .`, { maxBuffer: 1024 * 1024 * 100 } // 100MB buffer
        );
 ...
 */
export async function performFirestoreExport() {
    const startTime = Date.now();
    const date = new Date().toISOString().split('T')[0];
