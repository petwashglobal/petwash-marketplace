 ...
 */
import { db } from '../db';
import { partners, partnerAgreements, settlements, nayaxTransactions } from '@shared/schema';
...
export class FinanceSettlementService {
     ...
     */
    static async generateSettlement(partnerId, periodStart, periodEnd, periodType = 'monthly') {
        try {
            ...
            const existingSettlement = await db.query.settlements.findFirst({
                where: and(eq(settlements.partnerId, partnerId), eq(settlements.periodStart, periodStart), eq(settlements.periodEnd, periodEnd)),
            });
     ...
     */
    static async calculateRevenueShare(partnerId, periodStart, periodEnd) {
        try {
            // 1. Get partner's active agreements
            const agreements = await db.query.partnerAgreements.findMany({
                where: and(eq(partnerAgreements.partnerId, partnerId), eq(partnerAgreements.isActive, true), lte(partnerAgreements.startDate, periodEnd)),
            });
            ...
            if (stationIds.length > 0) {
                const nayaxResults = await db
                    .select({
                    totalAmount: sql `COALESCE(SUM(CAST(${nayaxTransactions.amount} AS DECIMAL)), 0)`,
                    count: sql `COUNT(*)`,
                })
                    .from(nayaxTransactions)
                    .where(and(sql `${nayaxTransactions.stationId}::integer = ANY(${stationIds})`, gte(nayaxTransactions.createdAt, periodStart), lte(nayaxTransactions.createdAt, periodEnd), eq(nayaxTransactions.status, 'completed')));
                if (nayaxResults[0]) {
            ...
            const maxSharePercent = Math.max(...agreements.map(a => parseFloat(a.revenueSharePercent || '0')));
     ...
     */
    static async approveSettlement(settlementId, userId) {
        try {
     ...
     */
    static async markSettlementPaid(settlementId, paymentReference) {
        try {
            const [updatedSettlement] = await db
                .update(settlements)
                .set({
     ...
     */
    static async exportSettlementToCSV(settlementId) {
        try {
    ...
    static async getPartnerDashboard(partnerId) {
        try {
            ...
            const currentPeriodRevenue = await this.calculateRevenueShare(partnerId, periodStart, periodEnd);
            ...
            const lifetimeMetrics = await db
                .select({
                totalRevenue: sql `COALESCE(SUM(CAST(${settlements.partnerShare} AS DECIMAL)), 0)`,
                totalPaid: sql `COALESCE(SUM(CASE WHEN ${settlements.status} = 'paid' THEN CAST(${settlements.partnerShare} AS DECIMAL) ELSE 0 END), 0)`,
                totalPending: sql `COALESCE(SUM(CASE WHEN ${settlements.status} IN ('pending', 'approved') THEN CAST(${settlements.partnerShare} AS DECIMAL) ELSE 0 END), 0)`,
            })
                .from(settlements)
                .where(eq(settlements.partnerId, partnerId));
            return {
     ...
     */
    static async generateSettlementNumber(periodStart) {
     ...
     */
    static calculateAuditHash(data) {
     ...
     */
    static verifySettlementIntegrity(settlement) {
