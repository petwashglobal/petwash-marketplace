 ...
 */
export async function analyzeWeatherForAlerts(location, targetAudience) {
    try {
        ...
        if (weather.temperature < 5 || weather.temperature > 35) {
            return {
                ...
                message: `Extreme temperature (${weather.temperature}°C) in ${geocoded.city}. ${weather.temperature < 5
 ...
 */
export async function canSendNotification(userId, notificationType) {
    try {
        ...
        const lastNotification = await db.query.notificationHistory.findFirst({
            where: and(eq(db.query.notificationHistory.userId, userId), eq(db.query.notificationHistory.type, notificationType), gte(db.query.notificationHistory.sentAt, new Date(Date.now() - NOTIFICATION_COOLDOWN_HOURS * 60 * 60 * 1000))),
        ...
        const hoursSinceLastNotification = (Date.now() - new Date(lastNotification.sentAt).getTime()) / (1000 * 60 * 60);
 ...
 */
export async function sendWeatherNotification(alert, location) {
    try {
 ...
 */
export async function checkWeatherForActiveLocations() {
    try {
 ...
 */
export async function sendShiftWeatherForecast(userId, shiftStartTime, location, userType) {
    try {
