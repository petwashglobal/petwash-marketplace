import { db } from '../db';
import { bookings, bookingPets, bookingItems, availabilitySlots, superAppPayouts, platforms, providers, pets, stations } from '@shared/schema';
import { eq, and, or, gte, lte, sql, inArray } from 'drizzle-orm';
import { nanoid } from 'nanoid';
import { unifiedBookingFacade } from './booking-facade';
import { logger } from '../lib/logger';
export class BookingService {
    /**
     * Normalize timezone-aware date to UTC for database storage
     */
    normalizeToUTC(dateInput, timezone = 'Asia/Jerusalem') {
     ...
     */
    validateBookingTimes(startTime, endTime, platformId) {
     ...
     */
    async verifyProvider(providerId, platformId) {
        const [provider] = await db
            .select()
            .from(providers)
            .where(and(eq(providers.id, providerId), eq(providers.platformId, platformId)));
        if (!provider) {
     ...
     */
    async verifyStation(stationId, platformId) {
     ...
     */
    async verifyPetsOwnership(petIds, userId) {
        const userPets = await db
            .select()
            .from(pets)
            .where(and(inArray(pets.id, petIds), eq(pets.userId, userId), eq(pets.isActive, true)));
     ...
     */
    async generateBookingNumber(platformId) {
        const date = new Date();
     ...
     */
    async checkAvailability(platformId, providerId, stationId, startTime, endTime, excludeBookingId) {
        ...
        if (stationId && platformId === 'k9000') {
            const conflictingBookings = await db
                .select()
                .from(bookings)
                .where(and(eq(bookings.stationId, stationId), eq(bookings.platformId, platformId), or(...blockingStatuses.map(status => eq(bookings.status, status))), or(
            // New booking overlaps with existing
            and(lte(bookings.startTime, startTime), gte(bookings.endTime, startTime)), and(lte(bookings.startTime, endTime), gte(bookings.endTime, endTime)), and(gte(bookings.startTime, startTime), lte(bookings.endTime, endTime))), excludeBookingId ? sql `${bookings.id} != ${excludeBookingId}` : sql `1=1`));
        ...
        if (providerId && platformId !== 'k9000') {
            const conflictingBookings = await db
                .select()
                .from(bookings)
                .where(and(eq(bookings.providerId, providerId), eq(bookings.platformId, platformId), or(...blockingStatuses.map(status => eq(bookings.status, status))), or(and(lte(bookings.startTime, startTime), gte(bookings.endTime, startTime)), and(lte(bookings.startTime, endTime), gte(bookings.endTime, endTime)), and(gte(bookings.startTime, startTime), lte(bookings.endTime, endTime))), excludeBookingId ? sql `${bookings.id} != ${excludeBookingId}` : sql `1=1`));
            if (conflictingBookings.length > 0) {
                return {
                    hasConflict: true,
                    conflictingBookings,
            ...
            const availableSlots = await db
                ...
                .where(and(eq(availabilitySlots.providerId, providerId), eq(availabilitySlots.platformId, platformId), eq(availabilitySlots.status, 'available'), lte(availabilitySlots.startTime, startTime), gte(availabilitySlots.endTime, endTime)));
     ...
     */
    async calculatePricing(platform, subtotal, discountAmount = 0) {
    ...
    async createBooking(input) {
        ...
        const availabilityCheck = await this.checkAvailability(input.platformId, input.providerId, input.stationId, startTime, endTime);
        ...
        if (useFacade) {
            ...
            const facadeAvailability = await unifiedBookingFacade.checkAvailability(platformId, {
        ...
        if (useFacade) {
            const pricingParams = {
                ...
                endDate: endTime,
                userId: input.userId,
                ipAddress: input.platformData?.ip || '0.0.0.0',
                metadata: {
        ...
        if (input.petIds && input.petIds.length > 0) {
            await db.insert(bookingPets).values(input.petIds.map(petId => ({
        ...
        if (input.items && input.items.length > 0) {
            await db.insert(bookingItems).values(input.items.map(item => ({
                bookingId: booking.id,
        ...
        if (useFacade) {
            try {
                const confirmation = await unifiedBookingFacade.confirmBooking(platformId, booking.id, pricingBreakdown, input.userId);
     ...
     */
    async updateBookingStatus(bookingId, newStatus, updatedBy, reason) {
        ...
        if (!allowedStatuses.includes(newStatus)) {
            throw new Error(`Invalid status transition: ${existingBooking.status} → ${newStatus}`);
        }
     ...
     */
    async createDraftBooking(input) {
