 ...
 */
class WalkAvailabilityStrategy {
    async checkAvailability(params) {
        try {
            ...
            const overlappingBookings = await db
                .select()
                .from(walkBookings)
                .where(and(eq(walkBookings.walkerId, walkerId), gte(walkBookings.scheduledDate, params.startDate), lte(walkBookings.scheduledDate, params.endDate), eq(walkBookings.status, 'confirmed')));
 ...
 */
class WalkPostConfirmationStrategy {
...
export class WalkEliteBookingEngine extends BaseLuxuryBookingEngine {
    constructor() {
        super(new WalkAvailabilityStrategy(), new WalkPricingStrategy(), new WalkPostConfirmationStrategy());
    }
    /**
     * Find nearby walkers with GPS proximity
     * Enhanced feature for "Find Walker" flow
     */
    async findNearbyWalkers(latitude, longitude, maxDistance = 5 // km
    ) {
        try {
            ...
            const walkers = await db
                .select()
                .from(walkerProfiles)
                .where(and(eq(walkerProfiles.isOnline, true), eq(walkerProfiles.isAvailable, true)));
            ...
            const walkersWithDistance = walkers
                ...
                .map((walker) => {
                const distance = this.calculateDistance(latitude, longitude, parseFloat(walker.lastKnownLatitude), parseFloat(walker.lastKnownLongitude));
                return {
                    ...walker,
                    distance,
