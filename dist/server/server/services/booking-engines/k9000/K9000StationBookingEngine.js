 ...
 */
class K9000AvailabilityStrategy {
 ...
 */
class K9000PostConfirmationStrategy {
...
export class K9000StationBookingEngine extends BaseLuxuryBookingEngine {
    constructor() {
        super(new K9000AvailabilityStrategy(), new K9000PricingStrategy(), new K9000PostConfirmationStrategy());
    }
    /**
     * Find nearest available station
     * GPS-based proximity search
     */
    async findNearestStation(latitude, longitude, maxDistance = 10 // km
    ) {
        try {
            ...
            const allStations = await db
                .select()
                .from(stations)
                .where(and(eq(stations.status, 'online')));
            // Calculate distance for each station
            const stationsWithDistance = allStations
                .filter((station) => station.latitude && station.longitude)
                .map((station) => {
                const distance = this.calculateDistance(latitude, longitude, parseFloat(station.latitude), parseFloat(station.longitude));
                return {
                    ...station,
                    distance,
