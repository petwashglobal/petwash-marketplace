 ...
 */
import { db } from '../db';
import { providerTaxCompliance, providerCommissions, providerIndependenceScore, complianceVerificationLogs, } from '@shared/schema';
...
export class IsraeliContractorComplianceService {
     ...
     */
    static async verifyTaxRegistration(providerId, providerType, taxData) {
        try {
            ...
            const verificationStatus = issues.length > 0 ? 'rejected' : 'verified';
            // Calculate risk level
            const riskLevel = this.calculateRiskLevel(issues, taxData);
            // Save to database
            ...
            // Log verification
            await this.logComplianceCheck(providerId, providerType, 'tax_registration', verificationStatus === 'verified' ? 'passed' : 'failed', { issues, recommendations });
     ...
     */
    static async calculateCommission(providerId, providerType, bookingId, customerPaidAmount, commissionRate = 20 // Default 20%
    ) {
        try {
     ...
     */
    static async calculateIndependenceScore(providerId, providerType, metrics) {
        try {
            ...
            await this.logComplianceCheck(providerId, providerType, 'independence_check', riskLevel === 'high' ? 'warning' : 'passed', {
                riskScore,
                riskLevel,
                recommendations
            });
     ...
     */
    static async runMonthlyAudit(providerId, providerType) {
        try {
            ...
            // Log audit results
            await this.logComplianceCheck(providerId, providerType, 'monthly_audit', passed ? 'passed' : 'failed', { issues, warnings });
     ...
     */
    static calculateWithholdingTax(grossPayment, hasExemptionCertificate = false, exemptionPercentage = 0, taxAuthorityCertificateId, validUntil, providerWithholdingRate // Provider-specific rate from certificate (0-50%)
    ) {
     ...
     */
    static calculateNationalInsurance(monthlyIncome) {
     ...
     */
    static async checkVatThreshold(providerId) {
        ...
        try {
            ...
            const earnings = await db.select({
                totalEarnings: sql `COALESCE(SUM(CAST(${providerCommissions.providerEarnings} AS NUMERIC)), 0)`,
                bookingCount: sql `COUNT(*)`,
            }).from(providerCommissions)
                .where(and(eq(providerCommissions.providerId, providerId), gte(providerCommissions.createdAt, yearStart), lte(providerCommissions.createdAt, yearEnd)));
            ...
            if (recommendUpgrade && !mustRegisterForVat) {
                ...
                await this.logComplianceCheck(providerId, 'sitter', // Default type, should be fetched from provider
                'vat_threshold_warning', 'warning', {
                    currentYearRevenue,
     ...
     */
    static async calculatePayoutTaxObligations(providerId, grossPayoutAmount, providerTaxInfo) {
        logger.info('[Israeli Compliance 2025] Calculating payout tax obligations', {
            providerId,
            grossPayoutAmount
        });
        // Calculate withholding tax
        const withholding = this.calculateWithholdingTax(grossPayoutAmount, providerTaxInfo?.hasWithholdingExemption || false, providerTaxInfo?.exemptionPercentage || 0, providerTaxInfo?.certificateId, providerTaxInfo?.certificateValidUntil);
    ...
    // HELPER METHODS
    // ============================================================================
    static calculateRiskLevel(issues, taxData) {
        if (issues.length > 0)
            return 'high';
        if (!taxData.isVatRegistered && taxData.taxIdType === 'osek_murshe')
            return 'medium';
        return 'low';
    }
    static async logComplianceCheck(providerId, providerType, verificationType, checkStatus, findings) {
     ...
     */
    static async getComplianceSummary(providerId) {
        try {
            const [tax, independence, commissions, logs] = await Promise.all([
                ...
                db.select({
                    total: sql `COUNT(*)`,
                    totalEarnings: sql `SUM(CAST(${providerCommissions.providerEarnings} AS NUMERIC))`,
