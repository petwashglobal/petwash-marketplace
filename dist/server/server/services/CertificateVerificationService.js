...
import { db } from '../db';
import { biometricCertificateVerifications, users, approvedCountries } from '@shared/schema';
...
export class CertificateVerificationService {
     ...
     */
    async verifyDocument(userId, documentType, documentCountry, uploadedFiles, ipAddress, userAgent) {
        try {
            ...
            const biometricResult = await biometricVerification.verifyIdentity(uploadedFiles.selfieUrl, uploadedFiles.frontUrl);
            // Step 3: Determine verification status
            const requiresManualReview = this.requiresManualReview(documentType, documentCountry, biometricResult.matchScore, ocrResult.confidence);
            const verificationStatus = this.determineStatus(biometricResult.isMatch, requiresManualReview, documentType);
     ...
     */
    async validateApprovedCountry(countryCode, documentType) {
        try {
     ...
     */
    requiresManualReview(documentType, country, biometricScore, ocrConfidence) {
     ...
     */
    determineStatus(biometricMatch, requiresManualReview, documentType) {
     ...
     */
    async updateUserProfile(userId, documentType) {
        const updates = {};
     ...
     */
    getStatusMessage(status, documentType, biometricScore) {
        const docTypeNames = {
     ...
     */
    async approveVerification(verificationId, adminUid, notes) {
        ...
        await db
            ...
            .where(eq(biometricCertificateVerifications.id, verificationId));
        // Update user profile
        await this.updateUserProfile(verification.userId, verification.documentType);
     ...
     */
    async rejectVerification(verificationId, adminUid, reason) {
        const [verification] = await db
            .select()
            .from(biometricCertificateVerifications)
            .where(eq(biometricCertificateVerifications.id, verificationId));
