import { db } from '../db';
import { walletAccounts, creditTransactions, redemptionSessions } from '@shared/schema';
...
class WalletService {
    ...
    async getWalletSummary(userId) {
        ...
        return {
            ...
            totalCreditsValueCents: (wallet.egiftBalanceCents || 0) +
    ...
    async previewCredits(userId, requestedAmountCents, platform) {
        const wallet = await this.getOrCreateWallet(userId);
        ...
        if (platform === 'k9000' && (wallet.washPackageCredits || 0) > 0) {
            ...
            if (remainingAmount > 0 && (wallet.loyaltyPointsBalance || 0) > 0) {
                ...
                loyaltyPointsApplicable = Math.min(Math.min(loyaltyValueAvailableCents, remainingAmount), maxLoyaltyDiscount);
    ...
    async createRedemptionSession(userId, platform, requestedAmountCents, serviceType, bookingId, stationId, deviceInfo) {
        const wallet = await this.getOrCreateWallet(userId);
    ...
    async confirmRedemption(sessionId, paymentConfirmed = false, idempotencyKey) {
    ...
    async refundRedemption(sessionId, reason, initiatedBy = 'system') {
    ...
    async addCredits(userId, creditType, amount, sourceType, sourceId, description) {
        const wallet = await this.getOrCreateWallet(userId);
    ...
    async validateRedemptionCode(code, stationId) {
        const [session] = await db.select()
            .from(redemptionSessions)
            .where(and(eq(redemptionSessions.redemptionCode, code), eq(redemptionSessions.status, 'code_generated')))
            .limit(1);
        if (!session)
            return null;
    ...
    async acknowledgeHardwareRedemption(sessionId) {
        const [session] = await db.select()
            .from(redemptionSessions)
            .where(eq(redemptionSessions.sessionId, sessionId))
            .limit(1);
    ...
    async adminInjectCredits(params) {
        const { adminUserId, adminEmail, targetUserId, creditType, amount, reason, expiresAt, ticketId, approvalReference, ipAddress, userAgent, } = params;
        if (amount <= 0) {
...
=======================================================`);
     ...
     */
    async getAdminInjectionHistory(userId) {
        const wallet = await this.getOrCreateWallet(userId);
        return await db.select()
            .from(creditTransactions)
            .where(and(eq(creditTransactions.walletId, wallet.walletId), eq(creditTransactions.sourceType, 'admin_injection')))
            .orderBy(desc(creditTransactions.createdAt));
    }
    /**
    ...
    async processExpiredCredits() {
        ...
        const expiredTransactions = await db.select()
            .from(creditTransactions)
            .where(and(sql `${creditTransactions.expiresAt} IS NOT NULL`, sql `${creditTransactions.expiresAt} < ${now}`, eq(creditTransactions.transactionType, 'issue')));
        const walletRunningBalances = new Map();
        for (const txn of expiredTransactions) {
            const hasBeenExpired = await db.select()
                .from(creditTransactions)
                .where(and(eq(creditTransactions.sourceId, txn.transactionId), eq(creditTransactions.transactionType, 'expire')))
                .limit(1);
            if (hasBeenExpired.length > 0)
                continue;
            const originalAmount = txn.amountCents || 0;
            const netUsageResult = await db.select({
                totalRedeemed: sql `COALESCE(SUM(CASE WHEN ${creditTransactions.transactionType} = 'redeem' THEN ABS(${creditTransactions.amountCents}) ELSE 0 END), 0)`,
                totalRefunded: sql `COALESCE(SUM(CASE WHEN ${creditTransactions.transactionType} = 'refund' THEN ${creditTransactions.amountCents} ELSE 0 END), 0)`,
                totalReleased: sql `COALESCE(SUM(CASE WHEN ${creditTransactions.transactionType} = 'release' THEN ${creditTransactions.amountCents} ELSE 0 END), 0)`
            })
                .from(creditTransactions)
                .where(and(eq(creditTransactions.walletId, txn.walletId), eq(creditTransactions.creditType, txn.creditType), sql `${creditTransactions.transactionType} IN ('redeem', 'refund', 'release')`, sql `${creditTransactions.createdAt} >= ${txn.createdAt}`, sql `${creditTransactions.createdAt} <= ${txn.expiresAt}`));
     ...
     */
    async checkDormantWallets() {
        const twelveMonthsAgo = new Date();
        twelveMonthsAgo.setMonth(twelveMonthsAgo.getMonth() - 12);
        const dormantWallets = await db.select()
            .from(walletAccounts)
            .where(and(sql `${walletAccounts.lastActivityAt} < ${twelveMonthsAgo}`, eq(walletAccounts.isActive, true), sql `(${walletAccounts.egiftBalanceCents} > 0 OR ${walletAccounts.promoBalanceCents} > 0 OR ${walletAccounts.referralBalanceCents} > 0)`));
        let atRiskCredits = 0;
     ...
     */
    async getExpiringCredits(userId, daysAhead = 30) {
        const wallet = await this.getOrCreateWallet(userId);
        const futureDate = new Date();
        futureDate.setDate(futureDate.getDate() + daysAhead);
        const expiringTxns = await db.select()
            .from(creditTransactions)
            .where(and(eq(creditTransactions.walletId, wallet.walletId), eq(creditTransactions.transactionType, 'issue'), sql `${creditTransactions.expiresAt} IS NOT NULL`, sql `${creditTransactions.expiresAt} > NOW()`, sql `${creditTransactions.expiresAt} <= ${futureDate}`))
