...
export class WalletTelemetryService {
    static COLLECTION = 'wallet_telemetry';
     ...
     */
    static async recordBeacon(beacon) {
        try {
     ...
     */
    static async recordDownload(event) {
        try {
    ...
    static inferSuccess(session) {
        ...
        const hasPostClickBeacon = session.beacons.some(b => b.type === 'apple-post-2s' || b.type === 'google-post-2s');
        ...
        const lastActivity = Math.max(session.clickedAt?.getTime() || 0, session.downloadedAt?.getTime() || 0, ...session.beacons.map(b => b.timestamp.getTime()));
     ...
     */
    static isApplePassbookInstaller(ua) {
        if (!ua)
            return false;
        const uaLower = ua.toLowerCase();
        // iOS 6-8 used "Passbook"
        // iOS 9+ uses "Wallet" or "PassKit"
        return (uaLower.includes('passbook') ||
