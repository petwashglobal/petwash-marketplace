...
export async function issueBadge(params) {
    try {
        const { contractorId, badgeType, badgeCategory, badgeName, description, iconUrl, expiresAt, metadata, issuedBy, } = params;
        // Check if badge already exists
        const existingBadges = await db
            .select()
            .from(contractorBadges)
            .where(and(eq(contractorBadges.contractorId, contractorId), eq(contractorBadges.badgeCategory, badgeCategory)));
 ...
 */
export async function checkAndAwardMilestoneBadges(contractorId) {
    try {
        // Get contractor info
 ...
 */
export async function issueCertificationBadge(contractorId, certType, certNumber, expiresAt, issuedBy) {
    try {
        const badgeNames = {
 ...
 */
export async function revokeBadge(badgeId, reason) {
    try {
        await db
 ...
 */
export async function deactivateExpiredCertifications() {
    try {
        const now = new Date();
        ...
        const expiredBadges = await db
            .select()
            .from(contractorBadges)
            .where(and(eq(contractorBadges.badgeType, 'certification'), eq(contractorBadges.isActive, true)));
 ...
 */
export async function getContractorBadges(contractorId) {
    try {
        const badges = await db
            .select()
            .from(contractorBadges)
            .where(and(eq(contractorBadges.contractorId, contractorId), eq(contractorBadges.isActive, true)));
        return badges;
    }
    catch (error) {
 ...
 */
export async function awardBackgroundCheckBadges(contractorId) {
    try {
        const [contractor] = await db
            .select()
            .from(providerApplications)
