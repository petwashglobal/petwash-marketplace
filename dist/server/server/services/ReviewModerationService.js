...
import { reviewModeration, reviewModerationRules, } from "@shared/schema-compliance";
...
export class ReviewModerationService {
     ...
     */
    async moderateReview(submission) {
     ...
     */
    async runAIModeration(reviewText, rules) {
        if (!this.geminiAI) {
...
Return ONLY a valid JSON object with this exact structure:
{
  ...
  "requiresLegalReview": boolean
     ...
     */
    async runKeywordModeration(reviewText, rules) {
        ...
        for (const rule of rules) {
            ...
            const keywordsHe = rule.keywordsHe || [];
     ...
     */
    mapIssusToRules(issues, rules) {
        const flaggedRules = [];
        for (const issue of issues) {
            const matchingRule = rules.find(r => r.ruleType.toLowerCase().includes(issue.toLowerCase()) ||
     ...
     */
    async approveReview(moderationId, reviewedBy, moderatorNotes) {
     ...
     */
    async rejectReview(moderationId, reviewedBy, moderatorNotes) {
        const [updated] = await db
            .update(reviewModeration)
            .set({
            status: "rejected",
     ...
     */
    async escalateToLegal(moderationId, reviewedBy, legalNotes) {
        const [updated] = await db
            .update(reviewModeration)
            .set({
            status: "escalated",
     ...
     */
    async addProviderResponse(moderationId, responseText) {
        const [updated] = await db
            .update(reviewModeration)
            .set({
    ...
    async getPendingReviews(limit = 50) {
        const pending = await db
            ...
            .where(or(eq(reviewModeration.status, "pending"), eq(reviewModeration.status, "flagged")))
            .limit(limit);
        return pending;
    }
    /**
     ...
     */
    async getReviewsForLegal(limit = 50) {
        const forLegal = await db
            .select()
            .from(reviewModeration)
            .where(and(eq(reviewModeration.legalReviewRequired, true), or(eq(reviewModeration.status, "flagged"), eq(reviewModeration.status, "escalated"))))
            .limit(limit);
        return forLegal;
    }
}
