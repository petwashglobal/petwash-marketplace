 ...
 */
import { db } from '../db';
import { userDevices, userDeviceEvents, } from '@shared/schema';
...
export class UserDeviceService {
     ...
     */
    static generateFingerprint(telemetry) {
     ...
     */
    static decryptWifiSsid(encrypted) {
        try {
            const secret = process.env.KYC_SALT;
            if (!secret) {
     ...
     */
    static async calculateFraudScore(userId, telemetry, existingDevices) {
        const signals = [];
        let score = 0;
        ...
        if (telemetry.ipLocation && existingDevices.length > 0) {
            ...
            const lastLocation = lastDevice.ipLocation;
     ...
     */
    static async registerDevice(telemetry) {
        ...
        const { score: fraudScore, signals: fraudSignals } = await this.calculateFraudScore(telemetry.userId, telemetry);
     ...
     */
    static async revokeDevice(deviceId, userId, reason = 'user_dismissed', ipAddress) {
        try {
            const [device] = await db
                .update(userDevices)
                .set({
                ...
                .where(and(eq(userDevices.id, deviceId), eq(userDevices.userId, userId), isNull(userDevices.revokedAt) // Can't revoke already revoked device
            ))
                .returning();
            if (!device) {
                return false;
     ...
     */
    static async updateDeviceLabel(deviceId, userId, newLabel) {
        try {
            const [device] = await db
                .update(userDevices)
                .set({
     ...
     */
    static async recordDeviceEvent(event) {
        try {
     ...
     */
    static async getDeviceEvents(deviceId, limit = 50) {
        return await db
            .select()
     ...
     */
    static async getSuspiciousDevices(userId) {
        const query = db
            .select()
            .from(userDevices)
            .where(and(sql `${userDevices.trustScore} < 40 OR array_length(${userDevices.fraudFlags}, 1) > 2`, isNull(userDevices.revokedAt)))
     ...
     */
    static async markAsCurrentDevice(deviceId, userId) {
        try {
