...
import { healthSafetyIncidents, incidentPhotos } from '@shared/schema';
...
export class HealthSafetyService {
     ...
     */
    static async generateIncidentNumber() {
        try {
     ...
     */
    static async reportIncident(params, photos) {
        try {
            ...
            await eventPublisher.publishEvent('INCIDENT_REPORTED', {
                incidentId: incident.id,
                incidentNumber: incident.incidentNumber,
     ...
     */
    static async uploadPhotos(incidentId, uploadedByUserId, files) {
        try {
     ...
     */
    static async uploadIncidentPhoto(incidentId, uploadedByUserId, file) {
        try {
    ...
    static async listIncidents(filters = {}) {
        try {
            ...
            const incidentsWithPhotos = await Promise.all(incidents.map(async (incident) => {
                const photos = await db
                    .select()
                    .from(incidentPhotos)
     ...
     */
    static async assignIncident(incidentId, assignedToUserId) {
        try {
            const [updated] = await db
                .update(healthSafetyIncidents)
                .set({
     ...
     * Resolve incident
     */
    static async resolveIncident(incidentId, resolutionNotes, resolvedByUserId) {
        try {
            const [incident] = await db
                .update(healthSafetyIncidents)
                .set({
            ...
            await eventPublisher.publishEvent('INCIDENT_RESOLVED', {
                incidentId: incident.id,
                incidentNumber: incident.incidentNumber,
                stationId: incident.stationId,
                severity: incident.severity,
     ...
     */
    static async getIncidentDashboard() {
        try {
            ...
            const resolvedIncidents = allIncidents.filter(i => i.resolvedAt && i.reportedAt);
