 ...
 */
import { db } from '../db';
import { walkBookings } from '@db/schema';
...
export class EmergencyWalkService {
     ...
     */
    static async requestEmergencyWalk(request) {
        try {
            ...
            const surgePricing = await this.calculateSurgePricing(request.walkDuration, request.location);
            ...
            const availableWalkers = await this.findAvailableWalkers(request.location, request.walkDuration);
            ...
            estimatedStartTime.setMinutes(estimatedStartTime.getMinutes() + matchedWalker.estimatedArrivalMinutes);
            const estimatedEndTime = new Date(estimatedStartTime);
            estimatedEndTime.setMinutes(estimatedEndTime.getMinutes() + request.walkDuration);
     ...
     * - Holidays: +0.3x
     */
    static async calculateSurgePricing(durationMinutes, location) {
     ...
     */
    static async findAvailableWalkers(location, requiredDuration) {
     ...
     */
    static selectBestWalker(walkers) {
     ...
     */
    static async sendEmergencyNotifications(params) {
...
- צוות Walk My Pet™
     ...
     */
    static async getActiveWalksInArea(location, radiusKm) {
     ...
     */
    static async getAvailableWalkersInArea(location, radiusKm) {
        // TODO: Query database with geospatial index
        // For now, return mock data
