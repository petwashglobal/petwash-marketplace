 ...
 */
export async function checkInsuranceExpirations() {
    try {
        const now = new Date();
        const alerts = [];
        ...
        const contractors = await db
            ...
            .where(and(eq(providerApplications.status, 'approved'), sql `${providerApplications.insuranceExpiresAt} IS NOT NULL`));
        for (const contractor of contractors) {
            if (!contractor.insuranceExpiresAt)
                continue;
 ...
 */
export async function sendInsuranceAlerts(alerts) {
    try {
 ...
 */
export async function suspendExpiredContractors() {
    try {
        const now = new Date();
        const suspended = [];
        ...
        const expiredContractors = await db
            .select()
            .from(providerApplications)
            .where(and(eq(providerApplications.status, 'approved'), lt(providerApplications.insuranceExpiresAt, now)));
 ...
 */
export async function verifyInsurancePolicy(contractorId, verified, notes) {
    try {
        const now = new Date();
        await db
            .update(providerApplications)
 ...
 */
export async function runDailyInsuranceMonitoring() {
    try {
