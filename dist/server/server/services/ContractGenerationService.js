import { db } from '../db';
import { contractTemplates, generatedContracts, legalEntities } from '@shared/schema';
...
export class ContractGenerationService {
    ...
    async loadTemplate(templateId) {
        ...
        const template = await db
            ...
            .where(and(eq(contractTemplates.templateId, templateId), eq(contractTemplates.isActive, true)))
            .limit(1)
            .then(rows => rows[0]);
        if (!template) {
    ...
    async generateOfferLetter(data) {
        ...
        if (data.country === 'US')
            templateId = 'TMPL-EMP-FT-US-001';
        else if (data.country === 'CA')
            templateId = 'TMPL-EMP-FT-CA-001';
        else if (data.country === 'UK')
            templateId = 'TMPL-EMP-FT-UK-001';
        else if (data.country === 'AU')
            templateId = 'TMPL-EMP-FT-AU-001';
    ...
    async generateContractorAgreement(data) {
        ...
        if (data.contractorType === 'sitter')
            templateFile = 'contractor-sitter-agreement.md';
    ...
    getContractorClassification(country) {
    ...
    async sendForSignature(contractId, docusealApiKey) {
        const contract = await db
            .select()
            .from(generatedContracts)
