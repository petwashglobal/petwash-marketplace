...
import { db } from '../db';
import { superAppPayouts, bookings, providers, bookingPhotos, domainEvents } from '@shared/schema';
...
export class AIPayoutVerificationService {
     ...
     */
    static async verifyWorkForPayout(payoutId) {
        ...
        try {
            ...
            const evidence = {
                ...
                expectedLocation: booking?.serviceLocation,
                photos: photos.map(p => ({
                    url: p.photoUrl,
     ...
     */
    static async performVerification(evidence, verificationId) {
        ...
        const confidenceScore = Math.round(scores.reduce((sum, s) => sum + s, 0) / scores.length);
        const issues = [];
     ...
     */
    static async verifyPhotoAuthenticity(evidence) {
        try {
            ...
            if (hasBeforePhoto)
                ...
                issues.push('Missing before photo');
            if (hasAfterPhoto)
                score += 15;
            else
                issues.push('Missing after photo');
     ...
     */
    static async verifyLocation(evidence) {
        try {
            let score = 70;
            const issues = [];
            ...
            if (evidence.startLocation && evidence.expectedLocation) {
                const startDistance = this.calculateDistance(evidence.startLocation.lat, evidence.startLocation.lng, evidence.expectedLocation.lat, evidence.expectedLocation.lng);
            ...
            if (photosWithGPS.length > 0 && evidence.expectedLocation) {
                const avgPhotoDistance = photosWithGPS.reduce((sum, p) => {
                    return sum + this.calculateDistance(p.gpsLat, p.gpsLng, evidence.expectedLocation.lat, evidence.expectedLocation.lng);
     ...
     */
    static async verifyTimestamps(evidence) {
        try {
            let score = 80;
            const issues = [];
     ...
     */
    static async analyzePatterns(evidence) {
        try {
            let score = 85;
            const issues = [];
            ...
            const recentPayouts = await db.select()
                .from(superAppPayouts)
                .where(and(eq(superAppPayouts.providerId, evidence.providerId), gte(superAppPayouts.createdAt, thirtyDaysAgo)));
            const dailyPayoutCounts = new Map();
     ...
     */
    static async assessWorkQuality(evidence) {
        try {
            let score = 70;
            const notes = [];
     ...
     */
    static async getGeminiAnalysis(evidence) {
        try {
...
Keep response under 150 words.`;
     ...
     */
    static async logVerificationEvent(payoutId, verificationId, result) {
        try {
     ...
     */
    static createFailedVerificationResult(verificationId, error) {
     ...
     */
    static async batchVerifyPendingPayouts() {
        try {
            const pendingPayouts = await db.select()
                .from(superAppPayouts)
                .where(and(eq(superAppPayouts.status, 'in_escrow'), sql `${superAppPayouts.aiVerified} IS NULL`))
                .limit(50);
            let verified = 0;
            let failed = 0;
            let flagged = 0;
