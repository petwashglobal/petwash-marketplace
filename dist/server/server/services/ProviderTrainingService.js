 ...
 */
import { db } from '../db';
import { providerTrainingProgress, providerTrainingQuizResults, providerCertificates, } from '@shared/schema';
...
class ProviderTrainingService {
     ...
     */
    async getModulesForPlatform(platform) {
        // Always include general modules + platform-specific
        return TRAINING_MODULES.filter(m => m.platform === 'general' || m.platform === platform);
    }
    /**
     ...
     */
    async getProviderProgress(providerId, platform) {
        try {
            const progress = await db
                .select()
                .from(providerTrainingProgress)
                .where(and(eq(providerTrainingProgress.providerId, providerId), eq(providerTrainingProgress.platform, platform)));
            const quizResults = await db
                .select()
                .from(providerTrainingQuizResults)
                .where(and(eq(providerTrainingQuizResults.providerId, providerId), eq(providerTrainingQuizResults.passed, true)));
            const certificate = await db
                .select()
                .from(providerCertificates)
                .where(and(eq(providerCertificates.providerId, providerId), eq(providerCertificates.platform, platform), eq(providerCertificates.status, 'active')))
                .limit(1);
            const requiredModules = TRAINING_MODULES.filter(m => (m.platform === 'general' || m.platform === platform) && m.requiredForCertification);
            const completedModules = progress
                .filter(p => p.completed)
                .map(p => p.moduleId);
            const passedQuizzes = quizResults.map(q => q.moduleId);
            const isFullyTrained = requiredModules.every(m => completedModules.includes(m.id) && passedQuizzes.includes(m.id));
            return {
                completedModules,
                passedQuizzes,
     ...
     */
    async markModuleCompleted(providerId, moduleId, platform) {
        try {
            ...
            const existing = await db
                .select()
                .from(providerTrainingProgress)
                .where(and(eq(providerTrainingProgress.providerId, providerId), eq(providerTrainingProgress.moduleId, moduleId)))
                .limit(1);
     ...
     */
    async submitQuiz(providerId, submission) {
        try {
            ...
            const previousAttempts = await db
                .select()
                .from(providerTrainingQuizResults)
                .where(and(eq(providerTrainingQuizResults.providerId, providerId), eq(providerTrainingQuizResults.moduleId, submission.moduleId)));
     ...
     */
    async generateCertificate(providerId, platform, providerName) {
        try {
