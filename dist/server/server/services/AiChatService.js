...
class AiChatService {
     ...
     */
    async detectIntent(text, sessionId, languageCode) {
        if (!this.isConfigured() || !client) {
            throw new Error('AiChatService is not configured. Please create a Dialogflow CX agent and add GOOGLE_AGENT_ID and GOOGLE_AGENT_LOCATION to Replit Secrets.');
        }
        try {
            const sessionPath = client.projectLocationAgentSessionPath(this.projectId, this.location, this.agentId, sessionId);
            const request = {
                session: sessionPath,
                queryInput: {
                    text: {
