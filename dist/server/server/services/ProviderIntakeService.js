...
export class ProviderIntakeService {
     ...
     * S: LinkedIn URL
     */
    async syncFromGoogleSheet(sheetId, sheetName = 'Form Responses 1') {
        const result = {
            totalRows: 0,
            newRecords: 0,
            skippedDuplicates: 0,
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                try {
                    ...
                    const existing = await db
                        ...
                        .where(and(eq(providerIntakeQueue.email, formData.email), eq(providerIntakeQueue.providerType, formData.providerType), eq(providerIntakeQueue.googleSheetRowNumber, rowNumber)))
                        .limit(1);
     ...
     */
    async approveAndInvite(intakeId, adminId, sendVia = 'email') {
     ...
     */
    async rejectApplicant(intakeId, adminId, reason) {
        await db
            .update(providerIntakeQueue)
            .set({
            status: 'rejected',
     ...
     */
    async getIntakeQueue(status) {
        let query = db.select().from(providerIntakeQueue);
        if (status) {
            query = query.where(eq(providerIntakeQueue.status, status));
        }
