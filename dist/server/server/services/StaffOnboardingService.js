 ...
 */
import { db } from '../db';
import { staffApplications, staffDocuments, staffESignatures, staffBackgroundChecks, } from '../../shared/schema';
...
export class StaffOnboardingService {
     ...
     */
    async uploadDocument(applicationId, documentType, documentUrl, metadata) {
        try {
     ...
     */
    async verifyBiometrics(applicationId, idPhotoUrl, selfieUrl) {
        try {
            logger.info('[Onboarding] Starting biometric verification', { applicationId });
            const result = await biometricVerification.verifyIdentity(idPhotoUrl, selfieUrl);
            ...
            await db.update(staffDocuments)
                ...
                .where(and(eq(staffDocuments.applicationId, applicationId), eq(staffDocuments.documentType, 'id_front')));
     ...
     */
    async sendESignatureDocuments(applicationId, applicantEmail) {
        try {
     ...
     */
    async rejectApplication(applicationId, reviewedBy, reason) {
        try {
            await db.update(staffApplications)
                .set({
                status: 'rejected',
    ...
    getRequiredDocuments(applicationType) {
        const common = [
