 ...
 */
export async function monthlyInvoiceRun() {
    try {
 ...
 */
async function generateAndSendInvoice(billingData) {
    try {
        ...
        await sendInvoiceEmail(clientEmail, clientName, invoiceNumber, totalAmount, pdfBuffer);
 ...
 */
async function getClientUsage(clientId, billingPeriod) {
    try {
 ...
 */
async function sendInvoiceEmail(email, name, invoiceNumber, amount, pdfBuffer) {
    try {
 ...
 */
export async function triggerMonthlyInvoicing(req, res) {
    try {
