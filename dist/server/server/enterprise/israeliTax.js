 ...
 */
export async function generateTaxInvoice(req, res) {
    try {
        const data = req.body;
        ...
        if (IsraeliTaxAPIService.isConfigured()) {
            ...
            let electronicInvoiceId;
            let itaReferenceNumber;
            ...
            const result = await IsraeliTaxAPIService.submitInvoice(invoicePayload);
            if (result.success) {
 ...
 */
export function calculateVAT(amountBeforeVAT) {
 ...
 */
async function checkFinanceRole(uid) {
    try {
...
export async function checkInvoiceStatus(req, res) {
    try {
        ...
        const response = await axios.get(`${RASA_API_ENDPOINT}/${allocationNumber}`, {
            headers: {
                'Authorization': `Bearer ${SUPPLIER_API_KEY}`
            },
            timeout: 15000
 ...
 */
export async function generateYearEndTaxReport(year) {
    try {
