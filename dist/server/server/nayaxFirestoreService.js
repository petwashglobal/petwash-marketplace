...
const STATION_KEYS = JSON.parse(process.env.STATION_KEYS);
 ...
 */
export function calculateMerchantFee(grossAmount, apiProvidedFee) {
 ...
 */
export function calculateFinancialBreakdown(grossAmount, merchantFee) {
...
export async function updateTransactionStatus(transactionId, status, nayaxTransactionId, merchantFee) {
    try {
        const updates = {
            status,
...
// VOUCHER MANAGEMENT
// =====================================
function generateQRToken(voucherId) {
...
export async function getVoucherByToken(token) {
    try {
...
export async function redeemVoucher(voucherId, terminalId) {
    try {
...
// TERMINAL MANAGEMENT
// =====================================
export async function validateStationKey(apiKey) {
    try {
        const snapshot = await db
...
export async function updateTerminalPing(terminalId) {
    try {
...
export async function getAdminTransactions(filters = {}) {
    try {
        ...
        if (filters.user) {
            ...
            filtered = filtered.filter(t => (t.uid?.toLowerCase() ?? '').includes(userLower) ||
                (t.userEmail?.toLowerCase() ?? '').includes(userLower));
        }
        // Station filter
        if (filters.station) {
            filtered = filtered.filter(t => t.stationId?.includes(filters.station) ||
 ...
 */
export async function startK9000Session(request) {
    try {
 ...
 */
export async function endK9000Session(sessionId, status, metadata) {
    try {
 ...
 */
export async function recordStationHeartbeat(stationId, metadata) {
    try {
        const now = new Date().toISOString();
 ...
 */
export async function createStationAlert(alert) {
    try {
 ...
 */
async function sendAlertEmail(alertId, alert) {
    try {
 ...
 */
export async function autoResolveAlerts(stationId, type) {
    try {
        const snapshot = await db
            .collection('station_alerts')
 ...
 */
export async function acknowledgeAlert(alertId, adminUid) {
    try {
        const now = new Date().toISOString();
        await db.collection('station_alerts').doc(alertId).update({
            acknowledged: true,
 ...
 * Log ECU fault
 */
export async function logECUFault(fault) {
    try {
        const now = new Date().toISOString();
        const faultData = {
            ...fault,
 ...
 */
export async function getActiveAlerts(stationId) {
    try {
 ...
 */
export async function getStationFaults(stationId, resolvedFilter) {
    try {
 ...
 */
export async function simulateRandomFault(stationId) {
    const faultCodes = [
        { code: 'E001', description: 'Water pump failure', severity: 'critical' },
        { code: 'E002', description: 'Soap dispenser malfunction', severity: 'warning' },
        { code: 'E003', description: 'Dryer fan motor failure', severity: 'critical' },
        { code: 'E004', description: 'Door sensor error', severity: 'warning' },
        { code: 'E005', description: 'Temperature sensor out of range', severity: 'info' }
    ];
