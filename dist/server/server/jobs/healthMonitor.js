 ...
 */
async function sendSlackAlert(failedStations) {
    try {
        ...
        const message = {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    text: {
                        type: 'mrkdwn',
                        text: failedStations.map(s => `• *${s.stationName}* (ID: ${s.id})\n  Error: ${s.lastError || 'Connection timeout'}`).join('\n'),
                    },
                },
                {
                    type: 'context',
 ...
 */
async function sendEmailAlert(failedStations) {
    try {
...
export async function runHealthCheck() {
    ...
    `);
        ...
        const checks = await Promise.all(stations.rows.map((station) => checkStationHealth(station)));
        ...
        const avgResponseTime = healthyStations.length > 0
            ? Math.round(healthyStations.reduce((sum, s) => sum + (s.responseTime || 0), 0) /
