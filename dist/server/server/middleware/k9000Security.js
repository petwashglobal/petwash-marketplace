...
const ALLOWED_MACHINE_IPS = ALLOWED_MACHINE_IPS_ENV
    .split(',')
    .map(ip => ip.trim())
 ...
 */
export function validateK9000MachineIP(req, res, next) {
 ...
 */
export function validateMachineSecretKey(req, res, next) {
    ...
    if (!isValid) {
        logger.error('[K9000 Security] Invalid machine secret key', {
            clientIP: req.k9000Station?.clientIP,
        });
        res.status(401).json({
 ...
 */
export function getK9000StationByIP(ip) {
    return K9000_STATIONS.find(s => s.allowedIPs.includes(ip)) || null;
}
export function getK9000StationByTerminal(terminalId) {
