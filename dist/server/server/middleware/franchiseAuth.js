 ...
 */
export async function loadEmployeeContext(req, res, next) {
    try {
 ...
 */
export function requireFranchiseScope(req, res, next) {
    if (!req.employee) {
        return res.status(403).json({
 ...
 */
export function canAccessFranchise(employee, targetFranchiseId) {
    if (!employee)
        return false;
 ...
 */
export function getFranchiseFilter(employee) {
    if (!employee)
        return null;
...
export function requirePermission(...requiredPermissions) {
    return (req, res, next) => {
        ...
        const hasAllPermissions = requiredPermissions.every(permission => req.employee.permissions.includes(permission));
