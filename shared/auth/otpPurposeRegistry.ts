 ...
 */
/**
 * CEO decisions applied 2026-09-01 (§ audit ledger #188-191):
 *
 * #188 signup OTP  → SPLIT into PHONE_VERIFICATION + ACCOUNT_ACTIVATION.
 *                    Two independent trust decisions. A leaked activation
 *                    OTP must not verify a phone.
 *
 * #189 MFA OTPs    → SPLIT into TWO_FACTOR_ENABLE / TWO_FACTOR_DISABLE
 *                    (kept LOGIN as its own purpose too — MFA is a
 *                    different action from "sign in with OTP factor").
 *                    Enrol and un-enrol have opposite risk profiles;
 *                    folding them into SENSITIVE_ACCOUNT_CHANGE muddies
 *                    rate limits + audit trails.
 *
 * #190 payout OTP  → KEEP CHANGE_PAYOUT_DESTINATION AND ADD PAYOUT_ACTION.
 *                    Changing destination is trust-critical; every payout
 *                    action after that is high-frequency ops. Different
 *                    rate limits, different audit weight.
 *
 * #191 GIFT_REDEEM → YES, add it. Buyer-side GIFT_PURCHASE exists; redeem
 *                    is a distinct action with a different threat model
 *                    (recipient may not own the buyer's device).
 */
export const OTP_PURPOSES = [
  'ACCOUNT_ACTIVATION',        // first-time signup — finalise the account after PHONE_VERIFICATION lands
  'EMAIL_VERIFICATION',        // add-or-change email contact
  'PHONE_VERIFICATION',        // add-or-change mobile contact — also runs during signup, distinct from ACCOUNT_ACTIVATION
  'LOGIN',                     // OTP-as-second-factor at sign-in
  'TWO_FACTOR_ENABLE',         // enrol a second factor (CEO #189)
  'TWO_FACTOR_DISABLE',        // un-enrol a second factor — high-risk, must not share code with enable
  ...
  'GIFT_REDEEM',               // eGift redemption confirmation (CEO #191 — distinct from GIFT_PURCHASE)
  'CLOSE_ACCOUNT',             // irreversible destructive action
  'CHANGE_PAYOUT_DESTINATION', // provider bank-account rebinding (trust-critical)
  'PAYOUT_ACTION',             // per-payout confirmation (high-frequency ops; CEO #190)
