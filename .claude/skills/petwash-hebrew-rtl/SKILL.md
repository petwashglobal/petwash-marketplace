...
2. ~~Hard-coded `dir="rtl"` + Hebrew-only labels in `components/ui/google-places-autocomplete.tsx` and `components/AiChatWidget.tsx`~~ — DONE (PR #2672): both read `useLanguage()`; copy lives in `ADDRESS_COPY` / `WIDGET_COPY` per language; pinned by `server/tests/componentsFollowSiteLanguage.regression.test.ts`. Pattern to copy for any other Hebrew-only component.
3. ~~`→` inside Hebrew strings in `components/NotificationCenterPanel.tsx`~~ — DONE (PR #2672) together with the wrong-way arrows in 15 other files (Hebrew forward hints carry no arrow; flow/menu paths use `←`; English forward arrows were `←` in two places). STILL OPEN: the unflipped `ChevronRight` (159 without `rtl:rotate-180`) — fix per page as item 1 is worked, only where the chevron means "forward".
...
**Last updated:** 2026-09-19 — items 2 and 3 done in PR #2672 (created from the audit; first fixes in the same PR: 41 dictionary strings, the Hebrew locale file, validation messages, BookingSearch icons, MarketplaceBookingFlow chevrons, email/phone inputs, three Arabic strings with Hebrew letters, two guards).
