...
### 2026-09-19 — Hebrew and RTL: the audit, the fixes, and the skill every agent now reads
- **CEO: "Hebrew not ok in all places — Google-Translate Hebrew — some letters/RTL wrong."** A full audit (register, terminology, bidi, layout) is condensed into a new skill: **`petwash-hebrew-rtl`** (`.claude/skills/petwash-hebrew-rtl/SKILL.md`). Read it before writing or reviewing any Hebrew string or RTL layout. It carries the one-voice register rule (plural/neutral, no `אנא`, no "failed to X" syntax), the terminology canon (`רחיצה`, `שמרטף/ית`, `מוליך/ה`, `ארנק/יתרה/נקודות`, `תו שי דיגיטלי`, `האזור האישי`, `תנאי השימוש`), what actually breaks under bidi (prefix glue, `+972`, `#ref`, mixed digit-Latin tokens — NOT numeric ranges), the layout rules (logical Tailwind, `rtl:rotate-180`, `dir` only on `<html>`), and the ordered backlog.
- **Fixed in the same PR:** 41 dictionary strings in `client/src/lib/i18n.ts` (the pet "babysitter", "installations" on the consent checkbox, "waiver" in the footer, `החיית`, "failed to" calques, `דלג על כעת`, …); `client/public/locales/he/translation.json` was **88 % English** and is now Hebrew; validation messages impersonal; three Arabic strings that contained Hebrew letters; the marketplace search box icons (logical utilities), booking-flow chevrons, phone/email inputs `dir="ltr"`.
- **Two guards:** `scripts/guards/trademark_bidi_anchored.py` now also scans `server/` (baselined — ~260 bare `™` on Hebrew lines in customer SMS/email/push, may not grow) and `scripts/guards/hebrew_bidi_glue.py` (prefix-glue / bare `+972`, baselined). Both in `money-safety-gate.yml` → brand-and-reachability-guards.

### 2026-09-19 — customer emails: how to see what a customer sees, and what the first look found
- **One run renders every customer email with fake data and sends it** (#2659): `scripts/admin/send-sample-emails.ts` + `.github/workflows/diagnose-email-samples.yml` (Run workflow → target_email, locale; or push `ops/diagnose/email-samples.json` on a `diag/email-samples/**` branch). Sitter Suite and Walk My Pet booking confirmed, Academy lesson receipt, the payment letter, Shop order, eGift buyer + recipient, welcome, Prestige membership. Subjects start with `[SAMPLE]`; nothing is created. `DRY_RUN=1` renders to files. `server/tests/customerEmailSamples.behavior.test.ts` renders all nine in he + en on every CI run.
- **First look fixed three things customers were reading** (#2664): shop delivery date printed as an ISO timestamp, booking duration as "(4320 min)", a purple band on sitter confirmations under a template header that says NO PURPLE. The logo: every template loads a hosted URL from `server/email/templates/logo-base64.ts` (since #2662 the white-on-dark asset `petwash-logo-black-bg.png` for dark headers, `petwash-logo-white-bg.png` for light sections — the old 870 KB official PNG rendered as a white box on dark headers). A broken image in a sandbox screenshot means only that the sandbox cannot reach petwash.co.il.
- **Academy has no booking-confirmation variant** (`BookingService` has no academy value); customers get the receipt only. Add the variant if the Academy is to match Walk / Sitter.
- **Testing the site from an agent sandbox:** petwash.co.il is blocked by the egress policy, so "the site is down" from curl here means nothing. The truth is the repo's own probes on GitHub runners: `public-route-sweep.yml` (every public route renders, ~300), `release-smoke.yml` (health, config, gating, assets, two pages), `uptime-healthcheck.yml` (every 15 min). Front-end journeys run locally with `npm run test:verification` (needs a Chromium; in the sandbox pass `launchOptions.executablePath: '/opt/pw-browsers/chromium'`).

### 2026-09-19 — SUMIT / UPay: what is true, what SUMIT answered, what changed
- **There is now a dedicated skill: `petwash-sumit-upay`** (`.claude/skills/petwash-sumit-upay/`) — read it before any SUMIT / UPay / receipt / card-payment change. It carries the live state, SUMIT's written answers, every endpoint we call with its gotchas, the runbooks (paid-but-got-nothing, Tax Authority errors, diagnose workflows) and the unbuilt marketplace-split design. The bullets below are the short version.
