...
async function getOwnerRole(caseType: string, caseRefId: string): Promise<'agent' | 'manager'> {
  try {
    const r = await db.execute(sql`
      ...
      WHERE ca.case_type   = ${caseType}
        AND ca.case_ref_id = ${caseRefId}
...
async function getCurrentAssignment(caseType: string, caseRefId: string): Promise<string | null> {
  try {
    const r = await db.execute(sql`
      ...
      WHERE case_type = ${caseType} AND case_ref_id = ${caseRefId} AND is_active = true
      LIMIT 1
    `);
  ...
  try {
    ...
    if (caseType === 'dispute') {
      r = await db.execute(sql`
        ...
        WHERE bd.id::text = ${caseRefId} AND fo.status = 'active'
        LIMIT 1
      `);
      ...
      r = await db.execute(sql`
        ...
        JOIN franchise_owners fo ON fo.id = st.franchise_id
        WHERE ss.id::text = ${sid} AND fo.status = 'active'
        LIMIT 1
      `);
    } else {
      ...
      r = await db.execute(sql`
        ...
        JOIN stations st ON st.id = b.station_id
        JOIN franchise_owners fo ON fo.id = st.franchise_id
        WHERE b.id::text = ${bid} AND fo.status = 'active'
        LIMIT 1
      `);
    }
...
async function doEscalate(
  ...
  const escalationNote = `Auto-escalated: SLA of ${slaBudgetHours}h exceeded at ${Math.round(ageHours)}h`;
  const breachNote = `SLA ${slaBudgetHours}h exceeded — auto-escalated at ${Math.round(ageHours)}h`;
  ...
  await db.execute(sql`
    ...
    SET escalated_at = NOW(), escalated_to_uid = ${toUid}
    WHERE case_type = ${caseType} AND case_ref_id = ${caseRefId}
  `);
...
async function processCases(cases: CaseRecord[]): Promise<{ newBreaches: number; escalated: number }> {
  ...
  for (const c of cases) {
    ...
    // Upsert SLA state — numeric server-computed values embedded directly
    const ageRounded    = Math.round(ageHours * 100) / 100;
    const breachDetectedExpr = slaStatus === 'breached'
      ? sql`NOW()`
      : sql`NULL`;
    ...
    await db.execute(sql`
      ...
      VALUES (
        ${caseType}, ${caseRefId}, ${slaStatus},
        ${slaBudgetHours}, ${ageRounded},
        ${breachDetectedExpr},
        NOW()
      )
    ...
    if (slaStatus === 'breached' && prevStatus !== 'breached') {
      ...
      // Breach event
      const breachLogNote = `SLA ${slaBudgetHours}h exceeded at ${Math.round(ageHours)}h`;
      await db.execute(sql`
        INSERT INTO case_escalation_log (case_type, case_ref_id, event_type, note)
        VALUES (
          ${caseType}, ${caseRefId}, 'sla_breached',
          ${breachLogNote}
        )
      `);
...
export async function runSlaMonitor(): Promise<void> {
  ...
  try {
    // ── Disputes ──────────────────────────────────────────────────────────
    const disputeOpen         = SLA.DISPUTE_OPEN;
    const disputeUnderReview  = SLA.DISPUTE_UNDER_REVIEW;
    const dRows = await db.execute(sql`
      SELECT
        bd.id::text                                                     AS case_ref_id,
        EXTRACT(EPOCH FROM (NOW() - bd.created_at)) / 3600             AS age_hours,
        CASE WHEN bd.status = 'under_review' THEN ${disputeUnderReview}
             ELSE ${disputeOpen} END                                   AS sla_budget_hours
      FROM booking_disputes bd
