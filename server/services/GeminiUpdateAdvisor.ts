...
export class GeminiUpdateAdvisor {
  ...
  private static async analyzeUpdate(update: any): Promise<UpdateAdvisory> {
    try {
      const prompt = `
You are a senior DevOps engineer advising on a software update for ⁦PetWash™⁩ production platform.
  ...
  private static async sendAdvisoryEmail(advisories: UpdateAdvisory[]): Promise<void> {
    try {
      ...
      await emailService.sendEmail({
        ...
        subject: `🤖 ⁦PetWash™⁩ - Gemini AI Update Recommendations (${advisories.length} updates)`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            <h1 style="color: #0B57D0;">🤖 Gemini AI Update Advisory</h1>
            <p>Gemini 2.5 Flash has analyzed ${advisories.length} available updates for your ⁦PetWash™⁩ platform.</p>
