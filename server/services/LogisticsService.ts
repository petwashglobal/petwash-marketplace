import { db } from '../db';
import { logisticsTasks, logisticsVehicles, stations, users } from '@shared/schema';
import type { InsertLogisticsTask, LogisticsTask, InsertLogisticsVehicle, LogisticsVehicle } from '@shared/schema';
import { eq, and, desc, asc, sql, inArray } from 'drizzle-orm';
import { logger } from '../lib/logger';

export class LogisticsService {
  async createTask(data: Omit<InsertLogisticsTask, 'taskNumber'>): Promise<LogisticsTask> {
    try {
  ...
  async assignTask(taskId: number, userId: string): Promise<LogisticsTask> {
    try {
      const [task] = await db
  ...
  async markTaskInProgress(taskId: number): Promise<LogisticsTask> {
    try {
      const [task] = await db
        .update(logisticsTasks)
        .set({
  ...
  async completeTask(taskId: number): Promise<LogisticsTask> {
    try {
      const [task] = await db
        .update(logisticsTasks)
        .set({
  ...
  async failTask(taskId: number, reason: string): Promise<LogisticsTask> {
    try {
      const [task] = await db
        .update(logisticsTasks)
        .set({
  ...
  async listTasksForUser(userId: string, statuses?: string[]): Promise<LogisticsTask[]> {
    try {
      let query = db
        .select()
        .from(logisticsTasks)
      ...
      if (statuses && statuses.length > 0) {
        query = query.where(
          and(
            eq(logisticsTasks.assignedToUserId, userId),
            inArray(logisticsTasks.status, statuses)
          )
        );
      }
  ...
  async listTasksByStation(stationId: number, statuses?: string[]): Promise<LogisticsTask[]> {
    try {
      let query = db
        .select()
        .from(logisticsTasks)
      ...
      if (statuses && statuses.length > 0) {
        query = query.where(
          and(
            eq(logisticsTasks.stationId, stationId),
            inArray(logisticsTasks.status, statuses)
          )
        );
      }
  ...
  async getTaskById(taskId: number): Promise<LogisticsTask | null> {
    try {
      const [task] = await db
        .select()
        .from(logisticsTasks)
  ...
  async createVehicle(data: InsertLogisticsVehicle): Promise<LogisticsVehicle> {
    try {
  ...
  async updateVehicle(vehicleId: number, data: Partial<InsertLogisticsVehicle>): Promise<LogisticsVehicle> {
    try {
  ...
  async listVehicles(isActive?: boolean): Promise<LogisticsVehicle[]> {
    try {
  ...
  async getVehicleById(vehicleId: number): Promise<LogisticsVehicle | null> {
    try {
      const [vehicle] = await db
        .select()
