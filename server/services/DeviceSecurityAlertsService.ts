...
export class DeviceSecurityAlertsService {
  ...
  private static getEmailSubject(alert: SecurityAlert): string {
    ...
    switch (alert.alertType) {
      case 'new_device':
        return `${emoji} New Device Logged In - ⁦PetWash™⁩`;
      case 'suspicious_login':
        return `${emoji} Suspicious Login Attempt - ⁦PetWash™⁩`;
      case 'location_change':
        return `${emoji} Unusual Location Detected - ⁦PetWash™⁩`;
      case 'fraud_detected':
        return `${emoji} URGENT: Potential Fraud Detected - ⁦PetWash™⁩`;
      default:
        return `${emoji} Security Alert - ⁦PetWash™⁩`;
    }
  }
              ...
              <table width="600" cellpadding="0" cellspacing="0" style="background-color: white; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                ...
                <tr>
                  <td style="padding: 40px 40px 20px; text-align: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px 12px 0 0;">
                    ...
                    <p style="margin: 10px 0 0; color: rgba(255,255,255,0.9); font-size: 16px;">⁦PetWash™⁩ Account Security</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 30px 40px; background-color: #f9fafb; border-radius: 0 0 12px 12px; text-align: center;">
                    <p style="margin: 0 0 10px; color: #6b7280; font-size: 12px;">
                      This is an automated security alert from ⁦PetWash™⁩
                    </p>
  ...
  private static getEmailText(alert: SecurityAlert): string {
    ...
    let text = `🔒 SECURITY ALERT - ⁦PetWash™⁩\n\n`;
    ...
    text += `This is an automated security alert from ⁦PetWash™⁩`;
  ...
  static createNewDeviceAlert(params: {
    ...
    return {
      ...
      details: `A new device (${params.deviceLabel}) has signed in to your ⁦PetWash™⁩ account. If this was you, you can safely ignore this message.`,
      timestamp: new Date(),
  ...
  static createSuspiciousLoginAlert(params: {
    ...
    return {
      ...
      details: `We detected suspicious activity during a login attempt on your ⁦PetWash™⁩ account. Please review your connected devices and secure your account if needed.`,
      timestamp: new Date(),
      fraudScore: 100 - params.trustScore,
