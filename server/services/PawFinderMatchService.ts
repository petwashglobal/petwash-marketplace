...
export async function refreshMatchesForPost(pool: any, postId: number): Promise<void> {
  try {
    ...
    const others = await pool.query(
      `SELECT * FROM paw_finder_posts WHERE post_type = $1 AND status IN ('published','matched') AND city = $2 AND id <> $3`,
    ...
    // Update matched_post_count and auto-elevate status to 'matched'
    await pool.query(
      `UPDATE paw_finder_posts p
       SET matched_post_count = COALESCE(sub.cnt, 0),
           status = CASE WHEN COALESCE(sub.cnt,0) > 0 AND p.status = 'published' THEN 'matched' ELSE p.status END,
           updated_at = NOW()
       FROM (
         SELECT lost_post_id AS pid, COUNT(*)::int AS cnt FROM paw_finder_matches WHERE status = 'suggested' GROUP BY 1
         UNION ALL
         SELECT found_post_id AS pid, COUNT(*)::int AS cnt FROM paw_finder_matches WHERE status = 'suggested' GROUP BY 1
       ) sub
       WHERE p.id = $1 AND p.id = sub.pid`,
      [postId],
    );

    logger.info('[PawFinderMatch] Matches refreshed', { postId });
  } catch (err: any) {
