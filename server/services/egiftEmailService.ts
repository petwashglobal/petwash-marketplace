...
export async function sendEGiftConfirmationEmail(config: EGiftEmailConfig): Promise<void> {
  ...
  const t = {
    subject: isHe 
      ? `${emoji} כרטיס מתנה דיגיטלי של PetWash™ נשלח בהצלחה!`
      : `${emoji} Your PetWash™ E-Gift Card Has Been Sent!`,
    ...
    thankYou: isHe 
      ? 'תודה שבחרת ב-PetWash™ - טיפוח יוקרתי לחיות מחמד'
      : 'Thank you for choosing PetWash™ - Luxury Pet Care',
...
<body style="margin:0;padding:0;background:#f5f5f0;font-family:'Helvetica Neue',Arial,sans-serif;">
  ...
  const recipientSubheading = isHe 
    ? `${config.senderName} שלח/ה לך כרטיס מתנה של PetWash™`
    : `${config.senderName} sent you a PetWash™ gift card`;
  ...
  try {
    const recipientSubject = isHe
      ? `${emoji} ${config.senderName} שלח/ה לך כרטיס מתנה של PetWash™!`
      : `${emoji} ${config.senderName} sent you a PetWash™ E-Gift Card!`;
