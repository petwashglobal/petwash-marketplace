...
// ============================================================
// KYC Drive Storage — per-provider folder management
// ============================================================

const PETWASH_ROOT_FOLDER = 'PetWash';
const KYC_PARENT_FOLDER = 'KYC';

let kycRootFolderId: string | null = null;
let kycParentFolderId: string | null = null;
...
/**
 * Ensure PetWash/KYC/{uid}/ folder exists and return its ID + web URL.
 * Creates missing parent folders as needed.
 */
export async function ensureKycProviderFolder(
  uid: string
): Promise<{ folderId: string; folderUrl: string }> {
  const drive = await getGoogleDriveClient();

  if (!kycRootFolderId) {
    kycRootFolderId = await ensureFolderExists(drive, PETWASH_ROOT_FOLDER);
  }
  if (!kycParentFolderId) {
    kycParentFolderId = await ensureFolderExists(drive, KYC_PARENT_FOLDER, kycRootFolderId);
  }

  const providerFolderId = await ensureFolderExists(drive, uid, kycParentFolderId);
  const folderUrl = `https://drive.google.com/drive/folders/${providerFolderId}`;
  return { folderId: providerFolderId, folderUrl };
}

/**
 * Upload a buffer to Drive inside the given folder.
 * If a file with the same name already exists in that folder it is deleted first
 * (overwrite semantics) to avoid duplicates on re-submission.
 */
export async function uploadKycFileToDrive(
  folderId: string,
  fileName: string,
  buffer: Buffer,
  mimeType: string
  ...
  // Delete existing file with same name (overwrite on re-submission)
