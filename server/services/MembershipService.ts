...
function generateDigits(): string {
  return String(Math.floor(1000000 + crypto.randomInt(9000000)));
}
...
export async function generateMembershipId(cls: MembershipClass): Promise<string> {
  for (let attempt = 0; attempt < 10; attempt++) {
    const candidate = `${cls}-${generateDigits()}`;
    if (!(await isCollision(cls, candidate))) {
      logger.info(`[MembershipService] Generated ${CLASS_CONFIG[cls].label} ID: ${candidate}`);
      return candidate;
    }
  }
  const fallback = `${cls}-${Date.now().toString(36).toUpperCase().slice(-7).padStart(7, '0')}`;
  logger.warn(`[MembershipService] Collision fallback used: ${fallback}`);
  return fallback;
}

export async function assignCustomerMembership(userId: string): Promise<string> {
  ...
  const memberId = await generateMembershipId('PWM');
...
export async function assignProviderMembership(applicantId: number): Promise<string> {
  const [existing] = await db
  ...
  if (existing?.membershipNumber && existing.membershipNumber.startsWith('PWP-')) {
    return existing.membershipNumber;
  }

  const memberId = await generateMembershipId('PWP');
...
export async function assignStaffMembership(applicationId: number): Promise<string> {
  const [existing] = await db
  ...
  const memberId = await generateMembershipId('PWS');
