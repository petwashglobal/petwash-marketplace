...
function detectPatterns(content: string): { patterns: string[]; positions: Array<{ start: number; end: number; type: string }> } {
...
function redactContent(content: string, positions: Array<{ start: number; end: number; type: string }>): string {
...
function calculateSeverity(patterns: string[]): 'none' | 'low' | 'medium' | 'high' {
  if (patterns.length === 0) return 'none';
  
  // High severity: direct payment bypass or explicit contact sharing
  if (patterns.includes('payment_bypass')) return 'high';
  if (patterns.includes('phone_number') && patterns.includes('whatsapp')) return 'high';
  
  // Medium severity: phone or email
  if (patterns.includes('phone_number') || patterns.includes('email')) return 'medium';
 ...
 */
export function scanForBypass(content: string): BypassDetectionResult {
 ...
 */
export function getBypassWarningMessage(patterns: string[], isProvider: boolean): string {
 ...
 */
export function getBypassAction(attemptCount: number): 'warn' | 'block' | 'suspend' {
  if (attemptCount >= 3) return 'suspend';
  if (attemptCount >= 1) return 'block';
  return 'warn';
}
