...
router.post('/submit', requireAuth, async (req: any, res) => {
  try {
    ...
    const result = await policeCheckService.submitPoliceCheck(
      req.userId,
      documentUrl,
      documentFileName,
      new Date(documentIssuedAt)
    );
 ...
 */
router.get('/admin/expiring', requireAdmin, async (req: any, res) => {
  try {
 ...
 */
router.post('/admin/approve/:checkId', requireAdmin, async (req: any, res) => {
  try {
    ...
    const result = await policeCheckService.approvePoliceCheck(
      checkId,
      req.userId,
      reviewNotes
    );
 ...
 */
router.post('/admin/reject/:checkId', requireAdmin, async (req: any, res) => {
  try {
    const checkId = parseInt(req.params.checkId);
    if (isNaN(checkId)) {
      return res.status(400).json({ error: 'מזהה בקשה לא תקין' });
    ...
    const result = await policeCheckService.rejectPoliceCheck(
      checkId,
      req.userId,
 ...
 */
router.post('/admin/expire-checks', requireAdmin, async (req: any, res) => {
  try {
