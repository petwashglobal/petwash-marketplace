 ...
 *
 * B-pay — opt-in Tranzila card capture for marketplace bookings.
 *
 * Status today
 * ────────────
 * Tranzila is gated by TRANZILA_MARKETPLACE_ENABLED (default false).
 * The internal `_charge` method in TranzilaService.ts is a STUB that
 * returns `success: false` until the real REST API call is wired by
 * the payments team. This route is therefore safe to ship today —
 * it returns 502 PAYMENT_FAILED for every attempt and never moves
 * money — but the customer-facing UI can already integrate against
 * a stable contract.
 *
 * Body
 * ────
 *   {
 *     cardToken:      string  // Tranzila iframe token (PCI scope OFF the
 *                                origin server)
 *     idempotencyKey: string  // client-generated UUID; safe to retry
 *   }
 *
 * Auth
 * ────
 *   - requireAuth (route-level middleware)
 *   - Customer role only — providers / admins cannot pay on someone
 *     else's behalf via this endpoint.
 *
 * Booking state
 * ─────────────
 *   The booking must be in `pending_payment` or `accepted` (the two
 *   statuses where a customer is expected to commit to paying). Any
 *   other status returns 409 INVALID_STATE.
 *
 * Side effects
 * ────────────
 *   On Tranzila success:
 *     1. Booking status → `confirmed` via bookingLifecycleService.
 *     2. Caller is responsible for showing the confirmation UI;
 *        escrow + ledger writes follow the existing post-confirm
 *        flow (already wired in bookingLifecycleService).
 *   On Tranzila failure:
 *     Booking stays in current state. Caller can retry with the
 *     SAME idempotencyKey — Tranzila treats it as a no-op.
 *
 * Architecture note
 * ─────────────────
 *   K9000 cannot use this route — its bookings never enter
 *   `booking_requests`. Confirmed by the K9000 guard in
 *   server/routes/booking-requests.ts (B6.5).
 */
router.post('/:bookingId/pay-with-tranzila', requireAuth, async (req, res) => {
  try {
    // Cast through `any` to match the project-wide req.user narrowing
    // gap — the requireAuth middleware above already guarantees uid.
    const userId = (req as any).user?.uid || (req as any).firebaseUser?.uid;
    if (!userId) {
      return res.status(401).json({ error: 'Authentication required' });
    }
    const { bookingId } = req.params;
    const { cardToken, idempotencyKey } = req.body || {};

    if (!cardToken || typeof cardToken !== 'string' || cardToken.length < 10) {
      return res.status(400).json({ error: 'cardToken is required (Tranzila iframe token).', code: 'INVALID_CARD_TOKEN' });
    }
    if (!idempotencyKey || typeof idempotencyKey !== 'string' || idempotencyKey.length < 8) {
      return res.status(400).json({ error: 'idempotencyKey is required (UUID-shaped string).', code: 'INVALID_IDEMPOTENCY_KEY' });
    }

    // Customer-only.
    const userRole = (req as any).user?.role || (req as any).firebaseUser?.claims?.role;
    const party = await assertBookingParty(bookingId, userId, userRole, res);
    if (!party) return;
    if (party.derivedRole !== 'customer') {
      return res.status(403).json({ error: 'Only the booking customer can pay via Tranzila.', code: 'WRONG_ROLE' });
    }

    // The booking must be awaiting payment.
    const ALLOWED_STATES = new Set(['pending_payment', 'accepted']);
    if (!ALLOWED_STATES.has(String(party.booking.status))) {
      return res.status(409).json({
        error: `Cannot pay — booking is in status '${party.booking.status}'.`,
        code: 'INVALID_STATE',
        currentStatus: party.booking.status,
      });
    }

    // Amount is read from the booking row (server-trusted), never from req.body.
    // booking.total is decimal(12,2) ILS — multiply by 100 to get agorot.
    const amountAgorot = Math.round(Number(party.booking.total ?? 0) * 100);
    if (!Number.isFinite(amountAgorot) || amountAgorot <= 0) {
      return res.status(409).json({ error: 'Booking total is not set.', code: 'NO_AMOUNT' });
    }

    // Lazy import — TranzilaService reads env at module load.
    const { default: TranzilaService } = await import('../services/TranzilaService');
    const charge = await TranzilaService.captureMarketplaceBooking({
      amountAgorot,
      customerId: userId,
      providerId: String(party.booking.providerId ?? ''),
      bookingId,
      cardToken,
      idempotencyKey,
    });
    ...
    if (!charge.success) {
      logger.warn('[MarketplaceBookings] Tranzila capture failed', {
        bookingId, customerId: userId, reason: charge.error,
      });
      // Surface the structured failure but never reveal raw processor errors.
      return res.status(502).json({
        success: false,
    ...
    // Capture succeeded — transition booking to `confirmed`. The existing
    // bookingLifecycleService handles the side-effects (escrow row,
    // notifications, etc.) — we do NOT duplicate them here.
    await bookingLifecycleService.transitionStatus(
      bookingId,
      'confirmed' as BookingLifecycleStatus,
      userId,
      'customer',
      'Booking confirmed via Tranzila card capture',
    );

    logger.info('[MarketplaceBookings] Tranzila capture + confirm OK', {
      bookingId, customerId: userId, transactionId: charge.transactionId,
    });
    ...
    logger.error('[MarketplaceBookings] pay-with-tranzila error', { error: error.message });
    res.status(400).json({ success: false, error: error.message });
  }
});
