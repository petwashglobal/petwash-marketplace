...
// NOTE: the former `POST /:bookingId/pay-with-tranzila` route was removed when
// Tranzila was retired (it called a stub that always failed). Online card
// capture for marketplace bookings will be re-introduced on UPay/SUMIT under a
// new route when that rail is wired. K9000/Nayax kiosk payment is unaffected.
...
// CEO RULE (2026-09-13): a machine key can no longer release provider money.
// Escrow is released only by a signed-in Pet Wash admin
// (POST /api/escrow/admin/:escrowId/approve-release).
router.post('/process-escrow-releases', (_req, res) => {
  res.status(410).json({ error: 'HUMAN_APPROVAL_REQUIRED', message: 'Provider payouts are approved by a Pet Wash admin.' });
});
