 ...
 *
 * Routes mounted at /api/pass and /api/wallet:
 *
 *   GET /api/pass/:signedToken           — UA-detect → Apple / Google / HTML chooser
 *   GET /api/wallet/apple/:signedToken   — Serve .pkpass file
 *   GET /api/wallet/google/:signedToken  — Redirect to Google Wallet save URL
 *
...
router.get('/:token', async (req: Request, res: Response) => {
  try {
    ...
    if (isIOS(ua) && isAppleWalletConfigured()) {
      return res.redirect(307, `/api/wallet/apple/${enc}`);
    }
    if (isAndroid(ua) && isGoogleWalletConfigured()) {
      return res.redirect(307, `/api/wallet/google/${enc}`);
    }

    const appleUrl  = `/api/wallet/apple/${enc}`;
    const googleUrl = `/api/wallet/google/${enc}`;
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/wallet/apple/:signedToken — Serve signed .pkpass file
// ─────────────────────────────────────────────────────────────────────────────
