...
const RUNBOOK: Record<string, { meaning: string; owner: string; safePath: string }> = {
  stale_pending_transfer_72h: {
    ...
    switch (anomaly) {
      case 'stale_pending_transfer_72h': {
        ...
        const r = await db.execute(sql`
          SELECT id AS payout_id, booking_id, provider_id, net_amount, status,
                 created_at, updated_at,
                 ROUND(EXTRACT(EPOCH FROM (NOW() - updated_at)) / 3600, 1) AS age_hours
          FROM super_app_payouts
          WHERE status = 'pending_transfer' AND updated_at < NOW() - INTERVAL '72 hours'
      ...
      case 'stale_pending_transfer_48h': {
        ...
        const r = await db.execute(sql`
          SELECT id AS payout_id, booking_id, provider_id, net_amount, status,
                 ...
                 ROUND(EXTRACT(EPOCH FROM (NOW() - updated_at)) / 3600, 1) AS age_hours
          FROM super_app_payouts
          WHERE status = 'pending_transfer' AND updated_at < NOW() - INTERVAL '48 hours'
          ORDER BY updated_at ASC LIMIT ${limit}
        `);
        rows = r.rows ?? [];
        break;
      ...
      case 'stale_pending_transfer_24h': {
        ...
        const r = await db.execute(sql`
          SELECT id AS payout_id, booking_id, provider_id, net_amount, status,
                 ...
                 ROUND(EXTRACT(EPOCH FROM (NOW() - updated_at)) / 3600, 1) AS age_hours
          FROM super_app_payouts
          WHERE status = 'pending_transfer' AND updated_at < NOW() - INTERVAL '24 hours'
          ORDER BY updated_at ASC LIMIT ${limit}
        `);
        rows = r.rows ?? [];
        break;
