...
async function loadThreadForCaller(
  ...
  // isSuperAdminVerified is SYNCHRONOUS and takes the Request, not an email.
  // Calling `.catch()` on its boolean return threw a TypeError for every
  // caller that had an email — and these handlers have no try/catch, so on
  // Express 4 the rejection was never forwarded and the request just hung.
