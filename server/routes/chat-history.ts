...
function canAccessConversation(req: AuthenticatedRequest, conversation: ChatConversation): boolean {
  ...
  // KNOWN GAP (2026-09-19): req.user.franchiseId is declared on the local
  // AuthenticatedRequest but NOTHING assigns it — no middleware in this mount's
  // chain writes a franchise onto the request. So this comparison is always
  // false and franchise-scoped access never grants. The admin bypass above now
  // works, which is what unblocked the reported 403s; wiring a real franchise
  // claim is a separate change and is deliberately NOT faked here.
