...
import { 
  ...
  campaignRedemptions,
  type InsertPetAwarenessDay,
  type InsertPromotionalCampaign,
  type InsertCampaignRedemption
} from '../../shared/schema';
...
router.get('/calendar', async (req, res) => {
  try {
    ...
    // Localize content
    const requestedLocale = (locale as string) || 'en';
    const localizedDays = days.map(day => {
      const titles = day.titleLocales as Record<string, string>;
      const descriptions = day.descriptionLocales as Record<string, string>;
  ...
  try {
    ...
    if (status) {
      campaigns = await db.select()
        ...
        .where(eq(promotionalCampaigns.status, status as string))
    ...
    const localizedCampaigns = campaigns.map(campaign => {
      const content = campaign.contentLocales as Record<string, { title: string; description: string }>;
      return {
        ...campaign,
...
router.get('/campaigns/live', async (req, res) => {
  try {
    ...
    const campaigns = await db.select()
      .from(promotionalCampaigns)
      .where(and(
        eq(promotionalCampaigns.status, 'live'),
        lte(promotionalCampaigns.startAt, now),
        gte(promotionalCampaigns.endAt, now)
      ))
...
router.post('/campaigns', async (req, res) => {
  try {
    ...
    const content = contentLocales as Record<string, { title: string; description: string }>;
    ...
    const data: InsertPromotionalCampaign = {
      ...rest,
      contentLocales,
      createdBy: user.id,
      status: 'draft',
...
router.post('/validate-code', async (req, res) => {
  try {
    ...
    const [campaign] = await db.select()
      .from(promotionalCampaigns)
      .where(and(
        eq(promotionalCampaigns.promoCode, promoCode.toUpperCase()),
        eq(promotionalCampaigns.status, 'live'),
        lte(promotionalCampaigns.startAt, now),
        gte(promotionalCampaigns.endAt, now)
      ))
...
router.post('/seed-awareness-days', async (req, res) => {
  try {
    ...
    const awarenessData: InsertPetAwarenessDay[] = [
      {
