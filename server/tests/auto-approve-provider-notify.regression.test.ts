...
describe('auto-approve cron — provider payout notification pin', () => {
  it('provider "job completed — payout under review" dispatch uses inbox + email + push', () => {
    const idx = SRC.indexOf('ההזמנה הושלמה — התשלום בבדיקה');
  ...
  it('bilingual title so push preview is legible for EN providers', () => {
    const idx = SRC.indexOf('ההזמנה הושלמה — התשלום בבדיקה');
    const window = SRC.slice(idx, idx + 1200);
    expect(window).toMatch(/Job completed — payout under review/);
    // CEO rule 2026-09-13: the cron never tells a provider money was released.
