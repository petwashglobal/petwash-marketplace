...
export function run(root = ROOT) {
  ...
  const live = serverRoutes.filter((r) => !r.is410);
  ...
  for (const call of calls) {
    ...
    for (const r of cands) {
      ...
      else if (s.extraServerParams > 0 && !paramShort) paramShort = r;
      else if (s.clientLonger && !clientLonger) clientLonger = r;
    }

    if (pathHits.length) {
      const verbHit = pathHits.find((r) => r.method === call.method || r.method === 'ALL');
      if (verbHit) {
    ...
    if (paramShort) {
      ...
      findings.push(mk('PARAM_MISMATCH', call, paramShort,
        call.hasQuery
          ...
          : `server requires ${missing} more path param(s) (${paramShort.path}); client stops at ${call.path}`));
      continue;
    }

    // same tail segment mounted under a different prefix?
    const tail = segments(call.path).slice(-2).join('/');
    const elsewhere = (byTail.get(tail) ?? []).filter((r) => r.method === call.method);
    if (elsewhere.length) {
