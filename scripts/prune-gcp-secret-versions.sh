...
#
# SAFETY
#   * Always keeps the latest KEEP versions enabled — never destroys
...
for NAME in "${SECRETS_ARR[@]}"; do
  ...
  # Pull every version with state ENABLED, newest first. We only ever destroy
  # ENABLED versions; DESTROYED and DISABLED are left alone.
  ...
  if [ -z "$ENABLED_VERSIONS" ]; then
    echo "  ✅ $NAME — no enabled versions, nothing to prune"
    continue
  fi

  ENABLED_COUNT=$(echo "$ENABLED_VERSIONS" | wc -l | tr -d ' ')

  if [ "$ENABLED_COUNT" -le "$KEEP" ]; then
    echo "  ✅ $NAME — $ENABLED_COUNT enabled (≤ KEEP=$KEEP), nothing to prune"
    continue
  fi
  ...
  echo "  🧹 $NAME — pruning $TO_PRUNE_COUNT of $ENABLED_COUNT enabled versions (keeping newest $KEEP)"

  while IFS= read -r VERSION; do
    [ -z "$VERSION" ] && continue
    if [ "$DRY_RUN" = "1" ]; then
      echo "     [dry-run] would destroy v$VERSION"
      TOTAL_SKIPPED=$((TOTAL_SKIPPED + 1))
    else
      if gcloud secrets versions destroy "$VERSION" \
           --secret="$NAME" --project="$PROJECT" --quiet >/dev/null 2>&1; then
        echo "     ✓ destroyed v$VERSION"
...
echo "----------------------------------------------------------------"
if [ "$DRY_RUN" = "1" ]; then
  echo "  DRY RUN — would destroy $TOTAL_SKIPPED version(s) across ${#SECRETS_ARR[@]} secret(s)"
