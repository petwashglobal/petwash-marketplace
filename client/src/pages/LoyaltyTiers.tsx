  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
              ...
              <div key={tier.id} className="flex items-center">
                <button
                  ...
                  className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-500 shrink-0
                    ${isCurrent
                      ? 'ring-2 ring-[#D9B84C] ring-offset-2 ring-offset-white scale-110'
                      : isReached
                        ? 'opacity-100'
                        : 'opacity-40'
                    }
                ...
                >
                  ...
                  {isCurrent && (
                    <span className="absolute -bottom-5 text-[9px] font-bold text-[#0a0a0a] whitespace-nowrap">
                      {isHebrew ? 'כאן' : 'YOU'}
                    </span>
                  )}
                </button>
                {idx < TIER_CONFIGS.length - 1 && (
                  <div className={`w-4 sm:w-8 h-0.5 mx-0.5 transition-all duration-500 ${idx < currentTierIndex ? 'bg-[#D9B84C]' : 'bg-white/10'}`} />
                )}
              </div>
            );
          })}
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isActive
                      ? 'border-[#D9B84C] shadow-[0_0_30px_rgba(217, 184, 76,0.15)]'
                      : isExpanded
                        ? `border-[rgba(217, 184, 76,0.4)]`
                        : 'border-[#E8E3D9] hover:border-[rgba(217, 184, 76,0.3)]'
                    }
                    ${isExpanded ? 'scale-[1.02] -translate-y-1' : 'hover:scale-[1.01]'}
                  `}
                  style={isExpanded ? { boxShadow: `0 0 40px ${tierGlows[tier.id] || 'rgba(217, 184, 76,0.15)'}` } : undefined}
                >
                  {isActive && (
                    <div className="mb-3">
                      <span className="text-xs font-semibold px-3 py-1 rounded-full bg-gradient-to-r from-[#D9B84C] to-[#D9B84C] text-[#0A0A0F] animate-pulse">
                  ...
                  <div className="space-y-3 mb-5">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[#0a0a0a] font-bold">{tier.threshold.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                  ...
                  <div className={`border-t border-[#E8E3D9] pt-4 space-y-2 transition-all duration-300 ${isExpanded ? 'opacity-100 max-h-[500px]' : 'opacity-70 max-h-[120px] overflow-hidden'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      <Check className="w-4 h-4 text-[#0a0a0a]/50 flex-shrink-0" />
        ...
        <div className="mt-8 p-6 rounded-2xl bg-gradient-to-r from-[rgba(217, 184, 76,0.08)] to-[rgba(217, 184, 76,0.03)] border border-[rgba(217, 184, 76,0.15)] text-center">
          ...
          <Link href="/sign-in">
            <a className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-[#D9B84C] to-[#D9B84C] text-[#0A0A0F] font-semibold rounded-xl transition-all duration-300 hover:shadow-[0_0_25px_rgba(217, 184, 76,0.3)] hover:scale-105 active:scale-95">
