              ...
              <div key={tier.id} className="flex items-center">
                <button
                  ...
                  className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-500 shrink-0
                    ${isCurrent
                      ? 'ring-2 ring-[#C9A96E] ring-offset-2 ring-offset-[#0A0A0F] scale-110'
                      : isReached
                        ? 'opacity-100'
                        : 'opacity-40'
                    }
            ...
            {TIER_CONFIGS.map((tier, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {isLocked && !isActive && (
                    <div className="mb-3">
                      <span className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1 rounded-full bg-white/5 text-white/30">
                  ...
                  <div className={`border-t border-[rgba(232,230,240,0.08)] pt-4 space-y-2 transition-all duration-300 ${isExpanded ? 'opacity-100 max-h-[500px]' : 'opacity-70 max-h-[120px] overflow-hidden'}`}>
                    ...
                    {tier.benefits.exclusiveAccess && (
                      <div className="flex items-center gap-2">
                        <Check className="w-4 h-4 text-[#C9A96E] flex-shrink-0" />
                        <span className="text-white/60 text-xs">{isHebrew ? 'גישה בלעדית לאירועים' : 'Exclusive Event Access'}</span>
                      </div>
                    )}
                    {tier.benefits.conciergeService && (
                      <div className="flex items-center gap-2">
                        <Check className="w-4 h-4 text-[#C9A96E] flex-shrink-0" />
                        <span className="text-white/60 text-xs">{isHebrew ? 'שירות קונסיירז׳ אישי' : 'Personal Concierge Service'}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
