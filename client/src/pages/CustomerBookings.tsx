...
function ProviderAvatar({ name, size = 44 }: { name: string; size?: number }) {
  ...
  return (
    <div
      ...
      style={{ width: size, height: size, background: `linear-gradient(135deg,${GOLD},#D9B84C)`, fontSize: size * 0.34 }}
    >
      {initials || '?'}
    </div>
  );
    ...
    <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
      ...
      <Link href={c.cta_href}>
        <button
          ...
          style={{ background: `linear-gradient(135deg,${GOLD},#D9B84C)` }}
        >
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {(booking.loyaltyRedeemedCents ?? 0) > 0 && (
                <div className="mt-2">
                  <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#D9B84C]/10 text-[#7A5C1E] border border-[#D9B84C]/20">
                    <Coins size={9} />
                    {isRTL
