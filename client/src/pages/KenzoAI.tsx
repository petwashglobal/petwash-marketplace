...
const uiLabels: Record<Language, { title: string; subtitle: string; placeholder: string; back: string; thinking: string; welcome: string }> = {
  he: { title: '🐾 Kenzo AI', subtitle: 'העוזר החכם של ⁦PetWash™⁩', placeholder: 'שאל את קנזו...', back: 'חזרה', thinking: 'קנזו חושב...', welcome: 'שלום! 🐾 אני קנזו, הגולדן רטריבר הלבן של ⁦PetWash™⁩. איך אוכל לעזור לך היום?' },
  en: { title: '🐾 Kenzo AI', subtitle: '⁦PetWash™⁩ Smart Assistant', placeholder: 'Ask Kenzo...', back: 'Back', thinking: 'Kenzo is thinking...', welcome: 'Hello! 🐾 I\'m Kenzo, the white Golden Retriever of ⁦PetWash™⁩. How can I help you today?' },
  ar: { title: '🐾 Kenzo AI', subtitle: 'المساعد الذكي لـ ⁦PetWash™⁩', placeholder: 'اسأل كنزو...', back: 'رجوع', thinking: 'كنزو يفكر...', welcome: 'مرحباً! 🐾 أنا كنزو، كلب الغولدن ريتريفر الأبيض من ⁦PetWash™⁩. كيف يمكنني مساعدتك اليوم؟' },
  ru: { title: '🐾 Kenzo AI', subtitle: 'Умный помощник ⁦PetWash™⁩', placeholder: 'Спросите Кензо...', back: 'Назад', thinking: 'Кензо думает...', welcome: 'Привет! 🐾 Я Кензо, белый Голден Ретривер из ⁦PetWash™⁩. Чем могу помочь сегодня?' },
  fr: { title: '🐾 Kenzo AI', subtitle: 'Assistant intelligent ⁦PetWash™⁩', placeholder: 'Demandez à Kenzo...', back: 'Retour', thinking: 'Kenzo réfléchit...', welcome: 'Bonjour! 🐾 Je suis Kenzo, le Golden Retriever blanc de ⁦PetWash™⁩. Comment puis-je vous aider aujourd\'hui?' },
  es: { title: '🐾 Kenzo AI', subtitle: 'Asistente inteligente ⁦PetWash™⁩', placeholder: 'Pregunta a Kenzo...', back: 'Volver', thinking: 'Kenzo está pensando...', welcome: '¡Hola! 🐾 Soy Kenzo, el Golden Retriever blanco de ⁦PetWash™⁩. ¿Cómo puedo ayudarte hoy?' },
};
      ...
      <div className="w-full max-w-3xl mx-auto flex flex-col h-screen px-3 sm:px-4">
        ...
        <div className="flex items-center justify-between py-3 shrink-0">
          <Link href="/">
            <button
              ...
              style={{ background: 'rgba(217, 184, 76,0.15)', color: '#0a0a0a', border: '1px solid rgba(217, 184, 76,0.3)' }}
            >
              <ArrowLeft className="w-4 h-4" />
              <span>{labels.back}</span>
            </button>
          </Link>
          <img src="/brand/petwash-logo-white-bg.png" alt="⁦PetWash™⁩" className="h-8 opacity-90" />
          <div className="flex items-center gap-2">
            <Globe className="w-4 h-4" style={{ color: '#0a0a0a' }} />
        ...
        <div className="flex flex-wrap gap-1.5 justify-center py-2 shrink-0">
          {languages.map((lang) => (
            <button
              ...
              style={{
                background: language === lang.code ? 'rgba(217, 184, 76,0.25)' : '#FFFFFF',
                color: language === lang.code ? '#D9B84C' : '#6A6A6A',
                border: language === lang.code ? '1px solid rgba(217, 184, 76,0.5)' : '1px solid #E8E3D9',
              }}
            >
        ...
        <div className="flex flex-col items-center py-3 shrink-0">
          <Kenzo3DRealistic
            ...
            size="md"
          />
          <h1 className="text-xl font-bold mt-2" style={{ color: '#0a0a0a' }}>{labels.title}</h1>
        ...
        <div
          ...
          style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(217, 184, 76,0.3) transparent' }}
        >
          {messages.map((msg) => (
            <div
              key={msg.id}
            ...
            <div className={`flex items-end gap-2 ${isRTL ? 'flex-row-reverse justify-end' : 'flex-row justify-start'}`}>
              ...
              >
                ...
                <span className="flex gap-0.5">
                  <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: '#D9B84C', animationDelay: '0ms' }} />
                  <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: '#D9B84C', animationDelay: '150ms' }} />
                  <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: '#D9B84C', animationDelay: '300ms' }} />
                </span>
              </div>
            </div>
          )}
        ...
        >
          <input
            ...
            onFocus={(e) => { e.currentTarget.style.borderColor = 'rgba(217, 184, 76,0.5)'; }}
          ...
          <button
            ...
            style={{
              background: input.trim() ? 'rgba(217, 184, 76,0.3)' : '#FFFFFF',
              border: '1px solid rgba(217, 184, 76,0.4)',
              color: '#0a0a0a',
            }}
          >
