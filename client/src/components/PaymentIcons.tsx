...
export default function PaymentIcons({ variant = 'default', showTitle = false }: PaymentIconsProps) {
  ...
  const height = isCompact ? 'h-8 sm:h-10' : 'h-10 sm:h-12 md:h-14';

  return (
    <div className="w-full" data-testid="payment-icons-container">
      {/* Payment Methods Grid - Modern 2025 Layout */}
      <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 md:gap-6 max-w-4xl mx-auto">
        
        {/* Visa */}
        <div 
          className={`bg-white rounded-lg px-4 py-2 flex items-center justify-center ${height} transition-transform hover:scale-105`}
          title="Visa"
          data-testid="payment-icon-visa"
        >
          <svg viewBox="0 0 141.732 141.732" className="h-full w-auto" aria-label="Visa">
            <g>
              <rect y="0" fill="#1A1F71" width="141.732" height="141.732"/>
              <g>
        ...
        >
          <svg viewBox="0 0 131.39 86.9" className="h-full w-auto" aria-label="Mastercard">
            <g>
        ...
        >
          <svg viewBox="0 0 152.4 152.4" className="h-full w-auto" aria-label="American Express">
            <g>
              <rect fill="#006FCF" width="152.4" height="152.4"/>
        ...
        {/* Diners Club - Modern 2025 Design */}
        <div 
          className={`bg-white rounded-lg px-4 py-2 flex items-center justify-center ${height} transition-transform hover:scale-105`}
          title="Diners Club"
          data-testid="payment-icon-diners"
        >
          <svg viewBox="0 0 780 500" className="h-full w-auto" aria-label="Diners Club">
        ...
        {/* Apple Pay - Official 2025 Logo */}
        <div 
          className={`bg-black rounded-lg px-4 py-2 flex items-center justify-center ${height} transition-transform hover:scale-105`}
          title="Apple Pay"
          data-testid="payment-icon-applepay"
        >
          <svg viewBox="0 0 165.52 105.97" className="h-full w-auto" aria-label="Apple Pay">
            <g>
              <g>
        ...
        {/* Google Pay - Official 2025 Logo */}
        <div 
          className={`bg-white rounded-lg px-4 py-2 flex items-center justify-center ${height} transition-transform hover:scale-105`}
          title="Google Pay"
          data-testid="payment-icon-googlepay"
        >
          <svg viewBox="0 0 512 244" className="h-full w-auto" aria-label="Google Pay">
            <g>
      ...
      {/* Nayax Payment Gateway Badge - Separated for clarity */}
      <div className="mt-6 sm:mt-8 flex justify-center">
        <div 
          className="group cursor-default inline-flex items-center gap-2 sm:gap-3 bg-gradient-to-r from-emerald-500 via-teal-600 to-cyan-600 px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg shadow-md hover:shadow-lg transition-all duration-300"
