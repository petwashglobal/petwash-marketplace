...
export function AccessibilityButton({ language }: AccessibilityButtonProps) {
  ...
  return (
    <>
      <button
        onClick={toggleMenu}
        className="floating-button-accessibility bg-black text-white rounded-full p-3 sm:p-4 shadow-lg hover:bg-gray-800 transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-[#D4AF37] focus:ring-opacity-50 min-w-[48px] min-h-[48px] flex items-center justify-center"
        aria-label={l.aria}
        title={l.text}
        tabIndex={0}
      >
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[10000] focus:bg-black focus:text-white focus:px-4 focus:py-2 focus:rounded-md focus:outline-none focus:ring-4 focus:ring-[#D4AF37]"
      >
        {l.skip}
      </a>
